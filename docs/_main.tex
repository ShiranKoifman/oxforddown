...
\caption{\label{tab:ST-AgeLMEM}ST: Age effect analysis using LMEM for SRdTs measured across condition, speech material, age and group (children with \& without an APD sibling, 0/1) as fixed factors and random intercepts for subjects. Reference levels: Condition = Quiet-NoAlt, Material = ASL, Group = none sibling (0). Note: only the control group data following outliers trimming was included (trimmed TD).}
\centering
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{8cm}cc>{}c}
\toprule
\multicolumn{4}{l}{SRdT \textasciitilde{} Condition + Material + Age + APDsibling + Condition:Material +} \\
\multicolumn{4}{r}{Condition:Age + Material:Age + Condition: APDsibling(1 | Subjects)} \\
...
\midrule
Quiet-NoAlt & 0.19 & 0.03 & 222.42 & 7.26 & \em{\textbf{< 0.001}} & 0.14 - 0.24\\
Quiet-Alt & 0.19 & 0.03 & 222.16 & 7.56 & \em{\textbf{< 0.001}} & 0.14 - 0.24\\
AMSSN-Alt & 0.18 & 0.03 & 222.24 & 6.97 & \em{\textbf{< 0.001}} & 0.13 - 0.23\\
MDR\_F-Alt & 0.23 & 0.03 & 222.16 & 9.26 & \em{\textbf{< 0.001}} & 0.18 - 0.28\\
ENG\_F-Alt & 0.27 & 0.03 & 222.12 & 10.57 & \em{\textbf{< 0.001}} & 0.22 - 0.32\\
\bottomrule
...
\caption{\label{tab:ST-AgeLMEMpost1}ST: Age-effect - post-hoc paired comparison t-test for Material (ASL/CCRM) x APDsibling (0/1) interaction. The test was performed on the fitted LMEM model and included adjusted least-squared-mean for the random intercept (subjects) using lsmeans package \autocite[emmeans package;][]{emmeansPackageR}.}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lcccc>{}cc}
\toprule
...
\midrule
ASL 0 - CCRM 0 & 0.24 & 0.01 & 222.02 & 17.06 & \em{\textbf{< 0.001}} & 0.21 - 0.26\\
ASL 0 - ASL 1 & 0.01 & 0.03 & 35.16 & 0.21 & \em{0.84} & -0.06 - 0.07\\
ASLN 0 - CCRM 1 & 0.19 & 0.03 & 36.33 & 5.84 & \em{\textbf{< 0.001}} & 0.13 - 0.26\\
CCRM 0 - ASL 1 & -0.23 & 0.03 & 34.87 & -7.08 & \em{\textbf{< 0.001}} & -0.3 - -0.16\\
CCRM 0 - CCRM 1 & -0.04 & 0.03 & 36.93 & -1.36 & \em{0.18} & -0.11 - 0.02\\
ASL 1 - CCRM 1 & 0.19 & 0.02 & 222.47 & 9.44 & \em{\textbf{< 0.001}} & 0.15 - 0.22\\
\bottomrule
\multicolumn{7}{l}{\textsuperscript{*} significant p-values (p < 0.05) are shown in bold.}\\
\end{tabular}}
\end{table}
...
Lastly, the significant interaction between test Material and Group was explored using two separate LMEM models for ASL and CCRM data. Both models found no significant effect of Group. Comparison of the average performance by material between the two control groups revealed an opposite direction of performance. While the performance of the TD children with APD sibling was on average 0.04 better than their TD peers for the ASL material, their performance was 0.02 poorer for the CCRM material. These results are in contradiction to our expectation in case differences between the two TD groups were measurable. In such event, we predicted a larger increment in performance (i.e., poorer score) for the ASL sentences which are more linguistically challenging than for the CCRM sentences. These results and the very small estimated mean difference (0.052) for the interaction between Material and Group in the full model suggests that the differences picked up by the model are due to sampling error. Alternatively, it is also possible that the switching task is not sensitive enough to detect smaller changes in performance between the two control groups.\\
...
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{l|>{}ccccccc|>{}cccccc>{}c}
\toprule
\multicolumn{1}{c}{ } & \multicolumn{7}{c}{ASL model} & \multicolumn{7}{c}{CCRM model} \\
\cmidrule(l{3pt}r{3pt}){2-8} \cmidrule(l{3pt}r{3pt}){9-15}
contrast & estimate & n1 & n2 & 95\%-CI & p & r & magnitude & estimate & n1 & n2 & 95\%-CI & p & r & magnitude\\
\midrule
APD - TD$_{none}$ & 0.89 & 100 & 75 & 0.46 - 1.34 & \textbf{< 0.001} & 0.30 & moderate & 0.96 & 100 & 75 & 0.58 - 1.33 & \textbf{< 0.001} & 0.37 & \em{moderate}\\
APD - TD$_{sibling}$ & 1.15 & 100 & 40 & 0.6 - 1.68 & \textbf{< 0.001} & 0.33 & moderate & 0.40 & 100 & 40 & -0.1 - 0.94 & 0.12 & 0.13 & \em{small}\\
TD$_{none}$ - TD$_{sibling}$ & 0.24 & 75 & 40 & -0.23 - 0.7 & 0.33 & 0.09 & small & -0.57 & 75 & 40 & -1.03 - -0.1 & \textbf{0.02} & 0.22 & \em{small}\\
\bottomrule
...
A factorial design model with repeated measures was used to test the main effects for Condition (SSN, SON0, S0N90, \& SRM), Age, and Group (TD children with/without APD sibling, 1/0) with TD group SRTs as a dependent variable. Interaction terms were included as well as a random intercept for subjects. Note that also here the model included only data for the control group. Assumptions of normal distribution and homogeneity were met, and thus a parametric approach was applied using LMEM (reference levels: Condition = SSN; Group = TD without APD sibling, 0). The model with the best fit and main effects are given in Table~\ref{tab:LiSNS-AgeLMEMTab}. There was a significant interaction between Condition x Age (p~\textless~0.05), thus indicating that age affected performance differently across the different test conditions. Moreover, there was no significant effect for the main effect of Group (p~\textgreater~0.05), thus suggesting that performance of TD children with and without an APD sibling was fairly similar.

\begin{table}

\caption{\label{tab:LiSNS-AgeLMEMTab}LiSNS-UK: Age effect - LMEM model for SRT with condition, age and group (TD children with \& without APD sibling, 1/0) as fixed factors and random intercepts for subjects (reference level: SSN, Group = TD without APD sibling, 0). Note: only data measured with the control group following outliers trimming was included.}
\centering
\begin{tabular}[t]{lcc>{}c}
\toprule
...
\caption{\label{tab:ENVASA-AgeLMEMTab}ENVASA: Age effect - LMEM model for PC (\%-correct) in the three background measures single, dual, \& combind background/s, group (TD children with/without APD siblings), and age as fixed factors and random intercepts for subjects (reference levels: Background = single-background, Group = non-APD sibling, 0). Note: only data measured with the control group following outliers trimming was included.}
\centering
\begin{tabular}[t]{lcc>{}c}
\toprule
...
Effect-size {[}Cohen's d; \emph{rstatix::cohens\_d()}{]} was calculated for pairwise group comparisons by material \& condition. Three test conditions resulted in a `large' effect size: \emph{CCRM\_F-Alt-CCRM} (d = 1.01), \emph{AMSSN-Alt-CCRM} (d = 1.00) and \emph{MDR\_F-ASL-Alt} (d = 0.85). Four other conditions resulted in a `moderate' effect-size, with d ranging between 0.63 to 0.75. These conditions comprised of speech distractors (ENG or MDR) from either material and Quiet-NoAlt for the ASL material. The test material which resulted in the largest effect-size was estimated by averaging d across conditions for each material. Both materials had a `moderate' average effect-size, whereby the CCRM material had the largest effect-size of 0.69, following with 0.56 for the ASL material.\\

\begin{table}

\caption{\label{tab:unnamed-chunk-14}Cohen's d by condition and material.}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lllrllrrrrl}
\toprule
.y. & group1 & group2 & effsize & material & CondCode & n1 & n2 & conf.low & conf.high & magnitude\\
\midrule
z & APD & TD & 0.63 & ASLN & Q-ASLN-NoAlt & 20 & 23 & 0.02 & 1.72 & moderate\\
z & APD & TD & 0.18 & ASLN & Q-ASLN-Alt & 20 & 23 & -0.39 & 0.98 & negligible\\
z & APD & TD & 0.49 & ASLN & AMSSN-ASLN-Alt & 20 & 23 & -0.15 & 1.57 & small\\
z & APD & TD & 0.85 & ASLN & MDR\_F-ASLN-Alt & 20 & 23 & 0.20 & 1.54 & large\\
z & APD & TD & 0.64 & ASLN & ENG\_F-ASLN-Alt & 20 & 23 & 0.05 & 1.31 & moderate\\
z & APD & TD & 0.34 & CCRM & Q-CCRM-NoAlt & 20 & 23 & -0.26 & 0.97 & small\\
z & APD & TD & 0.35 & CCRM & Q-CCRM-Alt & 20 & 23 & -0.27 & 1.07 & small\\
z & APD & TD & 1.01 & CCRM & AMSSN-CCRM-Alt & 20 & 23 & 0.49 & 1.79 & large\\
z & APD & TD & 0.76 & CCRM & MDR\_F-CCRM-Alt & 20 & 23 & 0.20 & 1.38 & moderate\\
z & APD & TD & 0.64 & CCRM & ENG\_F-CCRM-Alt & 20 & 23 & -0.01 & 1.36 & moderate\\
z & APD & TD & 1.01 & CCRM & CCRM\_F-CCRM-Alt & 20 & 23 & 0.43 & 1.86 & large\\
\bottomrule
\end{tabular}}
\end{table}
