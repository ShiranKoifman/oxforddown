...
{\centering \includegraphics[width=1\linewidth]{_main_files/figure-latex/ST-EMMplot-1} 
...
Lastly, the significant interaction between Material and Sibling was examined with a model-based post-hoc t-tests comparison (see Table~\ref{tab:ST-AgeLMEMpost1}). There was no significant difference in SRdTs between the two TD groups for both speech materials (both \(p\)'s \textgreater{} 0.05). The results suggests that the differences between the two groups depends upon the material type. This is not surprising considering the large differences between the two material types, where the CCRM sentences are expected to be more intelligible.\\

\begin{correction}
\textbf{Discussion???}\\
Differences in means between the two groups within speech material were
generally very small. Interestingly, however the model based estimated
marginal means for performance of the TD\(_{sib}\) children was on
average 0.008 better than their TD peers for the ASL material and 0.04
poorer for the CCRM material. These results are in contradiction to our
expectations if any differences arose. In such a case, we would have
predicted a larger decrement in performance (i.e., poorer score) for the
ASL sentences which are more linguistically challenging than for the
CCRM sentences. Nonetheless, the very small estimated mean differences
and the negligible effect-sizes suggests that the differences picked up
by the model are due to sampling error.\\
\end{correction}
...
\midrule
TD - TD$_{sib}$ & ASL & 0.01 & 0.03 & 45.37 & \em{0.32} & 0.75 & -0.04 - 0.06 & 0.19 & negligible\\
TD - TD$_{sib}$ & CCRM & -0.04 & 0.03 & 45.81 & \em{-1.58} & 0.12 & -0.09 - 0.01 & -0.13 & negligible\\
\bottomrule
...
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lcclcc>{}ccl}
\toprule
contrast & n1 & n2 & material & estimate & 95\%-CI & p-value & d & magnitude\\
\midrule
APD - TD & 100 & 75 & ASL & 0.89 & 0.46 - 1.34 & \em{\textbf{< 0.001}} & 0.49 & small\\
APD - TD$_{sib}$ & 100 & 40 & ASL & 1.15 & 0.6 - 1.68 & \em{\textbf{< 0.001}} & 0.65 & moderate\\
TD - TD$_{sib}$ & 75 & 40 & ASL & 0.24 & -0.23 - 0.7 & \em{1} & 0.14 & negligible\\
\midrule
APD - TD & 100 & 75 & CCRM & 0.96 & 0.58 - 1.33 & \em{\textbf{< 0.001}} & 0.76 & moderate\\
APD - TD$_{sib}$ & 100 & 40 & CCRM & 0.40 & -0.1 - 0.94 & \em{0.34} & 0.38 & small\\
TD - TD$_{sib}$ & 75 & 40 & CCRM & -0.57 & -1.03 - -0.1 & \em{0.05} & -0.42 & small\\
\bottomrule
\multicolumn{9}{l}{\textsuperscript{*} significant p-values (p < 0.05) are shown in bold.}\\
\end{tabular}}
\end{table}
...
\paragraph{Proportion of duty cycle (DC)}\label{proportion-of-duty-cycle-dc}}
...
{\centering \includegraphics[width=1\linewidth]{_main_files/figure-latex/ST-DerivedPlotsuRevs-1} 
...
\paragraph{Age-independent z-scores}\label{age-independent-z-scores-1}}

\hfill\break
As before, descriptive statistics for the listeners' z-scores for the derived scores are given in Table~\ref{tab:ST-DerivedTab} and plotted in Figure~\ref{fig:ST-DerivedPlotsz}. Overall, differences between the groups were larger for IM than for the switching cost. Furthermore, the percentage of abnormal performance is generally low in both test materials (range: 0\% - 25\%), with noticeably larger proportion in the CCRM material. Only one APD child (5\%) obtained abnormal score for the ASL material, when presented with the ENG\_F distractor. None of the TD children exhibited abnormal score for IM conditions. Surprisingly however, three TD children (\textasciitilde13\%) were significantly hampered when switching was introduced. As for the CCRM material, the proportion of abnormal score of the TD group ranged between 0\% to circa 9\% (2/23).\\

\begin{table}

\caption{\label{tab:ST-DerivedTab}ST: Descriptives for standardised residuals (z-scores) calculated for data measured with the ASL and CCRM speech material.}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lcccccl|>{}lccccc}
\toprule
\multicolumn{1}{c}{ } & \multicolumn{6}{c}{APD} & \multicolumn{6}{c}{TD} \\
\cmidrule(l{3pt}r{3pt}){2-7} \cmidrule(l{3pt}r{3pt}){8-13}
...
\addlinespace[0.3em]
\multicolumn{13}{l}{\textbf{ASL}}\\
\hspace{1em}Switching & 20 & -0.07 & 0.80 & -1.05 & 1.67 & 0.00\% & 23 & -0.11 & 1.19 & -1.28 & 2.51 & 13.04\%\\
\hspace{1em}AMSSN & 20 & 0.43 & 1.02 & -1.75 & 1.88 & 0.00\% & 23 & -0.22 & 0.99 & -1.79 & 1.65 & 0.00\%\\
\hspace{1em}MDR\_F & 20 & 0.55 & 0.74 & -0.66 & 2.21 & 5.00\% & 23 & -0.22 & 1.07 & -2.37 & 1.80 & 0.00\%\\
\hspace{1em}ENG\_F & 20 & 0.60 & 0.67 & -1.15 & 1.50 & 0.00\% & 23 & -0.01 & 1.13 & -2.93 & 1.91 & 0.00\%\\
\addlinespace[0.3em]
\multicolumn{13}{l}{\textbf{CCRM}}\\
\hspace{1em}Switching & 20 & 0.39 & 1.06 & -2.14 & 2.05 & 10.00\% & 23 & -0.15 & 1.66 & -5.09 & 3.95 & 4.35\%\\
\hspace{1em}AMSSN & 20 & 0.13 & 1.18 & -0.94 & 2.48 & 20.00\% & 23 & -0.02 & 1.08 & -2.42 & 1.68 & 0.00\%\\
\hspace{1em}MDR\_F & 20 & 0.52 & 0.94 & -1.17 & 2.15 & 5.00\% & 23 & -0.06 & 1.15 & -2.02 & 2.57 & 8.70\%\\
\hspace{1em}ENG\_F & 20 & 0.41 & 1.27 & -2.32 & 3.58 & 10.00\% & 23 & 0.00 & 1.62 & -3.51 & 4.25 & 4.35\%\\
\hspace{1em}CCRM\_F & 20 & 0.61 & 1.20 & -1.97 & 2.55 & 25.00\% & 23 & 0.01 & 1.01 & -1.85 & 1.64 & 0.00\%\\
\bottomrule
\multicolumn{13}{l}{\textsuperscript{} abnormal: defined as the percentage of abnormal z-score > 1.96.}\\
\multicolumn{13}{l}{\textsuperscript{} Switching = Quiet-Alt - Quiet-NoAlt; AMSSN = AMSSN - Quiet-Alt}\\
\multicolumn{13}{l}{\textsuperscript{} MDR\_F = MDR\_F - Quiet-Alt; ENG\_F = ENG\_F - Quiet-Alt; CCRM\_F = CCRM\_F - Quiet-Alt}\\
\end{tabular}}
\end{table}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{_main_files/figure-latex/ST-DerivedPlotsz-1} 
...
Group differences were examined using two separate LMEM models for switching and IM effects, excluding data measured with the CCRM\_F distractor. Parametric methods assumptions were met for IM data, whereas the assumption of normal distribution for the switching data was rejected. Nevertheless, since nonparametric methods gave similar results, it was decided to report here only the outcomes of the parametric method. Switching effect were examined using LMEM model with Material and Group (APD/TD) as fixed factors and random intercepts for subjects (reference levels: Material = ASL; Group = APD). The final model which gave the best fit did not include Material x Group interaction. There was no significant main effect of both Material (\(\chi^{2}\) = 0.00016, \(Df\) = 1, \(p\) = 0.990) and Group (\(\chi^{2}\) = 0.380, \(Df\) = 1, \(p\) = 0.538), thus indicating that the switching cost was the same in both groups. Similarly, group differences across the three distractors were tested using LMEM model with Distractor, Material and Group as fixed factors and random intercepts for subjects. The final model and main effects are given in Table~\ref{tab:ST-zIMEffTab}. There was a significant main effect of Group (\(p\) \textless{} 0.05), whereas there was no significant main effect for Distractor or Material (\(p\) \textgreater{} 0.05).\\

\begin{table}

\caption{\label{tab:ST-zIMEffTab}ST: Derived measures (z-score): LMEM model with Distractor (AMSSN, MDR\_F, and ENG\_F), Material (ASL/CCRM) and Group (APD/TD) as fixed factors and random intercepts for subjects. Reference levels: Distractor = AMSSN; Material = ASL; Group = APD).}
\centering
\begin{tabular}[t]{lcc>{}c}
\toprule
\multicolumn{4}{l}{z \textasciitilde{} Distractor + Material + Group + (1 | Subjects)} \\
\cmidrule(l{3pt}r{3pt}){1-4}
Main effects & $Df$ & $\chi^{2}$ & $p$\\
\midrule
Distractor & 2 & 0.702 & \em{0.704}\\
Material & 1 & 0.000 & \em{0.984}\\
Group & 1 & 5.089 & \em{\textbf{0.024}}\\
\bottomrule
\multicolumn{4}{l}{\textsuperscript{*} significant p-values (p < 0.05) are shown in bold.}\\
\end{tabular}
\end{table}

A separate LMEM model for the full CCRM data, including the CCRM\_F distractor, gave similar results, with a significant main effect of Group (\(\chi^{2}\) = 5.089, \(Df\) = 1, \(p\) = 0.024), whereas there as no signifiacnt main effect of Distractor (\(\chi^{2}\) = 0.990, \(Df\) = 3, \(p\) = 0.804).\\
...
\hypertarget{exploratory-predictors-td-vs.-td_sib}{%
\subsubsection{\texorpdfstring{Exploratory predictors -- TD vs.~TD\(_{sib}\)}{Exploratory predictors -- TD vs.~TD\_\{sib\}}}\label{exploratory-predictors-td-vs.-td_sib}}
...
Looking at the boxplots, two noticeable observations emerges. Firstly, the TD\(_{sib}\) children CELF-RS scores are noticeably poorer than compared with the TD children. \textbf{To elaborate?} Give Mdn's? Both groups still perform significantly better than the APD group..
...
Nonetheless, differences between the two TD groups are relatively small and were found insignificant (see ECLiPS analysis in section ???). In contrast, differences in rating between the groups for the CCC-2 GCC score were much smaller. A possible explanations for the smaller trend in the CCC-2 is that\ldots.\\
...
\hypertarget{peripheral-hearing-evaluation}{%
\subsection{Peripheral hearing evaluation}\label{peripheral-hearing-evaluation}}

\hypertarget{standard-audiometry-1}{%
\subsubsection{Standard Audiometry}\label{standard-audiometry-1}}

\begin{itemize}
\tightlist
\item
  Group difference in the left ear. The small difference (\textasciitilde2.5 dB) suggests it is due to sampling error rather than a real effect. Any similar effect in other studies? Hunter et al., 2020 did not find sig. difference between the ears.
\end{itemize}

\hypertarget{extended-audiometry}{%
\subsubsection{Extended Audiometry}\label{extended-audiometry}}
...
\hypertarget{central-auditory-evaluation}{%
\subsection{Central auditory evaluation}\label{central-auditory-evaluation}}

\hypertarget{st-2}{%
\subsubsection{ST}\label{st-2}}
...
siblings is a useful way to examine both genetic and environmental
...
factors were mostly heritable or acquired by the child's environment, we
hypothesised that the control children with APD sibling will perform
...
\textbf{Developmental Psychoacoustics BOOK, Chapter 4: Individual Differences in Auditory Capability Among Preschool Children (Frederic Wightman and Prudence Allen):}
...
\hypertarget{speech-in-noise-where-does-this-go}{%
\subsubsection{Speech in noise (where does this go?)}\label{speech-in-noise-where-does-this-go}}
...
\hypertarget{lisns-uk-1}{%
\subsubsection{LiSNS-UK}\label{lisns-uk-1}}

\hypertarget{cognitive-and-language-evaluation}{%
\subsection{Cognitive and language evaluation}\label{cognitive-and-language-evaluation}}

\hypertarget{ccc-2-1}{%
\subsubsection{CCC-2}\label{ccc-2-1}}

\hypertarget{eclips-1}{%
\subsubsection{ECLiPS}\label{eclips-1}}
...
\subsubsection{Language PCA}\label{language-pca}}
