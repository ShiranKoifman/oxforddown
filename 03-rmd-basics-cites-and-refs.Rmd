...
\
The locally developed Listening in Spatialised Noise Sentences UK (LiSNS-UK) assesses the ability to use binaural cues in speech-on-speech listening conditions. The test development, speech material normalisation, and norms standardisation followed @Cameron2007 development steps and are described in detail in Chapter\ \@ref(Chpt2). The test uses virtualisation techniques to create spatial distribution of sound sources in space for headphones presentation where target sentences [ASL; @MacLeod1990] are presented in two simultaneous speech distractors (unrelated children's stories spoken by the target talker). The LiSNS-UK comprises of two main listening conditions, differing in their availability of spatial cues. The target sentences are configured to always appear in front of the listener's head, at 0$^{\circ}$ azimuth on the horizontal plane, with the two streams of speech distractors either collocated in space with the target (S0N0), resulting in relatively poor speech perception, or offset in space, with one distractor to either side of the target at $\pm$ 90$^{\circ}$. The spatial separation in the later condition results in an improvement in speech perception of circa 13 dB [@Cameron2011], typically termed as spatial release from masking (SRM). This SRM advantage is calculated by taking the difference between performance in the collocated and the separated condition.\
...
car::leveneTest(residuals(w1) ~ Freq * Ear * Group, data = d_L)
...
```{r,label='ST-AgeEffectAPDsib', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE,eval=FALSE}
##### Age effect? ################################################################# 
...
tab_model(ST.m,title="Full model")
tab_model(ST_ASL.m,title="ASL only")
tab_model(ST_CCRM.m,title="CCRM only")
...
[^Chapt3-2]: A separate model for CCRM data only, also gave a significant Condition x Age interaction (p\ \<\ 0.001).
...
```{r,label='ST-zAssumptions2', message=FALSE, warning=FALSE, include=FALSE, results='hide'}
# remove CCRM_F condition
CCRM_s <- CCRM %>% filter(CondCode!="CCRM_F-CCRM-Alt") %>% 
  dplyr::select(., listener, Group, APDsibling, material, CondCode, z_trim) %>% droplevels()
...
ST$Group2 <- ifelse(ST$Group=="APD","APD",ifelse(ST$APDsibling<1,"TD_0","TD_1"))
ST$Group2 <- as.factor(ST$Group2)
...
# linear model:
w1 <- lm(z_trim ~ CondCode * Group2 * material, data = ST)
...
# Option 2: by fixed factors: --> mostly normal..
NormTest <- ST %>%
  group_by(CondCode, Group2, material) %>%
  rstatix::shapiro_test(z_trim)
...
# Option 1: Is the lm model fully covered here?
leveneTest(residuals(w1) ~ CondCode * Group2 * material, data = ST)
# -> homogeneity of the full model is met
...
# VarTest <- ST %>%
#   rstatix::group_by(CondCode) %>%
#   rstatix::levene_test(z_trim ~ Group2*material)

# VarTest <- ST %>%
#   rstatix::group_by(Group2) %>%
#   levene_test(z_trim ~ CondCode*material)

# VarTest <- ST %>%
#   rstatix::group_by(material) %>%
#   levene_test(z_trim ~ CondCode*Group2)
...
boxplot(residuals(w1) ~ CondCode * Group2 * material,data = ST)

plot(BestModel)

t1 <- ggplot(ST, aes(x=interaction(CondCode,Group2,material), y=BestModel.Res)) +
  geom_point(size=2,alpha=1) +
  # geom_text(label=ASLN$listener, size=3)+
  # labs(y = "SRdTs (proportion of duty cycle)",x = "Age (in years)") + 
  # scale_y_continuous(limits = c(-0.05,1.05),breaks=seq(0,1,0.1)) +
...
CCRM <- data.frame(CCRM)
# CCRM$Group2 <- ifelse(CCRM$Group=="APD","APD",ifelse(as.character(CCRM$APDsibling)<1,"TD_0","TD_1"))
CCRM$Group2 <- ifelse(CCRM$Group=="APD","APD",ifelse(CCRM$APDsibling<1,"TD_0","TD_1"))
CCRM$Group2 <- as.factor(CCRM$Group2)
...
To further examine the significant interaction found between Group and Material, we tested two separate f1.ld.f1 models  per test material with Group and Condition as predictors. Both models found a significant effect of Group [ASL: Statistic = 4.099, df=1.720, p\ < 0.05; CCRM: Statistic = 3.699, df = 1.605, p\ < 0.05], while there was no significant interaction or main effect of condition (all p's\ > 0.05). Wilcoxon rank-sum tests pairwise comparison of performance between the groups (see Table\ \@ref(tab:PostHocTab)). There was a highly significant difference in performance between the APD and the TD children without APD siblings in both test materials. Furthermore, erformance of APD children for ASL material was (highly) significantly poorer than the TD children with APD siblings, whereas there was no significant difference in performance between the two groups when measured with the CCRM material. Furthermore, while there was no significant difference between the TD groups when measured with the ASL material, TD children with APD siblings performed significantly poorer than their TD peers without APD siblings when presented with the CCRM material.\
...
Since data for the CCRM material with CCRM-type distractor was not included in the aforementioned model, a separate 3 x 6 model was computed for the CCRM speech material only. The model included Group and Condition as between- and within-subjects predictors, respectively, with z-scores as the dependent variable using nparLD ATS test (f1.ld.f1 design). The ATS test gave similar results to the full model, with a significant main effect of group (`r sprintf("Statistic = %0.3f, df = %0.3f, p < %0.3f", CCRMfull.nparLD[1,1], CCRMfull.nparLD[1,2],CCRMfull.nparLD[1,3])`), while there was no significant main effect for Condition nor a significant Group x Condition interaction (all p's > 0.05). A post-hoc pairwise comparison for groups (Wilcox signed-rank tests) found a significant difference between all three groups. 
...
# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf
kbl(ST_Z.postT, booktabs = T,escape = F, linesep = "",caption = "ST: Post-hoc paired comparison (Wilcoxon rank-sum test) for Group differences in z-score split by material.",
...
kbl(ST_zTab, booktabs = T,escape = F, linesep = "",caption = "ST: z-scores statistical analysis using 3x2x5 LMEM for the effects of Material, Condition, and Group as fixed factors and random intercepts for subjects. Groups comprised of three levels: APD children, and TD children with our without APD siblings. Note: data for the CCRM condition with CCRM-type sentences as distractor (CCRM_F) was not included in the model. Reference levels: Group = APD; Material = ASL; Condition = Quiet-NoAlt.",
    align = c("lccc"),format = "latex",digits = 3) %>% 
...
tab_model(model6)
m_withoutSRM <- model6
tab_model(m_withSRM,m_withoutSRM)
# ---------------------------------------------------------------------------------
# test main effects:
...
Eval <- merge(Eval, LiSNS_w[,c("listener","uRevs_S0N0","uRevs_S0N90","uRevs_SRM")],by="listener", na.rm=FALSE) %>% 
  droplevels()
...
```{block type='correction'}
*OME*: is a composite calculated based on the caregivers indication of history of **Ear infection** \& **Glue ear** in the background questionnaire. OME is 1 if response was 'Yes' to at least one of these items, whereas OME is 0 if response was 'No' for both items.
```

```{r echo=FALSE, fig.align='center', fig.asp=1.2, fig.cap="Association between predictors and performance in the APD group for the switching task composite (PC1.ST), language composite (PC1.Lang), SRM, standard and EHF pure-tone PTA. Predictors included: 1. APD diagnosis (APD vs. LiD), 2. SPD diagnosis (SPD vs. non-SPD), 3. Regular use of FM-device (FM vs. No FM), 4. History of OME (OME vs. No OME), 5. Pressure equalisation tube history (PET vs. No PET), and 6. Auditory training (Training vs. No training). Individual observations are marked in circles. Observations of children diagnosed with APD are filled in dark blue, and LiD observations are filled in light blue. Significant p-values for independent t-test comparison are marked with asterisk (p < 0.05).", fig.width=18, message=FALSE, warning=FALSE, label='EvalAPD-fig', out.width='100%'}
...
t2 <- ggplot(Eval_L, aes(x=SPD,y=score)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
  geom_quasirandom(dodge.width=0.8,shape=21,aes(fill=Diagnosis))+
  scale_fill_manual(values=c("blue","lightblue","black"))+
  facet_grid(. ~ CondCode, scales = "free", switch = "y")+
  labs(y="Weighted score" ,x = "") + 
  scale_y_continuous(limits = c(-7,7),breaks=seq(-7,7,2))+
  scale_x_discrete(breaks=c("1", "0",NA),labels=c("SPD", "non-SPD","TD"))+
...
t4 <- ggplot(Eval_L, aes(x=EarProblems,y=score)) +
  ...
  labs(y="Weighted score" ,x = "") + 
  scale_y_continuous(limits = c(-7,7),breaks=seq(-7,7,2))+
  scale_x_discrete(breaks=c("1", "0"),labels=c("OME", "No OME"))+
  stat_compare_means(comparisons = list( c("1","0")),label =  "p.signif", method = "t.test")+
  theme_bw()+
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
        axis.title.x = element_text(size=11, face="bold"),
...
t6 <- ggplot(Eval, aes(x=interaction(SPD,Group),y=zSRM)) +
  ...
  scale_x_discrete(breaks=c("1.APD","0.APD", NA),labels=c("SPD", "non-SPD","TD"))+
  stat_compare_means(comparisons = list( c("1.APD","0.APD")),label =  "p.signif", method = "t.test")+
  theme_bw()+
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
        axis.title.x = element_text(size=11, face="bold"),
