...
Two separate models were used to examine the effect of switching and IM with the derived scores as dependent variable (parametric model assumptions were met). The first model examined the effect of switching using a linear regression model with Material, Sibling and Age as predictors. The model that gave the best fit and test results are given in Table\ \@ref(fig:ST-AgeAltEffTab). There was a (marginally) significant interaction for Sibling x Age (p= 0.043). None of the remaining terms were significant (all p's \>\ 0.05). IM effect was tested using LMEM model for the three distractors (Distractor), Material, Sibling, and Age as fixed factors and random intercepts for subjects. The model that gave the best fit did not include the fixed factors Sibling or Age (see and main effects in Table\ \@ref(tab:ST-AgeIMEffTab). There was a highly significant main effect of Distractor (p\ \<\ 0.001) as well as a significant effect of Material (p\ \<\ 0.05). Post-hoc paired-comparison t-tests based on the fitted model for Distractor (with Bonferroni correction) found a significant difference in IM effect between AMSSN and MDR (Estimate = -0.24, 95\%-CI = -0.29 - -0.19, p\ \<\ 0.001) and AMSSN and ENG_F (Estimate = -0.28, 95\%-CI = -0.33 - -0.23, p\ \<\ 0.001), whereas there was no significant difference between the two speech distractors (Estimate = -0.04 95\%-CI = -0.09 - 0.01, p = 0.16)
...
kbl(ST_AgeTab_IM, booktabs = T,escape = F, linesep = "",caption = "ST: Add caption here.",
    align = c("lccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
...
kbl(ST_IM_DC_tab, booktabs = T,escape = F, linesep = "",row.names = F,caption = "ST: Add caption here.",
...
Final model: z_trim ~ CondCode + material + Group + (1 | listener)

Only main effect for Group is significant. 

**Analysis**
LMEM model with Distractor, Material and Group as fixed effects and random intercepts for subjects. Final model with best fit is given in Table\ \@ref(tab:ST-zIMEffTab)

- IM effect: significant main effect of group (Estimated mean difference -0.50, i.e., TD performance was on average 0.5 lower, i.e., better than the APD group)
```{r,label='ST-GroupEffectDerivedzIM', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
```{r,label='ST-GroupEffectDerivedzAlt', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
# Response: z_trim
#                 Chisq Df Pr(>Chisq)
# material       0.0002  1     0.9899
# Group          0.3818  1     0.5366
# material:Group 0.8219  1     0.3646

# Group:
(BestModel.1 <- update(BestModel,  . ~ . -Group))
Group.LMEM <- anova(BestModel,BestModel.1)
summary(BestModel.1)
# BestModel.1: z_trim ~ material + (1 | listener)
# BestModel: z_trim ~ material + Group + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# BestModel.1    4 284.98 294.79 -138.49   276.98                     
# BestModel      5 286.60 298.87 -138.30   276.60 0.3801  1     0.5375

#  Material
(BestModel.2 <- update(BestModel,  . ~ . -material))
Mat.LMEM <- anova(BestModel,BestModel.2)
summary(BestModel.2)
# estModel.2: z_trim ~ Group + (1 | listener)
# BestModel: z_trim ~ material + Group + (1 | listener)
#             npar   AIC    BIC logLik deviance Chisq Df Pr(>Chisq)
# BestModel.2    4 284.6 294.41 -138.3    276.6                    
# BestModel      5 286.6 298.87 -138.3    276.6 2e-04  1       0.99
...
ST_AltzTab[,c(2:3)]= apply(ST_IMzTab[,c(2:3)], 2, function(x) as.numeric(as.character(x)))
...
kbl(ST_IMzTab, booktabs = T,escape = F, linesep = "",caption = "ST: Add caption here.",
    align = c("lccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
...
- Group difference in the left ear. The small difference (~2.5) suggests it is due to sampling error rather than a real effect. Any similar effect in other studies? Hunter et al., 2020 did not find sig. difference between the ears. 
