...
```{r,label='ST-ASLLevsPC', fig.cap="add caption here", fig.width=9, fig.asp=0.7, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
ASL$CondCode <- revalue(ASL$CondCode, c("Q-ASLN-NoAlt"="Q-NoAlt","Q-ASLN-Alt"="Q-Alt",
                                        "AMSSN-ASLN-Alt"="AMSSN-Alt",
                                        "MDR_F-ASLN-Alt"="MDR_F-Alt",
                                        "ENG_F-ASLN-Alt"="ENG_F-Alt"))

ASL$CondCode <-factor(ASL$CondCode,levels=c("Q-NoAlt","Q-Alt","AMSSN-Alt",
                                            "MDR_F-Alt","ENG_F-Alt"))

ASL <- ASL %>% select(listener,group,CondCode,LevsPC,LevsPC_p) %>% droplevels()

p1 <- lattice::histogram(~ LevsPC | CondCode, data = ASL,
       type="count",
       xlab="LevsPC",
       ylab="Frequency",
       main="ASL: All LevsPC",
...
p2 <- lattice::histogram(~ LevsPC | CondCode, data = ASLOutliers,
       type="count",
       nint = 6,
       xlab="LevsPC",
       ylab="Frequency",
...
```{r,label='ST-CCRMLevsPC', fig.cap="add caption here", fig.width=9, fig.asp=0.7, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
# CCRM ---------------------------------------------------------------------------------
...
CCRM$CondCode <- revalue(CCRM$CondCode, c("Q-CCRM-NoAlt"="Q-NoAlt","Q-CCRM-Alt"="Q-Alt",
...
CCRM$CondCode <-factor(CCRM$CondCode,levels=c("Q-NoAlt","Q-CCRM-Alt","AMSSN-Alt",
                                              "MDR_F-Alt","ENG_F-Alt","CCRM_F-Alt"))

p1 <- lattice::histogram(~ LevsPC | CondCode, data = CCRM,
       type="count",
       xlab="LevsPC",
       ylab="Frequency",
...
p2 <- lattice::histogram(~ LevsPC | CondCode, data = CCRMOutliers,
       type="count",
       xlab="LevsPC",
       ylab="Frequency",
...
##### LiSNS-UK statistical analysis {.unnumbered}

##### ENVASA statistical analysis {.unnumbered}

##### CELF-RS statistical analysis {.unnumbered}
...
model1 <- lmer(uRevs~CondCode*material*Age+(1|listener)+(1|APDsibling), ST_trimmed, REML=FALSE)
summary(model1)
...
t1 <- t1 + facet_grid(. ~ reorder(CondCode,CondCode), scales = "free", switch = "y",
                      labeller = labeller(CondCode = c("Q-ASLN-NoAlt" ="Quiet-NoAlt",
...
t2 <- t2 + facet_grid(. ~ reorder(CondCode,CondCode), scales = "free", switch = "y",
                      labeller = labeller(CondCode = c("Q-CCRM-NoAlt" ="Quiet-NoAlt",
...
(ref:tabCap2) Post-hoc paired comparison t-test for Condition x Material two-way interaction. The test was performed on the fitted LMEM model and included adjusted least-squared-mean for the random intercept (subjects) using lsmeans package (REF).
...
kbl(CondMat.postLMEM,booktabs = T, escape = F, linesep = "",caption = '(ref:tabCap2)',
...
kbl(ST_tab,booktabs = T, escape = F, linesep = "",caption = "Switching task descriptives for standardised residuals (z-scores) calculated for data measured with the ASL and CCRM speech material.",
...
# get table
LiSNS_z_tab <- cbind(LiSNS_z_tab_APD,LiSNS_z_tab_TD[2:6])
colnames(LiSNS_z_tab)[1] = ""

# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf
kbl(LiSNS_z_tab, escape = F, booktabs = T, linesep = "", caption = "LiSNS standard residuals (z-scores) descriptives by group.",
    align = c("lccccccccc"),format = "latex") %>% 
  add_header_above(c(" ", "APD" = 5, "TD" = 5)) %>%
  column_spec(c(7),border_left = T)
```
...
kbl(LiSNS_AgeTab, booktabs = T,escape = F, linesep = "",caption = "Add caption here.",
    align = c("lccc"),format = "latex",digits = 3) %>% 
...
kbl(LiSNS_zTab, booktabs = T,escape = F, linesep = "",caption = "Add caption here.",
    align = c("lccc"),format = "latex",digits = 3) %>% 
...
kbl(ENVASA_AgeTab, booktabs = T,escape = F, linesep = "",caption = "Add caption here.",
    align = c("lccc"),format = "latex",digits = 3) %>% 
...
# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf
kbl(ENVASA_tab, booktabs = T,escape = F, linesep = "",caption = "Descriptive of the ENVASA standard residuals (z-scores) split by groups and test measures.",
...
t2 <- ggplot(SumScores, aes(x=GCC,y=SPC,color=Group))+
  ...
  annotate(geom ="text",  x=25, y = -20, label = "Pragmatic/ASD", size=5, face="bold",fontface=2) +
  annotate(geom ="text",  x=110, y = 20, label = "TD", size=5, face="bold",fontface=2) +
