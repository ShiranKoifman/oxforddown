...
# get colour palette:
# See: https://www.datanovia.com/en/blog/top-r-color-palettes-to-know-for-great-data-visualization/#viridis-color-palettes

# View a single RColorBrewer palette by specifying its name
# display.brewer.pal(n = 8, name = 'Dark2')

# my_colors <- RColorBrewer::brewer.pal(3, "Dark2")[c(2,1,3)]

# my_colors_3 <- RColorBrewer::brewer.pal(8, "Dark2")[c(2,3,5)]
# my_colors_2 <- RColorBrewer::brewer.pal(8, "Dark2")[c(2,1)]
# my_colors_2TD <- RColorBrewer::brewer.pal(8, "Dark2")[c(3,5)]

# color.APD = "#FDE725FF"
# color.TD1 = "#31688EFF" #"#3B528BFF"
# color.TD2 = "#21908CFF" 
# color.TDsib = "#8FD744FF" #"#5DC863FF"

# See: http://mkweb.bcgsc.ca/colorblind/img/colorblindness.palettes.v11.pdf

color.APD = "#F60239"   # tractor red
color.APD2 = "#FFAC3B"  # frenzee
color.TD1 = "#005FCC"   # royal blue
color.TD2 = "#009FFA"   # bleu de france
color.TDsib = "#00E5F8" # aqua blue

color.TD1_alpha30 <- adjustcolor(c("#005FCC"), alpha.f = 0.3)

my_colors_3 <- c(color.APD,color.TDsib,color.TD2)
my_colors_2 <- c(color.APD,color.TD1)
my_colors_2TD <- c(color.TD2,color.TDsib)

my_colors_2APD <- c(color.APD,color.APD2)
my_colors_3_expl <- c(color.APD,color.APD2,color.TD1)
...
# library(viridis)
# library(scales)
# show_col(viridis_pal(option="D")(10))
# APD ="#D95F02"
# TD1 = "#3E4A89FF"
# TD2 = "#26828EFF"
# TDsib = "#6DCD59FF"
...
```{r,label='CorrFunction', echo=FALSE, message=FALSE}
...
kbl(BackgroundTab, booktabs = T,escape = F, linesep = "\\addlinespace",caption = "APD group demographics and APD-related history background.", align = c("ll"), format = "latex") %>%
...
Children in the APD group were on average 1.5 years older than children in the TD group. The distribution of age across the two groups is shown in Figure\ \@ref(fig:Age-Hist). The agreement across the two groups between the ages 9 to 11 years was fairly good, whereas a better agreement for younger and older participants would have been desirable. The figure reveals that the age distribution of the APD group was skewed towards older children, with fewer younger APD children (between 7-8 years) than in the TD group. Whereas, the opposite goes for the TD group which was skewed towards older children (between 12-13 years), with fewer older TD children than in the APD group. This was due to difficulties in recruiting younger APD children. APD assessment is typically provided to children from the age of 7 years, however it is a lengthy process often requiring multiple assessments by various professionals prior to being given a referral; APD children therefore tend to be older. Difference in age between the two groups was tested with a one-way ANOVA using *anova()* function (parametric assumption of normal distribution and homoscedasticity were met). The test revealed a significant difference in age between the groups [$F(1,41)$ = 11.58, $p$ \< 0.01]. Nonetheless, since age is often reported as a strong predictor for performance in other similar behavioural studies, analysis of the results obtained in the current study was conducted for age-independent scaled scores and should not affect the comparison between the two groups. The project was approved by the UCL Research Ethics Committee (Project ID Number 0544/006) and the NHS Health Research Authority (REC reference: 18/LO/0250). The testing commenced once an informed consent was given by both the caregiver and the child.\

```{r, label='Age-Hist', fig.cap="Age distribution split by group", fig.align='center', fig.width=6, fig.asp=0.5, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE,fig.pos='h'}
##### Age histogram #####

ggplot(d, aes(x=Age, group=Group, colour=Group,fill=Group)) + 
  geom_histogram(position="dodge",binwidth=1,bins=6) +
  scale_x_continuous(breaks=seq(7,13,1),labels=c("7","8","9","10","11","12","13"))+
  labs(y = "Number of participants",x = "Age (years)") + 
...
Standing waves in the ear canal produces spatially non-uniform sound pressure at frequencies above 2-3 kHz, introducing calibration errors when estimating the sound pressure level arriving at the eardrum [@Siegel1994; @Richmond2011; @Lee2012]. Together with other factors such as individual variations in the ear canal length and differences in depth in which the ear probe is inserted into the ear canal, these factors can introduce up to 20 dB calibration error [@Siegel1994]. To account for that, in-situ forward-pressure-level (FPL) calibration was applied using ARLas MATLAB-based software package [@ARLas]. Thereby improving the accuracy of the threshold estimates, especially at high frequencies [@Lewis2009]. The target stimulus was converted from dB\ SPL to dB\ HL following minimal audible pressure values measured across 84 NH listeners aged 10 to 21 years [see Table 1 in @Lee2012]. Frequency-specific weighting factors were estimated using a logistic function on a log frequency scale with a cut-off of 2\ kHz with a 2 octave wide transition frequency. **A fixed maximum presentation level of 50 dB\ HL was set to ensure that the listeners are not exposed to potentially harmful sound levels, especially at higher frequencies.**\
...
<!-- non-significant interaction terms were subsequently removed to minimise moderation of the significance estimates for main effects !!!  -->
...
```{r, label='PTA2', fig.cap="Standard audiometry: APD participants pure-tone detection thresholds plotted seperately for the left and the right ear (black lines). The shaded grey area represents the TD group thresholds range and the white line represents the TD group mean at each frequency. The dashed line represents the threshold criterion of hearing level $\\leq$ 25\ dB\ HL.", echo=FALSE, fig.align='center', figures-side, out.width='85%',fig.width=12, fig.height=6, eval=FALSE}
...
for (j in 1:2){
  ...
  xx <- c(frqs, rev(frqs))
  yy <- c(lower,rev(upper))
  polygon(xx, yy, col=color.TD1_alpha30,border=NA)
  
  # plot mean control group
  lines(frqs,mn,col=color.TD1,lwd=4)
  
  # plot individual audiograms test group
  for (i in 1:length(APD[,1])){
    lines(frqs,APD[i,numCol],col=color.APD)
  }
  
  # plot mean test group
  lines(frqs,avg,lwd=4, col=color.APD)
  ...
  if (j<=1){
    ...
    # plot legend
    legend(0.24,yplot, c("Mean APD","Individual APD","Mean and range TD","NH criteria"),
           lty=c(1,1,1,2),lwd=c(3,2,10,2),col=c(color.APD,color.APD,color.TD1_alpha30,"black"),cex=0.8,bty=0,box.col="white")

    # Add white line to legend for mean
    segments(x0 = .257, y0 = 42.8, x1 = .322, y1 = 42.8,col = color.TD1, lwd = 3)   
  }
}
...
PTA_tab <-  d_PTA_L %>% ddply(.,~PTA*Group,summarise, N=length(dBHL),median=round(median(dBHL,na.rm=TRUE),2),sd=round(sd(dBHL,na.rm=TRUE),2), min=round(min(dBHL,na.rm=TRUE),2),max=round(max(dBHL,na.rm=TRUE),2))  %>%
  ...
  # mutate(Ear = c("L", "R","","","L", "R","","")) %>% select(.,PTA,Ear,Group,N:max)
  mutate(Ear = c("L", "R","","L", "R","")) %>% select(.,PTA,Ear,Group,N:max)
PTA_tab <- PTA_tab[,-grep("Group",colnames(PTA_tab))]

PTA_tab <- cbind(PTA_tab[1:3,1:ncol(PTA_tab)],PTA_tab[4:nrow(PTA_tab),3:ncol(PTA_tab)])

# PTA_tab$PTA <- c("PTA$_{Left}$","PTA$_{Right}$","PTA","BE")
PTA_tab$PTA <- c("PTA$_{Left}$","PTA$_{Right}$","PTA")
colnames(PTA_tab)[1] = "Frequency"

# --------------------------------------------------------------------------------------------------
# combine table

PTA_tab <- rbind(PTA_tab1,PTA_tab)
colnames(PTA_tab)[1] = ""
```
...
d_L$Group2 <- factor(d_L$Group2, levels = c("APD","TD_1","TD_0"))

t1 <- ggplot(d_L, aes(x=Freq,y=dBHL,fill=Group2))+ 
  geom_hline(yintercept=0, linetype="dashed",color = "black", size=0.5)+
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
  geom_quasirandom(dodge.width=0.8,colour="black", shape=1)+
  ...
  scale_fill_manual(values = my_colors_3, name = "Group", labels = c("APD",bquote(bold(TD[sib])),"TD"))+
  theme_bw()+
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
        axis.title.x = element_text(size=11, face="bold"),
        axis.title.y = element_text(size=11, face="bold"),
...
(t1 + t2) + plot_layout(ncol = 2, nrow = 1, heights = c(1, 1), widths = c(1.4,0.4)) + plot_annotation(tag_levels = 'A') + plot_layout(guides='keep') &
...
d_L$Group2 <- factor(d_L$Group2, levels = c("APD","TD_0","TD_1"))
 
```

```{r, label='RegAud-Tab', echo=FALSE}
...
kbl(PTA_tab, booktabs = T, escape = F, linesep = "", caption = "Standard audiometry: Descriptives for pure-tone detection thresholds (dB\ HL) by frequency (kHz) and ear split by the two groups.",
    align = c("lccccccccccccccc"),format = "latex",digits = 3) %>%
  kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c(" "=2, "APD" = 5, "TD" = 5)) %>%
  column_spec(c(8),border_left = T) %>% 
  # row_spec(6, hline_after =T) %>%
  kable_styling() %>%
  pack_rows("Frequency", 1, 12) %>%
  pack_rows("PTAs", 13, 15) %>%
  add_footnote(c("PTA: average detection threshold at 0.5, 1, 2, & 4 kHz."), notation = "none")
```
...
# sanity check: correlation between frequencies:
# Results: They highly correlate with one another! The closer they are, the more they correlate
HL_corr_R <- corstars(d[,27:32], result="none", method="spearman",p.adj = "none")
HL_corr_L <- corstars(d[,33:38], result="none", method="spearman",p.adj = "none")
```
...
d_PTA_L2 <- d_PTA_L[ ! d_PTA_L$PTA %in% c("PTA_RL"), ] %>% droplevels()
...
model1 <- lmer(dBHL~PTA*Group+(1|listener), data=d_PTA_L2, REML=FALSE)
...
# model2    5 438.95 451.23 -214.48   428.95                       
# model1    6 436.63 451.36 -212.32   424.63 4.3198  1    0.03767 *
...
# -----------------------------------------------------------------------------------------------------
# PTA x Group:
emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ Group | PTA,adjust="bonferroni")
PTAGroup.postLMEM <- summary(emm_s.conf,infer = c(TRUE,TRUE))$contrasts[] %>% 
               data.frame() %>% 
               mutate_if(is.numeric, round, 2) %>%
               mutate(CI = sprintf("%s - %s",
                      round(lower.CL,2),
                      round(upper.CL,2))) %>%
               select(PTA,contrast, estimate:df, t.ratio:CI) %>% 
  dplyr::rename("t-value"=t.ratio, "ear"=PTA)

PTAGroup.postLMEM$ear <- c("PTA$_{Left}$","PTA$_{Right}$")

PTAGroup.postLMEM$p.value <- ifelse(PTAGroup.postLMEM$p.value <.001,"\\textbf{< 0.001}", 
                             ifelse(PTAGroup.postLMEM$p.value <.05, sprintf("\\textbf{%.02f}",PTAGroup.postLMEM$p.value),
                                    PTAGroup.postLMEM$p.value))

PTAGroup.postLMEM_d <- d_PTA_L2 %>% rstatix::group_by(PTA) %>%
...
PTAGroup.postLMEM <- cbind(PTAGroup.postLMEM,PTAGroup.postLMEM_d)

colnames(PTAGroup.postLMEM)[1:7] <- c("Ear","Contrast","Estimate","SE","Df","t-value","p-value",sprintf("95\\%%-CI"),"d","magnitude")
row.names(PTAGroup.postLMEM) <- NULL

#             ear contrast estimate   SE    df t-value        p.value     95\\%-CI    d magnitude
# 1  PTA$_{Left}$ APD - TD     2.68 0.99 67.17     2.7 \\textbf{0.01}   0.7 - 4.67 0.89     large
# 2 PTA$_{Right}$ APD - TD     0.80 0.99 67.17     0.8           0.42 -1.18 - 2.78 0.23     small
# -------------------------------------------------------------------------
...
# linear model
w1 <- lm(dBHL ~ PTA * Group, data = d_PTA_L2)
...
# Option 1: Is the lm model fully covered here?
leveneTest(residuals(w1) ~ PTA * Group, data = d_PTA_L2)
# -> homogeneity of the full model is met
...
d_HF_PTA_L$PTA <- factor(d_HF_PTA_L$PTA,levels=c("PTA_L","PTA_R","PTA_RL"))
...
```{r, label='EHF2', fig.cap="EHF audiometry: Pure-tone detection thresholds for the extended high-frequencies measured in the left and the right ear. The thin black lines represent the individual thresholds in the APD group and the group mean is marked by the bold black line. The shaded grey area represents the TD group threshold range and the white line represents the TD group mean at each frequency.", echo=FALSE, fig.align='center', figures-side, out.width='85%',fig.width=12, fig.height=6, message=FALSE, error=FALSE, eval=FALSE}
...
names(Ear.labs) <- c("L","R")
d_HF_L$Group <- factor(d_HF_L$Group,levels=c("APD","TD"))
d_HF_L$Group2 <- factor(d_HF_L$Group2, levels = c("APD","TD_1","TD_0"))
...
(t1 + t2) + plot_layout(ncol = 2, nrow = 1, heights = c(1, 1), widths = c(1.2,0.6)) + plot_annotation(tag_levels = 'A') + plot_layout(guides='keep') &
  theme(legend.position='bottom',        
        legend.direction = "horizontal",
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"),
...
d_HF_L$Group2 <- factor(d_HF_L$Group2, levels = c("APD","TD_0","TD_1"))
```
...
kbl(EHF_tab, booktabs = T,escape = F, linesep = "", caption = "EHF audiometry: Descriptive for pure-tone detection thresholds (dB HL) by extended-high frequencies (kHz) split by ear and group.",
  ...
  pack_rows("PTAs", 7, 9) %>%
  add_footnote(c("PTA: average detection threshold at 8, 11, & 16 kHz."), notation = "none")
```
...
# sanity check: correlation between frequencies:
# Results: They highly correlate with one another! The closer they are, the more they correlate
EHF_corr_L <- corstars(d_HF[,35:37], result="none", method="spearman",p.adj = "none")
EHF_corr_R <- corstars(d_HF[,44:46], result="none", method="spearman",p.adj = "none")
```
...
Similarly, an additional nonparametric 2 x 2 factorial design model was used to examine the difference between the two groups (APD/TD) for the PTAs measured in the left and the right ear. As before, the TD group was treated as a single group since no significant difference was found between TD children with or without an APD sibling. Parametric model assumption of normal distribution was rejected (Shapiro-Wilk test; $p$ \< 0.05), while the assumption of homoscedasticity was met. The nparLD ATS test found no significant two-way interaction between Group and Ear nor a main effect of Group or Ear (all $p$'s \> 0.05; see Table \@ref(tab:EHF-PTATabnparLD)).\

<!-- DELETE? -->
```{r,label='EHF-PTALMEM', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide', eval=FALSE}

d_HF_PTA_L2 <- d_HF_PTA_L[ ! d_HF_PTA_L$PTA %in% c("PTA_RL"), ] %>% droplevels()
...
model1 <- lmer(dBHL~PTA*Group+(1|listener), data=d_HF_PTA_L2, REML=FALSE)
...
anova(model2,model3)
summary(model3)
# model3: dBHL ~ PTA + (1 | listener)
# model2: dBHL ~ PTA + Group + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model3    4 589.60 599.23 -290.80   581.60                     
# model2    5 591.02 603.05 -290.51   581.02 0.5829  1     0.4452

(model4<-update(model3,  ~ . - PTA))
anova(model3,model4)
summary(model4)
# model4: dBHL ~ (1 | listener)
# model3: dBHL ~ PTA + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model4    3 588.08 595.30 -291.04   582.08                     
# model3    4 589.60 599.23 -290.80   581.60 0.4744  1      0.491
...
# linear model
w1 <- lm(dBHL ~ PTA * Group, data = d_HF_PTA_L2)
...
# linearity is met if p>.05
shapiro.test(residuals(w1))
# W = 0.91463, p-value = 4.353e-05 --> NOT normally distr.

# Option 2: by fixed factors: --> ALL normal..
NormTest <- d_HF_PTA_L2 %>%
  group_by(PTA,Group) %>%
  rstatix::shapiro_test(dBHL)
...
# Option 1: Is the lm model fully covered here?
leveneTest(residuals(w1) ~ PTA * Group, data = d_HF_PTA_L2)
# -> homogeneity of the full model is met

# Levene's Test for Homogeneity of Variance (center = median)
#       Df F value Pr(>F)
# group  3  0.5304 0.6627
#       78      
...
# Normality: Not met (4/4 cases)
# homoscedasticity: Met

EHF_PTA.nparLD <- nparLD(dBHL ~ PTA * Group, data = d_HF_PTA_L2, subject = "listener", description = TRUE, plot.CI=FALSE)
...
EHF_PTA.nparLD <- cbind(" "=c("Group","Ear","Group:Ear"),EHF_PTA.nparLD)


########################### Test assumptions #########################################

# Normality: Not met (4/4)
# homoscedasticity: Met
...
# Option 1: Is the lm model fully covered here?
car::leveneTest(residuals(w1) ~ PTA * Group2, data = d_HF_PTA_L2)
# -> homogeneity of the full model is met

# Levene's Test for Homogeneity of Variance (center = median)
#       Df F value Pr(>F)
# group  5  1.4145 0.2286
#       76       
...
```{r, label='EHF-PTATabnparLD', echo=FALSE}
kbl(EHF_PTA.nparLD,booktabs = T, escape = F, linesep = "",caption = "EHF audiometry: Statistical analysis for the calculated PTAs by Ear (Left/Right) and Group (APD/TD) as well as their interaction (2 x 2 factorial design with repeated measures) tested with a robust rank-based method for analysis of nonparametric data using nparLD package (Noguchi et al., 2012). Analysis was based on a f1-ld-f1 design ANOVA-type statistic (ATS) test, whereby f1 refers to an experimental design with a single between-subjects factor (Group) and a single within-subjects factor (Ear).",
    align = c("lccc"),format = "latex",digits = 3) %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
  column_spec(4, italic = T)
...
```{r,label='ST-FigLevsPC', fig.cap="ST raw data: Frequency of potential outliers with LevsPC\ $\\leq$\ 35\\%. LevsPC denotes the proportion of correct keywords within the final test trials.", fig.width=8, fig.asp=0.8, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
t1 <- ggplot(ASLOutliers, aes(x=LevsPC)) + 
  geom_histogram(binwidth=0.03, color="#e9ecef") +
  scale_x_continuous(limits = c(0.1,0.4),breaks=seq(0.1,0.4,0.1)) +
  scale_y_continuous(limits = c(0,6),breaks=seq(0,6,2)) +
  labs(y = "Frequency",x = "LevsPC", title = "ASL") + 
  # scale_colour_manual(values = my_colors_2)+
  # scale_fill_manual(values = my_colors_2)+
  theme_bw() +
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
...
CCRM$CondCode <-factor(CCRM$CondCode,levels=c("Quiet-NoAlt","Quiet-Alt","AMSSN-Alt",
...
t2 <- ggplot(CCRMOutliers, aes(x=LevsPC)) + 
  geom_histogram(binwidth=0.03, color="#e9ecef") +
  scale_x_continuous(limits = c(0.1,0.4),breaks=seq(0.1,0.4,0.1)) +
  scale_y_continuous(limits = c(0,6),breaks=seq(0,6,2)) +
  labs(y = "Frequency",x = "LevsPC",title = "CCRM") +
  # scale_colour_manual(values = my_colors_2)+
  # scale_fill_manual(values = my_colors_2)+
  theme_bw() +
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
...
(t1 + t2) + plot_layout(ncol = 1, heights = c(1.4, 4.2),widths = c(1,1)) + plot_annotation(tag_levels = 'A') + plot_layout(guides='collect') &
...
TD_trimmed.ASL_s <- TD_trimmed.ASL %>% filter(Group=="TD") %>% dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs,APDsibling) %>% droplevels()
...
# ST.nparLD <- nparLD(uRevs ~ CondCode * material * APDsibling , data = ST_trimmed, subject = "listener", description = TRUE, plot.CI=TRUE)
# 
# 
...
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# model2   38 -420.13 -292.22 248.06  -496.13                     
# model1   42 -412.59 -271.21 248.29  -496.59 0.4587  4     0.9774
...
# model2:     material:Age:APDsibling
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# model3   37 -421.99 -297.45 248.00  -495.99                     
# model2   38 -420.13 -292.22 248.06  -496.13 0.1363  1      0.712
...
# model3:     CondCode:material:APDsibling + CondCode:Age:APDsibling
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# model4   33 -426.57 -315.50 246.29  -492.57                     
# model3   37 -421.99 -297.45 248.00  -495.99 3.4186  4     0.4904
...
# model4:     CondCode:material:APDsibling
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# model5   29 -433.38 -335.77 245.69  -491.38                     
# model4   33 -426.57 -315.50 246.29  -492.57 1.1887  4       0.88
...
# model5:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model6   25 -431.52 -347.37 240.76  -481.52                       
# model5   29 -433.38 -335.77 245.69  -491.38 9.8686  4     0.0427 *

# best model --> model 5: uRevs ~ CondCode + material + Age + APDsibling +  
                         # CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling +  
                         # material:APDsibling + Age:APDsibling + CondCode:material:Age + (1 | listener)
tab_model(model5)
# visreg::visreg(model5)
# visreg::visreg(model5,"APDsibling","CondCode")
# visreg::visreg(model5,"APDsibling","material")
...
# Response: uRevs
#                          Chisq Df Pr(>Chisq)    
# CondCode              824.1201  4  < 2.2e-16 ***
# material              465.1212  1  < 2.2e-16 ***
# Age                    23.7559  1  1.094e-06 ***
# APDsibling              0.8398  1  0.3594439    
# CondCode:material      25.4302  4  4.122e-05 ***
# CondCode:Age           19.5410  4  0.0006151 ***
# material:Age            0.2049  1  0.6507977    
# CondCode:APDsibling     6.1943  4  0.1851018    
# material:APDsibling     5.0993  1  0.0239361 *  
# Age:APDsibling          0.2116  1  0.6455179    
# CondCode:material:Age  10.1043  4  0.0387075 *  
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1


# 1. CondCode:material
(BestModel.1 <- update(BestModel,  . ~ . - CondCode:material))
CondMat.LMEM <- anova(BestModel,BestModel.1)
summary(BestModel.1)
# BestModel.1: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel.1:     CondCode:Age + material:Age + CondCode:APDsibling + material:APDsibling + 
# BestModel.1:     Age:APDsibling + CondCode:material:Age
...
# BestModel:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1   25 -432.58 -348.43 241.29  -482.58                       
# BestModel     29 -433.38 -335.77 245.69  -491.38 8.8036  4     0.0662 .

# 2. CondCode:Age
(BestModel.2 <- update(BestModel,  . ~ . - CondCode:Age))
CondAge.LMEM <- anova(BestModel,BestModel.2)
summary(BestModel.2)
# BestModel: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel:     material:APDsibling + Age:APDsibling + CondCode:material:Age
# BestModel.2: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel.2:     CondCode:material + material:Age + CondCode:APDsibling + 
# BestModel.2:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance Chisq Df Pr(>Chisq)
# BestModel     29 -433.38 -335.77 245.69  -491.38                    
# BestModel.2   29 -433.38 -335.77 245.69  -491.38     0  0  
...
# BestModel.3:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance Chisq Df Pr(>Chisq)
# BestModel     29 -433.38 -335.77 245.69  -491.38                    
# BestModel.3   29 -433.38 -335.77 245.69  -491.38     0  0  

# 4. CondCode:APDsibling
(BestModel.4 <- update(BestModel,  . ~ . -CondCode:APDsibling))
CondSib.LMEM <- anova(BestModel,BestModel.4)
summary(BestModel.4)
# BestModel.4: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel.4:     CondCode:material + CondCode:Age + material:Age + material:APDsibling + 
# BestModel.4:     Age:APDsibling + CondCode:material:Age
# BestModel: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# BestModel.4   25 -435.28 -351.13 242.64  -485.28                     
# BestModel     29 -433.38 -335.77 245.69  -491.38 6.1049  4     0.1914

# 5. material:APDsibling
(BestModel.5 <- update(BestModel,  . ~ . -material:APDsibling))
MatSib.LMEM <- anova(BestModel,BestModel.5)
summary(BestModel.5)
# BestModel.5: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel.5:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel.5:     Age:APDsibling + CondCode:material:Age
# BestModel: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.5   28 -430.36 -336.11 243.18  -486.36                       
# BestModel     29 -433.38 -335.77 245.69  -491.38 5.0272  1    0.02495 *

# 6. Age:APDsibling
(BestModel.6 <- update(BestModel,  . ~ . -Age:APDsibling))
AgeSib.LMEM <- anova(BestModel,BestModel.6)
summary(BestModel.6)
# BestModel.6:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel.6:     material:APDsibling + CondCode:material:Age
# BestModel: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# BestModel.6   28 -435.17 -340.93 245.59  -491.17                     
# BestModel     29 -433.38 -335.77 245.69  -491.38 0.2111  1     0.6459

# 7. CondCode:material:Age
(BestModel.7 <- update(BestModel,  . ~ . -CondCode:material:Age))
CondMatAge.LMEM <- anova(BestModel,BestModel.7)
summary(BestModel.7)
# BestModel.1: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel.1:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel.1:     material:APDsibling + Age:APDsibling
# BestModel: uRevs ~ CondCode + material + Age + APDsibling + (1 | listener) + 
# BestModel:     CondCode:material + CondCode:Age + material:Age + CondCode:APDsibling + 
# BestModel:     material:APDsibling + Age:APDsibling + CondCode:material:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1   25 -431.52 -347.37 240.76  -481.52                       
# BestModel     29 -433.38 -335.77 245.69  -491.38 9.8686  4     0.0427 *
...
# emmeans::emtrends(BestModel,pairwise~CondCode*material,var="Age")
# Plot
# p <- emmeans::emmip(BestModel, CondCode ~ material | Age,at = list(Age = unique(ST_trimmed$Age))) 
# 
...
# emmeans::emmip(BestModel, CondCode ~ material | Age,at = list(Age = c(7:12)),engine = "lattice") #at = list(Age = unique(ST_trimmed$Age))
# 
# emmeans::emmip(BestModel, material ~ CondCode | Age ,at = list(Age = c(7:12)),engine = "lattice")

ST_means <- summary(emmeans::emmeans(BestModel, ~ CondCode * material | Age,adjust="bonferroni",at = list(Age = c(7:12)))) %>% as.data.frame(.)

emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~  CondCode | material*Age, adjust="bonferroni",at = list(Age = c(7:12)))
postLMEM.3 <- summary(emm_s.conf,infer = c(TRUE,TRUE))$contrasts[] %>%
               data.frame() %>%
               mutate_if(is.numeric, round, 2)
...
ST_trimmed_w <- ST_trimmed %>%
  pivot_wider(
    id_cols = c("listener","Age","Group","Group2","material","CondCode","uRevs"),
    names_from = c("material","CondCode"),
    values_from = c("uRevs")) %>%
  ungroup() %>% 
  as.data.frame(.) %>% arrange(Age)

colnames(ST_trimmed_w)[1:length(ST_trimmed_w)] <- gsub("-", "_", colnames(ST_trimmed_w[,c(1:length(ST_trimmed_w))]))


t1 <- ggplot(ST_trimmed_w, aes(x=CCRM_MDR_F_Alt, y=CCRM_ENG_F_Alt, colour=Age)) +
  geom_abline(intercept = 0, slope = 1) +
  geom_point(size=3, aes(colour=sort(Age,decreasing = FALSE))) +
  labs(x = "MDR_F", y = "ENG_F",title = "CCRM") +
  scale_colour_continuous(trans = 'reverse')+
  scale_y_continuous(limits = c(0,1.01),breaks=seq(0,1,0.1)) +
...
t2 <- ggplot(ST_trimmed_w, aes(x=ASLN_MDR_F_Alt, y=ASLN_ENG_F_Alt, colour=Age)) +
  geom_abline(intercept = 0, slope = 1) +
  geom_point(size=3, aes(colour=sort(Age,decreasing = FALSE))) +
  labs(x = "MDR_F", y = "ENG_F",title = "ASL") +
  scale_colour_continuous(trans = 'reverse')+
  scale_y_continuous(limits = c(0,1.01),breaks=seq(0,1,0.1)) +
  scale_x_continuous(limits = c(0,1.01),breaks=seq(0,1,0.1)) +
  # guides(colour = guide_legend(reverse = FALSE)) +
...
# emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ CondCode * material | Age,adjust="bonferroni",at = list(Age = c(7:12)))
# # emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ CondCode | material*Age ,adjust="bonferroni",at = list(Age = unique(ST_trimmed$Age)))
# # emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ CondCode | material*Age ,adjust="bonferroni")
# postLMEM.3 <- summary(emm_s.conf,infer = c(TRUE,TRUE))$contrasts[] %>%
#                data.frame() %>%
#                mutate_if(is.numeric, round, 2) %>%
#                mutate(CI = sprintf("%s - %s",
#                       round(lower.CL,2),
#                       round(upper.CL,2))) %>%
#                select(contrast,material, estimate:df, t.ratio:CI)
# 
# postLMEM.3$p.value <- ifelse(postLMEM.3$p.value <.001,"\\textbf{< 0.001}",
#                              ifelse(postLMEM.3$p.value <.05, sprintf("\\textbf{%.02f}",postLMEM.3$p.value),
#                                     postLMEM.3$p.value))
# # change some names
# # postLMEM.3$material <-   str_replace(postLMEM.3$material,"ASLN","ASL")
# postLMEM.3$contrast <-   paste0(str_replace_all(postLMEM.3$contrast, fixed("_"), "\\_"))
# postLMEM.3$contrast <-   paste0(str_replace_all(postLMEM.3$contrast, fixed("("), ""))
# postLMEM.3$contrast <-   paste0(str_replace_all(postLMEM.3$contrast, fixed(")"), ""))
# 
# postLMEM.3$material <-   paste0(str_replace_all(postLMEM.3$material, fixed("ASLN"), "ASL"))
# 
# colnames(postLMEM.3)[8] = sprintf("95\\%%-CI")


# Material X Sibling:
# (ref1 <- emmeans::lsmeans(BestModel,~ material*sibling))
...
emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ APDsibling | material,adjust="bonferroni")
postLMEM.1 <- summary(emm_s.conf,infer = c(TRUE,TRUE))$contrasts[] %>% 
               data.frame() %>% 
               mutate_if(is.numeric, round, 2) %>%
               mutate(CI = sprintf("%s - %s",
                      round(lower.CL,2),
...
colnames(postLMEM.1)[8] = sprintf("95\\%%-CI")
postLMEM.1$contrast <- c("TD - TD$_{sib}$","TD - TD$_{sib}$")
postLMEM.1$material <- str_replace(postLMEM.1$material,"ASLN","ASL")

postLMEM.1_d <- ST_trimmed %>% rstatix::group_by(material) %>%
    rstatix::cohens_d(uRevs ~ APDsibling, data=., ci = TRUE) %>% as.data.frame() %>%
    select(effsize,magnitude) %>%
    dplyr::rename("d"=effsize)

postLMEM.1 <- cbind(postLMEM.1,postLMEM.1_d)

# remove attributes as it causes issues with kbl:
postLMEM.1$d <- labelled::remove_attributes(postLMEM.1$d, "Cohen's d")
postLMEM.1$magnitude <- as.character(postLMEM.1$magnitude)
...
# postLMEM.2_d <- ST_trimmed %>% rstatix::group_by(material) %>%
#     rstatix::cohens_d(uRevs ~ CondCode, data=., ci = TRUE) %>% as.data.frame() %>%
#     select(effsize,magnitude) %>%
#     dplyr::rename("d"=effsize)
...
ST_AgeTab <- data.frame(rbind(
  ...
  c("Age:Sibling",AgeSib.LMEM$Df[2],AgeSib.LMEM$Chisq[2],sprintf("%0.3f",AgeSib.LMEM$`Pr(>Chisq)`[2])),
  c("Condition:Material:Age",CondMatAge.LMEM$Df[2],CondMatAge.LMEM$Chisq[2],sprintf("\\textbf{%0.3f}",CondMatAge.LMEM$`Pr(>Chisq)`[2]))))

ST_AgeTab[,c(2:3)]= apply(ST_AgeTab[,c(2:3)], 2, function(x) as.numeric(as.character(x)))

colnames(ST_AgeTab)[1:4] <- c("Effects","Df",sprintf("$\\chi^{2}$"),"p")
# -------------------------------------------------------------------------
...
# ST_trimmed$Model.F.Res<- residuals(BestModel) #extracts the residuals and places them in a new column in our original data table
# ST_trimmed$Abs.Model.F.Res <-abs(ST_trimmed$Model.F.Res) #creates a new column with the absolute value of the residuals
# ST_trimmed$Model.F.Res2 <- ST_trimmed$Abs.Model.F.Res^2 #squares the absolute values of the residuals to provide the more robust estimate
# Levene.Model.F <- lm(Model.F.Res2 ~ listener, data=ST_trimmed) #ANOVA of the squared residuals
...
kbl(ST_AgeTab, booktabs = T,escape = F, linesep = "",caption = "ST: Age effect analysis using LMEM for SRdTs measured across condition, speech material, age and children from the TD group with/without an APD sibling (Sibling: TD/TD$_{sib}$) as fixed factors and random intercepts for subjects. Reference levels: Condition = Quiet-NoAlt, Material = ASL, Sibling = TD. Note: only data for the control group (TD) following outlier trimming was included.",
    align = c("lccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c("Material:Sibling + Age:Sibling + Condition:Material:Age + (1 | Subjects)" = 4),align = "l") %>%
  add_header_above(c("Condition:Material + Condition:Age + Material:Age + Condition:Sibling +" = 4),align = "l",line = FALSE) %>%
  add_header_above(c("SRdT ~ Condition + Material + Age + Sibling +" = 4), align = "l",line = FALSE) %>%
...
The significant three-way Condition x\ Material Age interaction supports the observation in Figure \@ref(fig:ST-Age) A-B, that the effect of age depends on material type and test condition. Model based estimated marginal means (EMMs) of the SRdTs (in DC) across the test conditions were computed per year from 7 to 12 for the two speech materials and are shown in Figure\ \@ref(fig:ST-EMMplot). The figure reveals several interesting trends. Firstly, we can see that performance is strongly depended on material type, with better performance (i.e. smaller DC) for the CCRM than for the ASL material. Second, there is a significant developmental trend across test conditions, where SRdTs decreased (i.e., improved) by age. However, the effect-size of age is not the same across test conditions and material type, with a decrement in SRdTs ranging between circa 0.02 to 0.3. Looking at the figure, we can see that the developmental effect was similar across the two material types for the AMSSN and ENG_F distractors, while it differed to a varying degree across the rest of the test conditions. Speech distractors exhibited the largest decrement (i.e., improvement) across the age span, varying between 0.18 to 0.30 DC decrement across the age span. Decrement in DC over age for nonspeech distractor and quiet conditions on the other hand varied between 0.02 to 0.16. Overall however, the combined age effect appears to be fairly similar in the two material types, with an average improvement of 0.024 and 0.022 DC per 1 year for the ASL and CCRM material respectively. As for the relationship between test conditions and material type, the figure suggests that there is a similar trend in both speech materials (albeit shifted), but with a larger effect size in some conditions than others.\

```{r, label='ST-EMMplot', fig.cap="ST: Condition x Material x Age interaction: model based estimated marginal means, EMMs (in DC). EMMs were computed using the emmeans function (emmeans package).", fig.align='center', fig.width=7, fig.asp=.8, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# ddply(ST_means,~CondCode*material,summarise,median=median(emmean,na.rm=TRUE),mean=mean(emmean,na.rm=TRUE),sd=sd(emmean,na.rm=TRUE), min=min(emmean,na.rm=TRUE),max=max(emmean,na.rm=TRUE)) %>%
#   mutate(Diff=max-min) %>%
#   mutate_if(is.numeric, round, 2) %>%
#   arrange(., group_by = material)
# 
#     CondCode material median mean   sd  min  max Diff
# 1    Q-NoAlt     ASLN   0.35 0.35 0.04 0.29 0.40 0.10
# 2      Q-Alt     ASLN   0.44 0.44 0.04 0.39 0.49 0.10
# 3  AMSSN-Alt     ASLN   0.42 0.42 0.02 0.38 0.45 0.06
# 4  MDR_F-Alt     ASLN   0.69 0.69 0.11 0.54 0.83 0.29
# 5  ENG_F-Alt     ASLN   0.77 0.77 0.07 0.68 0.86 0.18
# 6    Q-NoAlt     CCRM   0.18 0.18 0.01 0.17 0.19 0.02
# 7      Q-Alt     CCRM   0.27 0.27 0.06 0.18 0.35 0.16
# 8  AMSSN-Alt     CCRM   0.26 0.26 0.02 0.23 0.29 0.06
# 9  MDR_F-Alt     CCRM   0.46 0.46 0.05 0.40 0.52 0.12
# 10 ENG_F-Alt     CCRM   0.48 0.48 0.11 0.33 0.63 0.30

# CondCodexMaterialxAge plot
ggplot(ST_means, aes(x=CondCode, y=emmean, shape = material, group=interaction(material,as.factor(Age)))) +
  ...
  scale_x_discrete(labels=c("Q-NoAlt" ="Quiet-NoAlt","Q-Alt"="Quiet-Alt",
                            "AMSSN-Alt"="AMSSN-Alt","MDR_F-Alt"="MDR_F-Alt","ENG_F-Alt"="ENG_F-Alt"))+
  scale_shape_discrete(labels = c("ASL","CCRM"), name = "Material")+
  theme_bw() +
...
Lastly, the significant interaction between Material and Sibling was examined with a model-based post-hoc t-tests comparison (see Table\ \@ref(tab:ST-AgeLMEMpost1)). There was no significant difference in SRdTs between the two TD groups for both speech materials (both $p$'s \> 0.05). The results suggests that the differences between the two groups depends upon the material type. This is not surprising considering the large differences between the two material types, where the CCRM sentences are expected to be more intelligible.\

```{block type='correction'}
**Discussion???**
\
Differences in means between the two groups within speech material were generally very small. Interestingly, however the model based estimated marginal means for performance of the TD$_{sib}$ children was on average 0.008 better than their TD peers for the ASL material and 0.04 poorer for the CCRM material. These results are in contradiction to our expectations if any differences arose. In such a case, we would have predicted a larger decrement in performance (i.e., poorer score) for the ASL sentences which are more linguistically challenging than for the CCRM sentences. Nonetheless, the very small estimated mean differences and the negligible effect-sizes suggests that the differences picked up by the model are due to sampling error.\
```

(ref:tabCap2) ST: Age-effect: post-hoc paired-comparison t-tests for Material (ASL/CCRM) x Sibling (TD/TD$_{sib}$) interaction. The test was performed on the fitted LMEM model and included adjusted least-squared-mean for the random intercepts (subjects) using the emmeans package [@emmeansPackageR].

```{r, label='ST-AgeLMEMpost1', echo=FALSE}
# ST_trimmed %>% ddply(.,~Group2*material,summarise,median=median(uRevs,na.rm=TRUE),mean=mean(uRevs,na.rm=TRUE),sd=sd(uRevs,na.rm=TRUE), min=min(uRevs,na.rm=TRUE),max=max(uRevs,na.rm=TRUE)) %>%
#   mutate_if(is.numeric, round, 2) %>%
#   arrange(., group_by = material)
# 
# ST_trimmed %>% filter(CondCode==c("MDR_F-Alt","ENG_F-Alt")) %>% ddply(.,~Group2*material,summarise,median=median(uRevs,na.rm=TRUE),mean=mean(uRevs,na.rm=TRUE),sd=sd(uRevs,na.rm=TRUE), min=min(uRevs,na.rm=TRUE),max=max(uRevs,na.rm=TRUE)) %>%
#   mutate_if(is.numeric, round, 2) %>%
#   arrange(., group_by = material)
...
```{r,label='ST-ASL-SibsRegLines',fig.align='center', fig.width=6, fig.asp=1.5, out.width='100%', message=FALSE,warning=FALSE, echo=FALSE, results='hide'}
...
ASLN_TD0 <- ASLN %>% filter(Group2!="TD_1") %>% droplevels()

### Q-NoAlt ###
df <- ASLN_TD0 %>% filter(CondCode=="Q-ASLN-NoAlt" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(ASLN_TD0,"Q-ASLN-NoAlt",CutOff,slope1_temp,int1_temp,8,05)
df_1 <- Output[[1]]  # df_All
TD_trimmed_1 = Output[[11]]
FinalM_1 <- if(Output[[8]]==1){FinalM_1=Output[[6]]}else{FinalM_1=Output[[7]]}
L1<- sprintf("slope = %s, intcpt = %s",
...
### Q-Alt ###
df <- ASLN_TD0 %>% filter(CondCode=="Q-ASLN-Alt" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(ASLN_TD0,"Q-ASLN-Alt",CutOff,slope1_temp,int1_temp,10)
df_2 <- Output[[1]]  # df_All
TD_trimmed_2 = Output[[11]]
FinalM_2 <- if(Output[[8]]==1){FinalM_2=Output[[6]]}else{FinalM_2=Output[[7]]}
L2<- sprintf("slope = %s, intcpt = %s",
...
### AMSSN-Alt ###
Output <- getBestFit(ASLN_TD0,"AMSSN-ASLN-Alt",CutOff)
df_3 <- Output[[1]]  # df_All
TD_trimmed_3 = Output[[11]]
FinalM_3<- if(Output[[8]]==1){FinalM_3=Output[[6]]}else{FinalM_3=Output[[7]]}
L3<- sprintf("slope = %s, intcpt = %s",
...
### MDR-Alt ###
Output <- getBestFit(ASLN_TD0,"MDR_F-ASLN-Alt",CutOff)
df_4 <- Output[[1]]  # df_All
TD_trimmed_4 = Output[[11]]
FinalM_4 <- if(Output[[8]]==1){FinalM_4=Output[[6]]}else{FinalM_4=Output[[7]]}
MDR.formula <- FinalM_4$m$formula()
...
### ENG-Alt ###
Output <- getBestFit(ASLN_TD0,"ENG_F-ASLN-Alt",CutOff)
df_5 <- Output[[1]]  # df_All
TD_trimmed_5 = Output[[11]]
FinalM_5 <- if(Output[[8]]==1){FinalM_5=Output[[6]]}else{FinalM_5=Output[[7]]}
L5<- cbind("slope"=FinalM_5$m$getAllPars()[1],"intcpt"=FinalM_5$m$getAllPars()[2])
...
## combine all conditions together
FinalM.ASL_TD0 <- list(FinalM_1,FinalM_2,FinalM_3,FinalM_4,FinalM_5)
ASLN_TD0 <- rbind(df_1,df_2,df_3,df_4,df_5)
Lines.ASL_TD0 <- rbind(L1,L2,L3,L4,L5)
LStats.ASL_TD0 <- rbind(Lstats_1,Lstats_2,Lstats_3,Lstats_4,Lstats_5)
TD_trimmed.ASL_TD0 <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3,TD_trimmed_4,TD_trimmed_5)
...
ASLN_TD1 <- ASLN %>% filter(Group2!="TD_0") %>% droplevels()

### Q-NoAlt ###
df <- ASLN_TD1 %>% filter(CondCode=="Q-ASLN-NoAlt" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(ASLN_TD1,"Q-ASLN-NoAlt",CutOff,slope1_temp,int1_temp,8,05)
df_1 <- Output[[1]]  # df_All
TD_trimmed_1 = Output[[11]]
FinalM_1 <- if(Output[[8]]==1){FinalM_1=Output[[6]]}else{FinalM_1=Output[[7]]}
L1<- sprintf("slope = %s, intcpt = %s",
...
### Q-Alt ###
df <- ASLN_TD1 %>% filter(CondCode=="Q-ASLN-Alt" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(ASLN_TD1,"Q-ASLN-Alt",CutOff,slope1_temp,int1_temp,10)
df_2 <- Output[[1]]  # df_All
TD_trimmed_2 = Output[[11]]
FinalM_2 <- if(Output[[8]]==1){FinalM_2=Output[[6]]}else{FinalM_2=Output[[7]]}
L2<- sprintf("slope = %s, intcpt = %s",
...
### AMSSN-Alt ###
Output <- getBestFit(ASLN_TD1,"AMSSN-ASLN-Alt",CutOff)
df_3 <- Output[[1]]  # df_All
TD_trimmed_3 = Output[[11]]
FinalM_3<- if(Output[[8]]==1){FinalM_3=Output[[6]]}else{FinalM_3=Output[[7]]}
L3<- sprintf("slope = %s, intcpt = %s",
...
### MDR-Alt ###
Output <- getBestFit(ASLN_TD1,"MDR_F-ASLN-Alt",CutOff)
df_4 <- Output[[1]]  # df_All
TD_trimmed_4 = Output[[11]]
FinalM_4 <- if(Output[[8]]==1){FinalM_4=Output[[6]]}else{FinalM_4=Output[[7]]}
MDR.formula <- FinalM_4$m$formula()
...
### ENG-Alt ###
Output <- getBestFit(ASLN_TD1,"ENG_F-ASLN-Alt",CutOff)
df_5 <- Output[[1]]  # df_All
TD_trimmed_5 = Output[[11]]
FinalM_5 <- if(Output[[8]]==1){FinalM_5=Output[[6]]}else{FinalM_5=Output[[7]]}
L5<- cbind("slope"=FinalM_5$m$getAllPars()[1],"intcpt"=FinalM_5$m$getAllPars()[2])
...
## combine all conditions together
FinalM.ASL_TD1 <- list(FinalM_1,FinalM_2,FinalM_3,FinalM_4,FinalM_5)
ASLN_TD1 <- rbind(df_1,df_2,df_3,df_4,df_5)
Lines.ASL_TD1 <- rbind(L1,L2,L3,L4,L5)
LStats.ASL_TD1 <- rbind(Lstats_1,Lstats_2,Lstats_3,Lstats_4,Lstats_5)
TD_trimmed.ASL_TD1 <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3,TD_trimmed_4,TD_trimmed_5)
```

```{r,label='ST-CCRM-SibsRegLines',fig.align='center', fig.width=6, fig.asp=1.5, out.width='100%', message=FALSE,warning=FALSE, echo=FALSE, results='hide'}
...
CCRM_TD0 <- CCRM %>% filter(Group2!="TD_1") %>% droplevels()

### Q-NoAlt ###
Output <- getBestFit(CCRM_TD0,"Q-CCRM-NoAlt",CutOff)
df_1 <- Output[[1]]  # df_All
TD_trimmed_1 = Output[[11]]
FinalM_1 <- if(Output[[8]]==1){FinalM_1=Output[[6]]}else{FinalM_1=Output[[7]]}
L1<- sprintf("slope = %s, intcpt = %s",
...
### Q-Alt ###
df <- CCRM_TD0 %>% filter(CondCode=="Q-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(CCRM_TD0,"Q-CCRM-Alt",CutOff,-slope1_temp,int1_temp,8)
df_2 <- Output[[1]]  # df_All
TD_trimmed_2 = Output[[11]]
FinalM_2 <- if(Output[[8]]==1){FinalM_2=Output[[6]]}else{FinalM_2=Output[[7]]}
L2<- sprintf("slope = %s, intcpt = %s",
...
### AMSSN ###
df <- CCRM_TD0 %>% filter(CondCode=="AMSSN-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(CCRM_TD0,"AMSSN-CCRM-Alt",CutOff,-slope1_temp,int1_temp,8.5)
df_3 <- Output[[1]]  # df_All
TD_trimmed_3 = Output[[11]]
FinalM_3 <- if(Output[[8]]==1){FinalM_3=Output[[6]]}else{FinalM_3=Output[[7]]}
L3<- sprintf("slope = %s, intcpt = %s",
...
### MDR_F ###
df <- CCRM_TD0 %>% filter(CondCode=="MDR_F-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

# Output <- getBestFit(CCRM,"MDR_F-CCRM-Alt",CutOff,slope1_temp,int1_temp,11.5,0.078)
Output <- getBestFit(CCRM_TD0,"MDR_F-CCRM-Alt",CutOff,slope1_temp,int1_temp,10,0.5)
df_4 <- Output[[1]]  # df_All
TD_trimmed_4 = Output[[11]]
FinalM_4 <- if(Output[[8]]==1){FinalM_4=Output[[6]]}else{FinalM_4=Output[[7]]}
L4<- sprintf("slope = %s, intcpt = %s",
...
### ENG_F ###
df <- CCRM_TD0 %>% filter(CondCode=="ENG_F-CCRM-Alt" & Group=="TD" & TempRmv<1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)


Output <- getBestFit(CCRM_TD0,"ENG_F-CCRM-Alt",CutOff,slope1_temp,int1_temp,8,0.5)
df_5 <- Output[[1]]  # df_All
TD_trimmed_5 = Output[[11]]
FinalM_5 <- if(Output[[8]]==1){FinalM_5=Output[[6]]}else{FinalM_5=Output[[7]]}
L5<- sprintf("slope = %s, intcpt = %s",
...
### CCRM_F ###
df <- CCRM_TD0 %>% filter(CondCode=="CCRM_F-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(CCRM_TD0,"CCRM_F-CCRM-Alt",CutOff,-0.1173,1.7272,8.8838)
df_6 <- Output[[1]]  # df_All
TD_trimmed_6 = Output[[11]]
FinalM_6 <- if(Output[[8]]==1){FinalM_6=Output[[6]]}else{FinalM_6=Output[[7]]}
L6<- sprintf("slope = %s, intcpt = %s",
...
## combine all conditions together
FinalM.CCRM_TD0 <- list(FinalM_1,FinalM_2,FinalM_3,FinalM_4,FinalM_5,FinalM_6)
CCRM_TD0 <- rbind(df_1,df_2,df_3,df_4,df_5,df_6)
Lines.CCRM_TD0 <- rbind(L1,L2,L3,L4,L5,L6)
LStats.CCRM_TD0 <- rbind(Lstats_1,Lstats_2,Lstats_3,Lstats_4,Lstats_5,Lstats_6)
TD_trimmed.CCRM_TD0 <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3,TD_trimmed_4,TD_trimmed_5,TD_trimmed_6)

########################################################################################
# TD_1
########################################################################################

CCRM_TD1 <- CCRM %>% filter(Group2!="TD_0") %>% droplevels()

### Q-NoAlt ###
Output <- getBestFit(CCRM_TD1,"Q-CCRM-NoAlt",CutOff)
df_1 <- Output[[1]]  # df_All
TD_trimmed_1 = Output[[11]]
FinalM_1 <- if(Output[[8]]==1){FinalM_1=Output[[6]]}else{FinalM_1=Output[[7]]}
L1<- sprintf("slope = %s, intcpt = %s",
...
### Q-Alt ###
df <- CCRM_TD1 %>% filter(CondCode=="Q-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(CCRM_TD1,"Q-CCRM-Alt",CutOff,-slope1_temp,int1_temp,8)
df_2 <- Output[[1]]  # df_All
TD_trimmed_2 = Output[[11]]
FinalM_2 <- if(Output[[8]]==1){FinalM_2=Output[[6]]}else{FinalM_2=Output[[7]]}
L2<- sprintf("slope = %s, intcpt = %s",
...
### AMSSN ###
df <- CCRM_TD1 %>% filter(CondCode=="AMSSN-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(CCRM_TD1,"AMSSN-CCRM-Alt",CutOff,-slope1_temp,int1_temp,8.5)
df_3 <- Output[[1]]  # df_All
TD_trimmed_3 = Output[[11]]
FinalM_3 <- if(Output[[8]]==1){FinalM_3=Output[[6]]}else{FinalM_3=Output[[7]]}
L3<- sprintf("slope = %s, intcpt = %s",
...
### MDR_F ###
df <- CCRM_TD1 %>% filter(CondCode=="MDR_F-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

# Output <- getBestFit(CCRM,"MDR_F-CCRM-Alt",CutOff,slope1_temp,int1_temp,11.5,0.078)
Output <- getBestFit(CCRM_TD1,"MDR_F-CCRM-Alt",CutOff,slope1_temp,int1_temp,10,0.5)
df_4 <- Output[[1]]  # df_All
TD_trimmed_4 = Output[[11]]
FinalM_4 <- if(Output[[8]]==1){FinalM_4=Output[[6]]}else{FinalM_4=Output[[7]]}
L4<- sprintf("slope = %s, intcpt = %s",
...
### ENG_F ###
df <- CCRM_TD1 %>% filter(CondCode=="ENG_F-CCRM-Alt" & Group=="TD" & TempRmv<1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)


Output <- getBestFit(CCRM_TD1,"ENG_F-CCRM-Alt",CutOff,slope1_temp,int1_temp,8,0.5)
df_5 <- Output[[1]]  # df_All
TD_trimmed_5 = Output[[11]]
FinalM_5 <- if(Output[[8]]==1){FinalM_5=Output[[6]]}else{FinalM_5=Output[[7]]}
L5<- sprintf("slope = %s, intcpt = %s",
...
### CCRM_F ###
df <- CCRM_TD1 %>% filter(CondCode=="CCRM_F-CCRM-Alt" & Group=="TD") %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)

Output <- getBestFit(CCRM_TD1,"CCRM_F-CCRM-Alt",CutOff,-0.1173,1.7272,8.8838)
df_6 <- Output[[1]]  # df_All
TD_trimmed_6 = Output[[11]]
FinalM_6 <- if(Output[[8]]==1){FinalM_6=Output[[6]]}else{FinalM_6=Output[[7]]}
L6<- sprintf("slope = %s, intcpt = %s",
...
## combine all conditions together
FinalM.CCRM_TD1 <- list(FinalM_1,FinalM_2,FinalM_3,FinalM_4,FinalM_5,FinalM_6)
CCRM_TD1 <- rbind(df_1,df_2,df_3,df_4,df_5,df_6)
Lines.CCRM_TD1 <- rbind(L1,L2,L3,L4,L5,L6)
LStats.CCRM_TD1 <- rbind(Lstats_1,Lstats_2,Lstats_3,Lstats_4,Lstats_5,Lstats_6)
TD_trimmed.CCRM_TD1 <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3,TD_trimmed_4,TD_trimmed_5,TD_trimmed_6)
```

```{r, label='ST-AgeTD', fig.cap="ST: sibling and age effect: Scatterplot and linear regression lines for the TD listeners SRdTs measured with the ASL (A) and the CCRM speech material (B) as a function of age.", fig.align='center', fig.width=14, fig.asp=0.8, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE,fig.pos='h'}

dat_text.ASL <- data.frame(
  label_TD0 = Lines.ASL_TD0,
  label2_TD0 = matrix(LStats.ASL_TD0),
  label_TD1 = Lines.ASL_TD1,
  label2_TD1 = matrix(LStats.ASL_TD1),
  CondCode   = levels(ASLN$CondCode),
  Group = "TD",
  Group2 = "TD_0")

dat_text.CCRM <- data.frame(
  label_TD0 = Lines.CCRM_TD0,
  label2_TD0 = matrix(LStats.CCRM_TD0),
  label_TD1 = Lines.CCRM_TD1,
  label2_TD1 = matrix(LStats.CCRM_TD1),
  CondCode   = levels(CCRM$CondCode),
  Group = "TD",
  Group2 = "TD_0")
...
TwoLines <- function(Age){
  TwoLinesFCT(Age,
              as.numeric(coefficients(FinalM.ASL_TD0[[4]])[1]), 
              0, 
              as.numeric(coefficients(FinalM.ASL_TD0[[4]])[2]), 
              as.numeric(coefficients(FinalM.ASL_TD0[[4]])[3]))
}

t1 <- ggplot(ASLN, aes(x=Age, y=uRevs, colour=Group2)) +
  geom_hline(yintercept=0.97, linetype="dashed",colour = "black", size=0.5)+
  geom_hline(yintercept=0.05, linetype="dashed",colour = "black", size=0.5)+
  geom_point(data=filter(ASLN,Group=="TD"),size=2,alpha=1,aes(colour=Group2)) +
  # geom_text(label=ASLN$listener, size=3)+
  # reg lines for TD_0
  geom_line(data=filter(ASLN_TD0, Group2=="TD_0",CondCode!="MDR_F-ASLN-Alt"),aes(x = Age, y = predicted), size=1) +
  # a separate segmented line for MDR_F (TD_0)
  stat_function(data=filter(ASLN_TD0, Group2=="TD_0" & CondCode=="MDR_F-ASLN-Alt"),
                fun = TwoLines, xlim= c(7.0,12.1), size=1)+ 
  # reg lines for TD_1:
  geom_line(data=filter(ASLN_TD1, Group2=="TD_1"),aes(x = Age, y = predicted), size=1) +
  # geom_smooth(data=filter(ASLN,Group2=="TD_1"),method="lm" , aes(group=Group2), se=FALSE) +
  # reg stats TD_0:
  # shadowtext::geom_shadowtext(data = dat_text.ASL, mapping = aes(x = -Inf, y = Inf, label = label_TD0),
  #           hjust = -0.05, vjust = 2,size = 3.5,show.legend = FALSE, bg.colour='black')+
  geom_text(data = dat_text.ASL, mapping = aes(x = -Inf, y = Inf, label = label_TD0),
            hjust = -0.05, vjust = 2,size = 3.5,show.legend = FALSE)+
  # shadowtext::geom_shadowtext(data = dat_text.ASL, mapping = aes(x = -Inf, y = Inf, label = label2_TD0),
  #           hjust = -0.06, vjust = 3.5, size = 3.5,show.legend = FALSE, bg.colour='black')+
  geom_text(data = dat_text.ASL, mapping = aes(x = -Inf, y = Inf, label = label2_TD0),
            hjust = -0.06, vjust = 3.5, size = 3.5,show.legend = FALSE)+
  # reg stats TD_1:
  # shadowtext::geom_shadowtext(data = dat_text.ASL, mapping = aes(x = -Inf, y = -Inf, label = label_TD1),
  #           hjust = -0.05, vjust = -3,size = 3.5, colour = color.TDsib, show.legend = FALSE ,bg.r=0.05)+
  geom_text(data = dat_text.ASL, mapping = aes(x = -Inf, y = -Inf, label = label_TD1),
            hjust = -0.05, vjust = -3,size = 3.5, colour = color.TDsib, show.legend = FALSE)+
  # shadowtext::geom_shadowtext(data = dat_text.ASL, mapping = aes(x = -Inf, y = -Inf, label = label2_TD1),
  #           hjust = -0.06, vjust = -1, size = 3.5, colour = color.TDsib, show.legend = FALSE ,bg.r=0.05)+
  geom_text(data = dat_text.ASL, mapping = aes(x = -Inf, y = -Inf, label = label2_TD1),
            hjust = -0.06, vjust = -1, size = 3.5, colour = color.TDsib, show.legend = FALSE)+
  geom_boxplot(ASLN_adults,mapping = aes(x = 14 , y = uRevs),colour="black",show.legend = FALSE) +
  labs(y = "SRdTs (proportion of duty cycle)",x = "Age (years)") + 
  scale_y_continuous(limits = c(-0.05,1.05),breaks=seq(0,1,0.1)) +
  scale_x_continuous(breaks=seq(7,14,1),labels=c("7","8","9","10","11","12","13","18-35"))+
  scale_colour_manual(values = my_colors_2TD, name = "Group", labels = c("TD",bquote(bold(TD[sib]))))+
  guides(colour=FALSE) +
  theme_bw() +
  theme(axis.text = element_text(size = 10, face="bold",colour = "black"),
        axis.title.x = element_text(size=11, face="bold"),
...
###### CCRM ##############
t2 <- ggplot(CCRM, aes(x=Age, y=uRevs, colour=Group2)) +
  geom_hline(yintercept=0.97, linetype="dashed",colour = "black", size=0.5)+
  geom_hline(yintercept=0.05, linetype="dashed",colour = "black", size=0.5)+
  geom_point(data=filter(CCRM,Group=="TD"),size=2,alpha=1,aes(colour=Group2)) +
  # reg lines for TD_0
  geom_line(data=filter(CCRM_TD0, Group2=="TD_0"),aes(x = Age, y = predicted), size=1) +
  # reg lines for TD_1
  geom_line(data=filter(CCRM_TD1, Group2=="TD_1"),aes(x = Age, y = predicted), size=1) +
  # geom_text(label=CCRM$listener, size=3)+
  # reg stats TD_0:
  geom_text(data = dat_text.CCRM, mapping = aes(x = -Inf, y = Inf, label = label_TD0),
            hjust = -0.05, vjust = 2,size = 3.5,show.legend = FALSE)+
  geom_text(data = dat_text.CCRM, mapping = aes(x = -Inf, y = Inf, label = label2_TD0),
            hjust = -0.06, vjust = 3.5, size = 3.5,show.legend = FALSE)+
  # reg stats TD_1:
  geom_text(data = dat_text.CCRM, mapping = aes(x = -Inf, y = -Inf, label = label_TD1),
            hjust = -0.05, vjust = -3,size = 3.5, colour = color.TDsib, show.legend = FALSE)+
  geom_text(data = dat_text.CCRM, mapping = aes(x = -Inf, y = -Inf, label = label2_TD1),
            hjust = -0.06, vjust = -1, size = 3.5, colour = color.TDsib, show.legend = FALSE)+
  # adults boxplots:
  geom_boxplot(CCRM_adults,mapping = aes(x = 14 , y = uRevs),colour="black",show.legend = FALSE) +
  labs(y = "SRdTs (proportion of duty cycle)",x = "Age (years)") + 
...
#   geom_hline(yintercept=0.05, linetype="dashed",colour = "black", size=0.5)+
#   geom_point(data=filter(ASLN,Group=="TD"),size=2,alpha=1,aes(colour=Group2)) +
#   stat_regline_equation(data=filter(ASLN,Group=="TD"), label.x = c(7,7), label.y = c(1.05,0), show.legend = FALSE)+
#   stat_cor(data=filter(ASLN,Group=="TD"), method = "spearman", cor.coef.name = "rho", aes(color = Group2), label.x = c(7,7), label.y = c(1,-0.05), na.rm = TRUE, show.legend = FALSE) +
#   # geom_text(label=ASLN$listener, size=3)+
#   geom_smooth(data=filter(ASLN,Group=="TD"),method="lm" , aes(group=Group2), se=FALSE) +
#   geom_boxplot(ASLN_adults,mapping = aes(x = 14 , y = uRevs),colour="black",show.legend = FALSE) +
#   labs(y = "SRdTs (proportion of duty cycle)",x = "Age (years)") + 
#   scale_y_continuous(limits = c(-0.05,1.05),breaks=seq(0,1,0.1)) +
#   scale_x_continuous(breaks=seq(7,14,1),labels=c("7","8","9","10","11","12","13","18-35"))+
...
#   geom_hline(yintercept=0.05, linetype="dashed",colour = "black", size=0.5)+
#   geom_point(data=filter(CCRM,Group=="TD"),size=2,alpha=1,aes(colour=Group2)) +
#   stat_regline_equation(data=filter(CCRM,Group=="TD"), label.x = c(7,7), label.y = c(1.05,0), show.legend = FALSE)+
#   stat_cor(data=filter(CCRM,Group=="TD"), method = "spearman", cor.coef.name = "rho", aes(color = Group2), label.x = c(7,7), label.y = c(1,-0.05), na.rm = TRUE, show.legend = FALSE) +
#   # geom_text(label=CCRM$listener, size=3)+
#   geom_smooth(data=filter(CCRM,Group=="TD"),method="lm", aes(group=Group2), se=FALSE) +
#   labs(y = "SRdTs (proportion of duty cycle)",x = "Age (years)") + 
#   scale_y_continuous(limits = c(-0.05,1.05),breaks=seq(0,1,0.1)) +
#   scale_x_continuous(breaks=seq(7,14,1),labels=c("7","8","9","10","11","12","13","18-35"))+
#   scale_colour_manual(values = my_colors_2TD, name = "Group", labels = c("TD",bquote(bold(TD[sib]))))+
...
###### ASL ##############
ASLN$Group2 <- factor(ASLN$Group2, levels = c("APD","TD_1","TD_0"))
CCRM$Group2 <- factor(CCRM$Group2, levels = c("APD","TD_1","TD_0"))
...
t1 <- ggplot(ASLN, aes(x=CondCode,y=z_trim,fill=Group2))+ 
  ...
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
  geom_quasirandom(dodge.width=0.8,shape=1,colour="black")+
  geom_hline(yintercept=0, linetype="dashed",color = "black", size=0.5) +
...
ASLN$Group2 <- factor(ASLN$Group2, levels = c("APD","TD_0","TD_1"))
CCRM$Group2 <- factor(CCRM$Group2, levels = c("APD","TD_0","TD_1"))
```
...
### Both materials --------
ST_z_Wilcox <- ST %>% rstatix::group_by(material) %>%
  rstatix::wilcox_test(data = ., z_trim ~ Group2, detailed=TRUE, p.adjust.method ="bonferroni") %>% 
  as.data.frame() %>%
  mutate_if(is.numeric, round, 2) %>%
  mutate( "95\\%-CI" = sprintf("%s - %s",conf.low,conf.high),
          "p-value" = ifelse(p.adj<.001,"\\textbf{< 0.001}", ifelse(p.adj<.05, sprintf("\\textbf{%.02f}",p.adj),p.adj)),
          contrast = c("APD - TD","APD - TD$_{sib}$","TD - TD$_{sib}$","APD - TD","APD - TD$_{sib}$","TD - TD$_{sib}$")) %>% 
  select(contrast,n1,n2,material,estimate,`95\\%-CI`,`p-value`)

# change some names
ST_z_Wilcox$material <-   str_replace(ST_z_Wilcox$material,"ASLN","ASL")

ST_z_effsize <- ST %>% rstatix::group_by(material) %>%
    rstatix::cohens_d(z_trim ~ Group2, data=., ci = TRUE) %>% 
    as.data.frame() %>%
    mutate_if(is.numeric, round, 2) %>%
    select(effsize,magnitude) %>%
    dplyr::rename("d"=effsize)

ST_Z.postT <- cbind(ST_z_Wilcox,ST_z_effsize)
...
CCRMfull.ST_z_Wilcox <- data.frame(rstatix::wilcox_test(z_trim ~ Group2, data = CCRM,detailed=TRUE, p.adjust.method ="bonferroni")) %>% 
  mutate( "95\\%-CI" = sprintf("%s - %s",conf.low,conf.high),
            p = ifelse(p.adj<.001,"\\textbf{< 0.001}", ifelse(p.adj<.05, sprintf("\\textbf{%.02f}",p.adj),p.adj))) %>% 
  select(n1,n2,estimate,`95\\%-CI`,p)
...
kbl(ST_Z.postT, booktabs = T,escape = F, linesep = "",caption = "ST: Post-hoc paired-comparison tests (Wilcoxon rank-sum test) for Group differences in z-scores split by material type.",
    align = c("lcclccccl"),format = "latex",digits = 2) %>% 
  kable_styling(latex_options = c("scale_down")) %>%
  # column_spec(c(4,9),border_left = T) %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
  column_spec(c(7), italic = T) %>%
...
ggpairs(CCRM_w[c(4:20,2)],
        lower = list(continuous = "smooth"),
        mapping = aes(color = CCRM_w$Group),title="ST-CCRM: Condition - Q-NoAlt (z)")

# -------------------------------------------------------

ggplot(CCRM_L_z, aes(x=CondCode,y=zScore,fill=Group))+ 
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
...
CorrAll <- Hmisc::rcorr(as.matrix(ASLN_w)[,5:9],type="spearman")
# CorrAll

# uRevs
CorrAll_s <- corstars(ASLN_w[,5:9], result="none", method="spearman",p.adj = "none")
CorrAll_s <- corstars(CCRM_w[,5:10], result="none", method="spearman",p.adj = "none")

# z-scores
CorrAll_s <- corstars(ASLN_w[,10:14], result="none", method="spearman",p.adj = "none")
CorrAll_s <- corstars(CCRM_w[,11:16], result="none", method="spearman",p.adj = "none")
...
```{r,label='ST-DerivedScoreSRdT',echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
ASLN_w <- ASLN %>%
  pivot_wider(
    id_cols = c("listener","Group","Age","Group2"),
    names_from = "CondCode",
...
# -- get derived scores --------------------------------------
# uRevs
ASLN_w$Diff_QNoAlt <-  ASLN_w$uRevs_Q_ASLN_Alt     - ASLN_w$uRevs_Q_ASLN_NoAlt
ASLN_w$Diff_AMSSN  <-  ASLN_w$uRevs_AMSSN_ASLN_Alt - ASLN_w$uRevs_Q_ASLN_Alt
ASLN_w$Diff_MDR    <-  ASLN_w$uRevs_MDR_F_ASLN_Alt - ASLN_w$uRevs_Q_ASLN_Alt
ASLN_w$Diff_ENG    <-  ASLN_w$uRevs_ENG_F_ASLN_Alt - ASLN_w$uRevs_Q_ASLN_Alt

# -- Change data layout from Wide2Long -----------------------
ASLN_L_uRevs <- ASLN_w %>%
  select(listener,Age, Group,Group2, Diff_QNoAlt:Diff_ENG) %>%
  pivot_longer(
    cols = c("Diff_QNoAlt","Diff_AMSSN",
             "Diff_MDR","Diff_ENG"),
    names_to = "CondCode",
...
# This might help to get a more accurate fit.
# All observations where returned in the final stage, for z-scores calculation!
ASLN_L_uRevs$TempRmv <- 0
ASLN_L_uRevs$TempRmv[c(93:94,121:124)] <- 1
...
# -- get derived scores --------------------------------------
# uRevs
CCRM_w$Diff_QNoAlt <-  CCRM_w$uRevs_Q_CCRM_Alt      - CCRM_w$uRevs_Q_CCRM_NoAlt
CCRM_w$Diff_AMSSN  <-  CCRM_w$uRevs_AMSSN_CCRM_Alt  - CCRM_w$uRevs_Q_CCRM_Alt
CCRM_w$Diff_MDR    <-  CCRM_w$uRevs_MDR_F_CCRM_Alt  - CCRM_w$uRevs_Q_CCRM_Alt
CCRM_w$Diff_ENG    <-  CCRM_w$uRevs_ENG_F_CCRM_Alt  - CCRM_w$uRevs_Q_CCRM_Alt
CCRM_w$Diff_CCRM   <-  CCRM_w$uRevs_CCRM_F_CCRM_Alt - CCRM_w$uRevs_Q_CCRM_Alt

# -- Change data layout from Wide2Long -----------------------
CCRM_L_uRevs <- CCRM_w %>%
  select(listener, Age, Group, Group2, Diff_QNoAlt:Diff_CCRM) %>%
  pivot_longer(
    cols = c("Diff_QNoAlt","Diff_AMSSN",
             "Diff_MDR","Diff_ENG","Diff_CCRM"),
    names_to = "CondCode",
    names_ptypes = list(CondCode = factor()),
    values_to = "uRevs") %>%
  ungroup()
...
# This might help to get a more accurate fit.
# All observations where returned in the final stage, for z-scores calculation!
CCRM_L_uRevs$TempRmv <- 0
CCRM_L_uRevs$TempRmv[c(124,156:160,171)] <- 1
```

```{r,label='ST-DerivedScoreGetz',echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# -------------------------------------------------------------
# get z-scores:
# -------------------------------------------------------------
...
df <- ASLN_L_uRevs %>% filter(CondCode=="Diff_QNoAlt" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(ASLN_L_uRevs,"Diff_QNoAlt",CutOff,slope1_temp,int1_temp,8,05)
df_1 <- Output[[1]]  # df_All
TD_trimmed_1 = Output[[11]]
FinalM_1 <- if(Output[[8]]==1){FinalM_1=Output[[6]]}else{FinalM_1=Output[[7]]}
L1<- sprintf("slope = %s, intcpt = %s",
...
### Diff_AMSSN ###
df <- ASLN_L_uRevs %>% filter(CondCode=="Diff_AMSSN" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(ASLN_L_uRevs,"Diff_AMSSN",CutOff,slope1_temp,int1_temp,8,05)
df_2 <- Output[[1]]  # df_All
TD_trimmed_2 = Output[[11]]
FinalM_2 <- if(Output[[8]]==1){FinalM_2=Output[[6]]}else{FinalM_2=Output[[7]]}
L2<- sprintf("slope = %s, intcpt = %s",
...
### Diff_MDR ###
df <- ASLN_L_uRevs %>% filter(CondCode=="Diff_MDR" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(ASLN_L_uRevs,"Diff_MDR",CutOff,slope1_temp,int1_temp,10,1)
df_3 <- Output[[1]]  # df_All
TD_trimmed_3 = Output[[11]]
FinalM_3 <- if(Output[[8]]==1){FinalM_3=Output[[6]]}else{FinalM_3=Output[[7]]}
L3<- sprintf("slope = %s, intcpt = %s",
...
### Diff_ENG  ###
df <- ASLN_L_uRevs %>% filter(CondCode=="Diff_ENG" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(ASLN_L_uRevs,"Diff_ENG",CutOff,slope1_temp,int1_temp,10,1)
df_4 <- Output[[1]]  # df_All
TD_trimmed_4 = Output[[11]]
FinalM_4 <- if(Output[[8]]==1){FinalM_4=Output[[6]]}else{FinalM_4=Output[[7]]}
L4<- sprintf("slope = %s, intcpt = %s",
...
## combine all conditions together
# FinalM.ASL_Diff <- list(FinalM_1,FinalM_2,FinalM_3,FinalM_4)
ASLN_Diff <- rbind(df_1,df_2,df_3,df_4)
Lines.ASL_Diff <- rbind(L1,L2,L3,L4)
LStats.ASL_Diff <- rbind(Lstats_1,Lstats_2,Lstats_3,Lstats_4)
TD_trimmed.ASL_Diff <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3,TD_trimmed_4)

# get outliers: 
ASLN_Diff <- ASLN_Diff %>% group_by(CondCode) %>%
  mutate(Outlier = ifelse(z_trim > CutOff,1,0))

plyr::ddply(ASLN_Diff,~CondCode*Group,summarise,nConde= length(listener),nOutlier=sum(Outlier, na.rm=TRUE),prcntOutlier=round((sum(Outlier,na.rm=TRUE)/length(listener)*100),1))
...
### Diff_QNoAlt ###
df <- CCRM_L_uRevs %>% filter(CondCode=="Diff_QNoAlt" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(CCRM_L_uRevs,"Diff_QNoAlt",CutOff,slope1_temp,int1_temp,10)
df_1 <- Output[[1]]  # df_All
TD_trimmed_1 = Output[[11]]
FinalM_1 <- if(Output[[8]]==1){FinalM_1=Output[[6]]}else{FinalM_1=Output[[7]]}
L1<- sprintf("slope = %s, intcpt = %s",
...
### Diff_AMSSN ###
df <- CCRM_L_uRevs %>% filter(CondCode=="Diff_AMSSN" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(CCRM_L_uRevs,"Diff_AMSSN",CutOff,slope1_temp,int1_temp,10,05)
df_2 <- Output[[1]]  # df_All
TD_trimmed_2 = Output[[11]]
FinalM_2 <- if(Output[[8]]==1){FinalM_2=Output[[6]]}else{FinalM_2=Output[[7]]}
L2<- sprintf("slope = %s, intcpt = %s",
...
### Diff_MDR ###
df <- CCRM_L_uRevs %>% filter(CondCode=="Diff_MDR" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(CCRM_L_uRevs,"Diff_MDR",CutOff,slope1_temp,int1_temp,10,1)
df_3 <- Output[[1]]  # df_All
TD_trimmed_3 = Output[[11]]
FinalM_3 <- if(Output[[8]]==1){FinalM_3=Output[[6]]}else{FinalM_3=Output[[7]]}
L3<- sprintf("slope = %s, intcpt = %s",
...
### Diff_ENG  ###
df <- CCRM_L_uRevs %>% filter(CondCode=="Diff_ENG" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(CCRM_L_uRevs,"Diff_ENG",CutOff,slope1_temp,int1_temp,10,1)
df_4 <- Output[[1]]  # df_All
TD_trimmed_4 = Output[[11]]
FinalM_4 <- if(Output[[8]]==1){FinalM_4=Output[[6]]}else{FinalM_4=Output[[7]]}
L4<- sprintf("slope = %s, intcpt = %s",
...
### Diff_CCRM  ###
df <- CCRM_L_uRevs %>% filter(CondCode=="Diff_CCRM" & Group=="TD" & TempRmv <1) %>% droplevels()
slope1_temp <- mean(df$uRevs)
int1_temp <- mean(df$Age)
# ggplot(df, aes(x=Age,y=uRevs))+
#   geom_point()+
...
Output <- getBestFit(CCRM_L_uRevs,"Diff_CCRM",CutOff,slope1_temp,int1_temp,8,05)
df_5 <- Output[[1]]  # df_All
TD_trimmed_5 = Output[[11]]
FinalM_5 <- if(Output[[8]]==1){FinalM_5=Output[[6]]}else{FinalM_5=Output[[7]]}
L5<- sprintf("slope = %s, intcpt = %s",
...
## combine all conditions together
# FinalM.CCRM_Diff <- list(FinalM_1,FinalM_2,FinalM_3,FinalM_4,FinalM_5)
CCRM_Diff <- rbind(df_1,df_2,df_3,df_4,df_5)
Lines.CCRM_Diff <- rbind(L1,L2,L3,L4,L5)
LStats.CCRM_Diff <- rbind(Lstats_1,Lstats_2,Lstats_3,Lstats_4,Lstats_5)
TD_trimmed.CCRM_Diff <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3,TD_trimmed_4,TD_trimmed_5)

# get outliers: 
CCRM_Diff <- CCRM_Diff %>% group_by(CondCode) %>%
  mutate(Outlier = ifelse(z_trim > CutOff,1,0))

plyr::ddply(CCRM_Diff,~CondCode*Group,summarise,nConde= length(listener),nOutlier=sum(Outlier, na.rm=TRUE),prcntOutlier=round((sum(Outlier,na.rm=TRUE)/length(listener)*100),1))

```

##### Proportion of duty cycle (DC)

```{r, echo=FALSE, eval=FALSE}

# Quiet: Alt vs NoAlt
ggplot(ASLN_w, aes(x=uRevs_Q_ASLN_Alt, y=uRevs_Q_ASLN_NoAlt, colour=Group)) +
  geom_point(size=2,alpha=1,show.legend = FALSE, aes(colour=Group)) +
  geom_smooth(method="lm" , aes(colour=Group), se=FALSE) 

ggplot(CCRM_w, aes(x=uRevs_Q_CCRM_Alt, y=uRevs_Q_CCRM_NoAlt, colour=Group)) +
  geom_point(size=2,alpha=1,show.legend = FALSE, aes(colour=Group)) +
  geom_smooth(method="lm" , aes(colour=Group), se=FALSE) 

# Switching cost: split by siblings

ggplot(ST_trimmed_Diff_switch, aes(x=Age, y=uRevs, colour=Group2)) +
  geom_point(size=2,alpha=1,show.legend = FALSE) +
  facet_grid(. ~ material, scales = "free", switch = "y")+
  # facet_wrap(., var(material),nrow=1)+
 geom_smooth(method="lm" , aes(group=Group2), se=FALSE) 

# Switching cost: 

ggplot(ST_trimmed_Diff_switch, aes(x=Age, y=uRevs)) +
  geom_point(size=2,alpha=1,show.legend = FALSE) +
  facet_grid(. ~ material, scales = "free", switch = "y")+
  # facet_wrap(., var(material),nrow=1)+
 geom_smooth(method="lm", se=FALSE) 

ggplot(ST_trimmed_Diff, aes(x=Age, y=uRevs)) +
  geom_point(size=2,alpha=1,show.legend = FALSE) +
  facet_grid(. ~ material*CondCode, scales = "free", switch = "y")+
  # facet_wrap(., var(material),nrow=1)+
 geom_smooth(method="lm" , se=FALSE) 



# x <- ST_trimmed_Diff_switch %>%
#   pivot_wider(
...
#   ungroup() %>% drop_na()
# 
# x %>% ggplot( aes(x=CCRM, y=ASL)) + geom_point() 
# 
# 
# cor(x$CCRM, x$ASL)
# 
# require(GGally)
# 
# ST_trimmed_Diff_w <- ST_trimmed_Diff %>%
#   pivot_wider(
#     id_cols = c("listener","Group","Age"),
#     names_from = c("material","CondCode"),
#     values_from = c("uRevs")) %>%
...
# 
# 
# ggpairs(ST_trimmed_Diff_w[c(4:9)],
#         lower = list(continuous = "smooth"),
...
\
The cost of switching the stimuli between the two ears was calculated for the test conditions without an additional distractor by subtracting the non-switched condition Quiet-NoAlt from the switched condition Quiet-Alt. Whereby, the larger the derived score (in DC), the larger the switching cost. Similarly, the effect of IM after accounting for the switching cost was calculated by subtracting the Quiet-Alt condition from the test conditions with distractors (see Figure\ \@ref(fig:ST-DerivedPlotsuRevs)). It appears from the figure that performance in the TD and APD group are more similar for less challenging conditions, and becomes more different for more challenging conditions with distractors. The results indicates that switching alone or the use of nonespeech distractor had little effect on the children from both groups. We hypothesised that the switching cost and IM effect would decline with age, resulting in negative correlation between the derived score and age. Looking at the TD group regression lines, this was mostly true, except for the CCRM conditions AMSSN and MDR. Nonetheless, the effect of age was generally small.\


```{r, label='ST-DerivedPlotsuRevs', fig.cap="ST: Scatterplot and linear regression lines for the listeners derived scores (in DC) with the ASL (A) and the CCRM speech material (B) as a function of age. Corresponding regression coefficients and statistics are provided for the TD group only. Red indicates data from the APD group and blue indicates data from the TD control group (square shapes: TD children with an APD sibling; triangle shapes: the remaining TD children).", fig.align='center', fig.width=14, fig.asp=0.8, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE,fig.pos='h'}

# meaning of the difference score (condition - Q-Alt):
# - Conditions with Nz: IM effect/intrusion 
# - Quiet: switching effect

dat_text.ASL_d <- data.frame(
  label = Lines.ASL_Diff,
  label2 = matrix(LStats.ASL_Diff),
  CondCode   = levels(ASLN_Diff$CondCode),
  Group = "TD",
  Group2 = "TD_0")

dat_text.CCRM_d <- data.frame(
  label = Lines.CCRM_Diff,
  label2 = matrix(LStats.CCRM_Diff),
  CondCode   = levels(CCRM_Diff$CondCode),
  Group = "TD",
  Group2 = "TD_0")

###### ASL ##############

p1 <- ggplot(ASLN_Diff, aes(x=Age, y=uRevs, colour=Group)) +
  geom_point(size=2,alpha=1,aes(colour=Group, shape=Group2),show.legend = FALSE) +
  # geom_text(label=ASLN_Diff$listener, size=3)+
  # get TD lines
  geom_line(data=filter(ASLN_Diff, Group=="TD"),aes(x = Age, y = predicted), size=1) +
  # get APD lines
  geom_smooth(data=filter(ASLN_Diff,Group=="APD"),method="lm" , aes(group=Group), se=FALSE) +
  geom_text(data = dat_text.ASL_d, mapping = aes(x = -Inf, y = -Inf, label = label),
            hjust = -0.05, vjust = -3,size = 3.5,show.legend = FALSE)+
  geom_text(data = dat_text.ASL_d, mapping = aes(x = -Inf, y = -Inf, label = label2),
            hjust = -0.06, vjust = -1, size = 3.5,show.legend = FALSE)+
  labs(y = latex2exp::TeX("\\textbf{\\overset{proportion of duty cycle (DC)}{$\\leftarrow$ better performance}}"),x = "Age (years)")+ 
  scale_y_continuous(limits = c(-0.5,0.6),breaks=seq(-0.5,0.6,0.2)) +
  # guides(shape=FALSE) +
  scale_colour_manual(values = my_colors_2)+
  theme_bw() +
  theme(axis.text = element_text(size = 10, face="bold",colour = "black"),
...
p1 <- p1 + facet_grid(. ~ reorder(CondCode,CondCode), scales = "free", switch = "y",
                      labeller = as_labeller(c("Diff_QNoAlt" ="Switching \n (Quiet-Alt - Quiet-NoAlt)","Diff_AMSSN"="AMSSN \n (AMSSN - Quiet-Alt)",
                            "Diff_MDR"="MDR_F \n (MDR_F - Quiet-Alt)","Diff_ENG"="ENG_F \n (ENG_F - Quiet-Alt)"))) +
  theme(panel.spacing.x = unit(0,"line"),
        strip.background = element_rect(fill="white", colour="white"),
        strip.text = element_text(face = "bold", size = 11))
...
p2 <- ggplot(CCRM_Diff, aes(x=Age, y=uRevs, colour=Group)) +
  geom_point(size=2,alpha=1,aes(colour=Group, shape=Group2),show.legend = FALSE) +
  # geom_text(label=CCRM_Diff$listener, size=3)+
  # get TD lines
  geom_line(data=filter(CCRM_Diff, Group=="TD"),aes(x = Age, y = predicted), size=1) +
  # get APD lines
  geom_smooth(data=filter(CCRM_Diff,Group=="APD"),method="lm" , aes(group=Group), se=FALSE) +
  geom_text(data = dat_text.CCRM_d, mapping = aes(x = -Inf, y = -Inf, label = label),
            hjust = -0.05, vjust = -3,size = 3.5,show.legend = FALSE)+
  geom_text(data = dat_text.CCRM_d, mapping = aes(x = -Inf, y = -Inf, label = label2),
            hjust = -0.06, vjust = -1, size = 3.5,show.legend = FALSE)+
  labs(y = latex2exp::TeX("\\textbf{\\overset{proportion of duty cycle (DC)}{$\\leftarrow$ better performance}}"),x = "Age (years)")+ 
  scale_y_continuous(limits = c(-0.5,0.6),breaks=seq(-0.5,0.6,0.2)) +
  # scale_x_continuous(breaks=seq(7,14,1),labels=c("7","8","9","10","11","12","13","18-35"))+
...
p2 <- p2 + facet_grid(. ~ reorder(CondCode,CondCode), scales = "free", switch = "y",
                      labeller = as_labeller(c("Diff_QNoAlt" ="Switching \n (Quiet-Alt - Quiet-NoAlt)","Diff_AMSSN"="AMSSN \n (AMSSN - Quiet-Alt)",
                            "Diff_MDR"="MDR_F \n (MDR_F - Quiet-Alt)","Diff_ENG"="ENG_F \n (ENG_F - Quiet-Alt)",
                            "Diff_CCRM"="CCRM_F \n (CCRM_F - Quiet-Alt)"))) +
  theme(panel.spacing.x = unit(0,"line"),
...
(p1 + p2) + plot_layout(ncol = 1, nrow = 2, heights = c(1, 1), widths = c(1.5,.5)) + plot_annotation(tag_levels = 'A') + plot_layout(guides='collect') &
  theme(legend.position='bottom',        
        legend.direction = "horizontal",
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"),
...
Two separate LMEM models were used to examine the effect of switching and IM with the derived scores (in DC) as dependent variable and random intercepts for subjects (parametric model assumptions were met). Note that data for CCRM_F was excluded from the analysis since it was only measured for the CCRM material. Furthermore, since we did not find strong evidence for sibling effect for the non-derived scores, the TD children were treated as a single group. The first model examined the effect of switching with Material and Age as fixed factors. The model that gave the best fit did not include Material, indicating that the switching cost was the same across the two material types. However, there was a significant main effect of Age ($\chi^{2}$ = 4.448, $Df$ = 1, $p$ = 0.035). IM effects were tested for the three distractors (Distractor), Material, and Age as fixed factors. The model that gave the best fit and test results are given in Table\ \@ref(tab:ST-AgeIMEffTab). There was a significant main effect of Material ($p$ = 0.037) as well as a marginally significant Distractor x Age interaction ($p$ = 0.05). Additional LMEM model looked into IM effect in the full CCRM data, including the CCRM_F distractor (parametric model assumptions were met). The model that gave the best fit included Distractor x Age which was again marginally significant ($\chi^{2}$ = 7.836, $Df$ = 3, $p$ = 0.05).\

<!-- Material ($\chi^{2}$ = 0.3078, $Df$ = 1, $p$ = 0.579) -->

<!-- IM effects were tested using LMEM model for the three distractors (Distractor), Material, Sibling, and Age as fixed factors and random intercepts for subjects. The model that gave the best fit did not include the fixed factors Sibling or Age (see Table\ \@ref(tab:ST-AgeIMEffTab)). There was a highly significant main effect of Distractor ($p$\ \<\ 0.001) as well as a significant effect of Material ($p$\ \< 0.05). Post-hoc paired-comparison t-tests based on the fitted model for Distractor (with Bonferroni correction) found a significant difference in IM effect between AMSSN and MDR ($Estimate$ = -0.24, $95\%-CI$ = -0.29 - -0.19, $p$\ \< 0.001) and AMSSN and ENG_F ($Estimate$ = -0.28, $95\%-CI$ = -0.33 - -0.23, $p$\ \< 0.001), whereas there was no significant difference between the two speech distractors ($Estimate$ = -0.04 $95\%-CI$ = -0.09 - 0.01, $p$ = 0.16).\ -->

<!-- Additional LMEM model looked into IM effect in the CCRM data, including also data for the CCRM_F distractor (parametric model assumptions were met). The model that gave the best fit included the following interaction terms: Distractor x Age \& and Distractor x Sibling. There was a significant Distractor x Age interaction ($\chi^{2}$ = 8.275, $Df$ = 3, $p$ = 0.041), whereas there was no significant interaction for Distractor x Sibling ($\chi^{2}$ = 5.362, $Df$ = 3, $p$ = 0.147) or main effect of Sibling ($\chi^{2}$ = 5.382, $Df$ = 4, $p$ = 0.250).\ -->

<!-- **Post-hoc test for Distractor x Age interaction:** -->
<!-- Model based EMMs suggests opposite age effects between conditions:  -->

<!-- - AMSSN \& MDR_F: IM is the largest for older kids and smallest in younger kids (WHY?!)   -->

<!-- - ENG_F and CCRM_F: IM is the largest for younger kids and decreases with age.   -->

<!-- A model without sibs -->
...
TD_trimmed.CCRM_Diff$material <- "CCRM"

TD_trimmed.CCRM_Diff_s <- TD_trimmed.CCRM_Diff %>% filter(CondCode!="Diff_CCRM") %>% 
  dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs) %>% 
  as.data.frame(.) %>%
  droplevels()

TD_trimmed.ASL_Diff$material <- "ASL"
TD_trimmed.ASL_Diff_s <- TD_trimmed.ASL_Diff %>% 
  dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs) %>% 
  as.data.frame(.) %>% 
  droplevels()

ST_trimmed_Diff <- rbind(TD_trimmed.ASL_Diff_s,TD_trimmed.CCRM_Diff_s)

ST_trimmed_Diff <- ST_trimmed_Diff %>% filter(CondCode!="Diff_QNoAlt") %>%droplevels()
...
model1 <- lmer(uRevs~CondCode*material*Age+(1|listener), ST_trimmed_Diff, REML=FALSE)
summary(model1)
...
(model4<-update(model3,  ~ . - CondCode:material))
anova(model3,model4)
summary(model4)
...
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model5    7 -186.15 -166.19 100.08  -200.15                       
# model4    9 -189.50 -163.83 103.75  -207.50 7.3472  2    0.02539 *

# best model --> model4:  uRevs ~ CondCode + material + Age + CondCode:Age + (1 | listener)
...
# Response: uRevs
#                 Chisq Df Pr(>Chisq)    
# CondCode     195.7700  2    < 2e-16 ***
# material       4.4482  1    0.03494 *  
# Age            0.6879  1    0.40687    
# CondCode:Age   7.6491  2    0.02183 *  
...
#             npar     AIC      BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1    7 -122.44 -105.423 68.219  -136.44                       
# BestModel     10 -124.28  -99.967 72.138  -144.28 7.8367  3    0.04951 *
...
summary(BestModel.2)
# BestModel.2: uRevs ~ CondCode + Age + (1 | listener) + CondCode:Age
# BestModel: uRevs ~ CondCode + material + Age + (1 | listener) + CondCode:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.2    8 -187.13 -164.31 101.56  -203.13                       
# BestModel      9 -189.50 -163.83 103.75  -207.50 4.3706  1    0.03656 
...
# linear model
w1 <- lm(uRevs~CondCode*material*Age, data = ST_trimmed_Diff)
...
# Option 1:
car::leveneTest(uRevs ~ material* CondCode, data=ST_trimmed_Diff,center=median)

# Option 2: 
DescTools::LeveneTest(lm(uRevs~ material* CondCode, data = ST_trimmed_Diff))
...
ST_trimmed_Diff_switch <- rbind(TD_trimmed.ASL_Diff,TD_trimmed.CCRM_Diff)

ST_trimmed_Diff_switch <- ST_trimmed_Diff_switch %>% filter(CondCode=="Diff_QNoAlt") %>% select(., -z) %>% droplevels() %>% as.data.frame(.) 
...
ST_trimmed_Diff_switch$material <- factor(ST_trimmed_Diff_switch$material)

# count subjects observations
var_select = c("listener")
plyr::count(ST_trimmed_Diff_switch, var_select)

# ddply(ST_trimmed_Diff_switch,~Group2,summarise,N=length(uRevs),median=median(uRevs,na.rm=TRUE),mean=mean(uRevs,na.rm=TRUE),sd=sd(uRevs,na.rm=TRUE), min=min(uRevs,na.rm=TRUE),max=max(uRevs,na.rm=TRUE)) %>%
#    mutate_if(is.numeric, round, 2) %>% arrange(., group_by = Group2)

######## Parametric model ##########################################
## Find Best fit 

model1 <- lmer(uRevs~material*Age+(1|listener), ST_trimmed_Diff_switch, REML=FALSE)
m1.lm<- lm(uRevs~material*Age, ST_trimmed_Diff_switch, REML=FALSE)
# lmer is singular..
summary(model1)
...
(model2<-update(model1,  ~ . - material:Age))
anova(model1,model2)
summary(model2)


(model3<-update(model2,  ~ . - material))
anova(model2,model3)
summary(model3)
...
anova(model3,model4)
summary(model4)
# model4: uRevs ~ (1 | listener)
# model3: uRevs ~ Age + (1 | listener)
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model4    3 -74.950 -69.809 40.475  -80.950                       
# model3    4 -77.399 -70.544 42.699  -85.399 4.4485  1    0.03493 *
...
res.aov <- anova_test(data = ST_trimmed_Diff_switch, dv = uRevs, wid = listener, within = material, between = Age,detailed=TRUE)
...
# linear model
w1 <- lm(uRevs~material*Age, data = ST_trimmed_Diff_switch)
...
# Option 1:
car::leveneTest(uRevs ~ material, data=ST_trimmed_Diff_switch,center=median)

# Option 2: 
DescTools::LeveneTest(lm(uRevs~ material, data = ST_trimmed_Diff_switch))
```
...
<!-- DELETE? -->
<!-- ---------------------------------------------------------------------------------------------------------------- -->
<!-- A model with sibs and age -->
```{r,label='ST-IMEffectDerivedOLD', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
<!-- A model with sibs and age -->
```{r,label='ST-SwitchEffectDerivedOLD', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
anova(model1,model2)
summary(model2)
# model2: uRevs ~ material + Group2 + Age + (1 | listener) + material:Group2 + 
# model2:     material:Age + Group2:Age
# model1: uRevs ~ material * Group2 * Age + (1 | listener)
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# model2    9 -78.080 -62.658 48.040  -96.080                     
# model1   10 -76.092 -58.957 48.046  -96.092 0.0121  1     0.9124

(model3<-update(model2,  ~ . - material:Group2))
anova(model2,model3)
summary(model3)
# model3: uRevs ~ material + Group2 + Age + (1 | listener) + material:Age + 
# model3:     Group2:Age
# model2: uRevs ~ material + Group2 + Age + (1 | listener) + material:Group2 + 
# model2:     material:Age + Group2:Age
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# model3    8 -79.167 -65.459 47.584  -95.167                     
# model2    9 -78.080 -62.658 48.040  -96.080 0.9128  1     0.3394

(model4<-update(model3,  ~ . - material:Age))
anova(model3,model4)
summary(model4)
# model4: uRevs ~ material + Group2 + Age + (1 | listener) + Group2:Age
# model3: uRevs ~ material + Group2 + Age + (1 | listener) + material:Age + 
# model3:     Group2:Age
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model4    7 -78.307 -66.312 46.154  -92.307                       
# model3    8 -79.167 -65.459 47.584  -95.167 2.8601  1     0.0908 .

(model5<-update(model4,  ~ . - Group2:Age))
anova(model4,model5)
summary(model5)
# model5: uRevs ~ material + Group2 + Age + (1 | listener)
# model4: uRevs ~ material + Group2 + Age + (1 | listener) + Group2:Age
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model5    6 -76.123 -65.841 44.061  -88.123                       
# model4    7 -78.307 -66.312 46.154  -92.307 4.1848  1    0.04079 *
...
# # visreg::visreg(model4)
# # visreg::visreg(model2,"Group2","Age")
...
model1.lm <- lm(uRevs ~ material*Group2*Age, data = ST_trimmed_Diff_switch)
summary(model1.lm)
anova(model1.lm)

(model2.lm<-update(model1.lm,  ~ . - material:Group2:Age))
anova(model1.lm,model2.lm)
summary(model2.lm)

(model3.lm<-update(model2.lm,  ~ . - Group2:Age))
anova(model2.lm,model3.lm)
summary(model3.lm)

# Best model: model2: uRevs ~ material + Group2 + Age + material:Group2 + material:Age + Group2:Age
# summary(model2.lm)
# tab_model(model2.lm, show.df = TRUE)
# anova(model2.lm)

ST_IM_DC_tab <- apaTables::apa.aov.table(model2.lm,conf.level = 0.95)[["table_body"]]
ST_IM_DC_tab$Predictor <- str_replace_all(ST_IM_DC_tab$Predictor,"material","Material")
ST_IM_DC_tab$Predictor <- str_replace_all(ST_IM_DC_tab$Predictor,"Group2","Sibling")

ST_IM_DC_tab$p<- c("0.062","0.127", "0.098","0.127","0.388","0.168","\\textbf{0.043}", "")
  
colnames(ST_IM_DC_tab) <- c("Predictor", "$SS$", "$df$", "$MS$", "$F$", "$p$", "$\\eta^{2}_{partial}$", "$\\eta^{2}_{partial}$ 95\\%-CI")


# model1.aov <- aov(uRevs ~ material*Group2*Age, data = ST_trimmed_Diff_switch)
# summary(model1.aov)
# anova(model1.aov)
# # 
# tab_model(model1.lm, show.df = TRUE)
# BestModel <- model1
# -------------------------------------------------------------------------
# get table
# -------------------------------------------------------------------------
...
# linear model
w1 <- lm(uRevs~material*Group2*Age, data = ST_trimmed_Diff_switch)
...
<!-- A model without Age -->
```{r,label='ST-SwitchEffectDerived2', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
# TD_trimmed.CCRM_Diff$material <- "CCRM"
# TD_trimmed.ASL_Diff$material <- "ASL"
# ST_trimmed_Diff <- rbind(TD_trimmed.ASL_Diff,TD_trimmed.CCRM_Diff)
# write.csv(ST_trimmed_Diff,"ST_trimmed_Diff.csv")
...
(model2<-update(model1,  ~ . - material:Group2))
# anova(model1,model2)
# summary(model2)
(model3<-update(model2,  ~ . - material))
# anova(model2,model3)
...
GroupAge <- anova(BestModel,BestModel.1)
summary(BestModel.1)

# material:Age
(BestModel.2 <- update(BestModel,  . ~ . -material:Age))
MatAge <- anova(BestModel,BestModel.2)
summary(BestModel.2)

# material:Age
(BestModel.3 <- update(BestModel,  . ~ . -material:Group2))
MatAge <- anova(BestModel,BestModel.3)
summary(BestModel.4)

# 
# tab_model(model2)
# # visreg::visreg(model5)
# # visreg::visreg(model5,"Group","CondCode")
# # visreg::visreg(model5,"Group","material")
# 
# BestModel <- model2
# report::report(model2)
...
```{r,label='ST-AgeAltEffTab',echo=FALSE,warning=FALSE, message=FALSE, eval=FALSE}

kbl(ST_IM_DC_tab, booktabs = T,escape = F, linesep = "",row.names = F,caption = "ST: Derived measures (DC): A linear regression model with DC as a dependent variable and Material, Sibling and Age as predictors (reference levels: Material = ASL; Sibling = TD). Note: only TD data following outliers trimming was included.",
    align = c("lccccccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c("DC ~ Material + Sibling + Age + Material:Sibling + Material:Age + Sibling:Age" = 8), align = "l",line = TRUE) %>%
  # column_spec(1, width = "8cm") %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
  column_spec(6, italic = T)
...
<!-- CCRM ONLY (with sibs) -->
```{r,label='ST-IMEffectDerivedCCRMoLD', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
TD_trimmed.CCRM_Diff_s <- TD_trimmed.CCRM_Diff %>% filter(CondCode!="Diff_QNoAlt") %>% 
  dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs) %>% 
  as.data.frame(.) %>%
  droplevels()

TD_trimmed.CCRM_Diff_s$listener <- factor(TD_trimmed.CCRM_Diff_s$listener)
...
model1 <- lmer(uRevs~CondCode*Group2*Age+(1|listener), TD_trimmed.CCRM_Diff_s, REML=FALSE)
summary(model1)

(model2<-update(model1,  ~ . - CondCode:Group2:Age))
anova(model1,model2)
summary(model2)

(model3<-update(model2,  ~ . - Group2:Age))
anova(model2,model3)
summary(model3)
...
anova(model3,model4)
summary(model4)
# model4: uRevs ~ CondCode + Group2 + Age + (1 | listener) + CondCode:Group2
# model3: uRevs ~ CondCode + Group2 + Age + (1 | listener) + CondCode:Group2 + 
# model3:     CondCode:Age
#        npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model4   11 -96.709 -69.840 59.355  -118.71                       
# model3   14 -98.985 -64.787 63.492  -126.98 8.2755  3    0.04065 *


# best mode --> model 3: uRevs ~ CondCode + Group2 + Age + CondCode:Group2 + CondCode:Age + (1 | listener)

tab_model(model3)
# visreg::visreg(model3)
# visreg::visreg(model3,"CondCode","Group2")
...
# Response: uRevs
#                    Chisq Df Pr(>Chisq)    
# CondCode        227.5840  3    < 2e-16 ***
# Group2            0.0209  1    0.88517    
# Age               0.0934  1    0.75991    
# CondCode:Group2   5.5479  3    0.13580    
# CondCode:Age      8.8459  3    0.03141 * 

# CondCode x Age:
(BestModel.1 <- update(BestModel,  . ~ . -CondCode:Age))
CondAge.LMEM <- anova(BestModel,BestModel.1)
summary(BestModel.1)
# BestModel.1: uRevs ~ CondCode + Group2 + Age + (1 | listener) + CondCode:Group2
# BestModel: uRevs ~ CondCode + Group2 + Age + (1 | listener) + CondCode:Group2 + 
# BestModel:     CondCode:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1   11 -96.709 -69.840 59.355  -118.71                       
# BestModel     14 -98.985 -64.787 63.492  -126.98 8.2755  3    0.04065 *


# CondCode x Group2:
(BestModel.2 <- update(BestModel,  . ~ . -CondCode:Group2))
CondGroup.LMEM <- anova(BestModel,BestModel.2)
summary(BestModel.2)
# BestModel.2: uRevs ~ CondCode + Group2 + Age + (1 | listener) + CondCode:Age
# BestModel: uRevs ~ CondCode + Group2 + Age + (1 | listener) + CondCode:Group2 + 
# BestModel:     CondCode:Age
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)
# BestModel.2   11 -99.623 -72.754 60.811  -121.62                     
# BestModel     14 -98.985 -64.787 63.492  -126.98 5.3617  3     0.1471


# Group2:
(BestModel.3 <- update(BestModel,  . ~ . -Group2 -CondCode:Group2))
Group2.LMEM <- anova(BestModel,BestModel.3)
summary(BestModel.3)
...
#                       round(upper.CL,2))) 
# 
# postLMEM.1_d <- ST_trimmed_Diff %>%
#     rstatix::cohens_d(uRevs ~ material, data=., ci = TRUE) %>% as.data.frame() %>%
#     select(effsize,magnitude) %>%
#     dplyr::rename("d"=effsize)

# Condition: -----------------------------------------------------------------------------------
# emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ CondCode,adjust="bonferroni") 
# 
# postLMEM.1 <- summary(emm_s.conf,infer = c(TRUE,TRUE))$contrasts[] %>% 
#                data.frame() %>% 
#                mutate_if(is.numeric, round, 2) %>%
...
# ST_AgeTab_IM <- data.frame(rbind(
#   c("Distractor",Cond.LMEM$Df[2],Cond.LMEM$Chisq[2],ifelse(Cond.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}",
#                                                                    ifelse(Cond.LMEM$`Pr(>Chisq)`[2] <.05,
#                                                                           sprintf("\\textbf{%.03f}",Cond.LMEM$`Pr(>Chisq)`[2]),
#                                                                           Cond.LMEM$`Pr(>Chisq)`[2]))),
#     c("Material",Mat.LMEM$Df[2],Mat.LMEM$Chisq[2],sprintf("\\textbf{%0.3f}",Mat.LMEM$`Pr(>Chisq)`[2]))
#   ))
# 
# 
# ST_AgeTab_IM[,c(2:3)]= apply(ST_AgeTab_IM[,c(2:3)], 2, function(x) as.numeric(as.character(x)))
...
# linear model
w1 <- lm(uRevs~CondCode*Group2*Age, data = TD_trimmed.CCRM_Diff_s)
...
# Option 1:
car::leveneTest(uRevs ~ CondCode * Group2, data=TD_trimmed.CCRM_Diff_s,center=median)

# Option 2: 
DescTools::LeveneTest(lm(uRevs~ CondCode * Group2, data = TD_trimmed.CCRM_Diff_s))
...
# TD_trimmed.CCRM_Diff_s$Model.F.Res<- residuals(BestModel) #extracts the residuals and places them in a new column in our original data table
# TD_trimmed.CCRM_Diff_s$Abs.Model.F.Res <-abs(TD_trimmed.CCRM_Diff_s$Model.F.Res) #creates a new column with the absolute value of the residuals
#TD_trimmed.CCRM_Diff_s$Model.F.Res2 <- TD_trimmed.CCRM_Diff_s$Abs.Model.F.Res^2 #squares the absolute values of the residuals to provide the more robust estimate
# Levene.Model.F <- lm(Model.F.Res2 ~ listener, data=TD_trimmed.CCRM_Diff_s) #ANOVA of the squared residuals
# anova(Levene.Model.F) #displays the results
# 
# 
# plot_model(BestModel, type='diag') # [[4]]
...
```{r,label='ST-AgeIMEffTab',echo=FALSE,warning=FALSE, message=FALSE}

kbl(ST_AgeTab_IM, booktabs = T,escape = F, linesep = "",caption = "ST: Derived measures (DC): LMEM model examining the effect of Distractor (AMSSN, MDR\\_F, and ENG\\_F), Material (ASL/CCRM) and Age as fixed factors and random intercepts for subjects (reference levels: Distractor = AMSSN; Material = ASL). Note: only TD data following outliers trimming was included.",
    align = c("lccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c("DC ~ Distractor + Material + Age + Distractor:Age + (1 | Subjects)" = 4), align = "l",line = TRUE) %>%
  # column_spec(1, width = "8cm") %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
  column_spec(4, italic = T)
...
<!-- CCRM ONLY -->
```{r,label='ST-IMEffectDerivedCCRM', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE, eval=FALSE}
##### Age effect? ################################################################# 
...
model1 <- lmer(uRevs~CondCode*Age+(1|listener), TD_trimmed.CCRM_Diff_s, REML=FALSE)
summary(model1)
...
#        npar     AIC      BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model2    7 -122.44 -105.423 68.219  -136.44                       
# model1   10 -124.28  -99.967 72.138  -144.28 7.8367  3    0.04951 *

# best mode --> model 1: uRevs ~ CondCode * Age + (1 | listener)

tab_model(model1)
# visreg::visreg(model1)
# visreg::visreg(model1,"CondCode","Age")
...
# Response: uRevs
#                Chisq Df Pr(>Chisq)    
# CondCode     70.3777  3  3.543e-15 ***
# Age           0.6543  1    0.41856    
# CondCode:Age  8.2139  3    0.04179 * 
...
# linear model
w1 <- lm(uRevs~CondCode*Age, data = TD_trimmed.CCRM_Diff_s)
...
# Option 1:
car::leveneTest(uRevs ~ CondCode, data=TD_trimmed.CCRM_Diff_s,center=median)

# Option 2: 
DescTools::LeveneTest(lm(uRevs~ CondCode, data = TD_trimmed.CCRM_Diff_s))
```

##### Age-independent z-scores
\
As before, descriptive statistics for the listeners' z-scores for the derived scores are given in Table\ \@ref(tab:ST-DerivedTab) and plotted in Figure\ \@ref(fig:ST-DerivedPlotsz). Overall, differences between the groups were larger for IM than for the switching cost. Furthermore, the percentage of abnormal performance is generally low in both test materials (range: 0\% - 25\%), with noticeably larger proportion in the CCRM material. Only one APD child (5\%) obtained abnormal score for the ASL material, when presented with the ENG_F distractor. None of the TD children exhibited abnormal score for IM conditions. Surprisingly however, three TD children (~13\%) were significantly hampered when switching was introduced. As for the CCRM material, the proportion of abnormal score of the TD group ranged between 0\% to circa 9\% (2/23).\

```{r,label='ST-DerivedzTab', echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
ASLN_Diff_s <- ASLN_Diff %>% as.data.frame(.) %>% mutate(material = "ASL")

CCRM_Diff_s <- CCRM_Diff %>% as.data.frame(.) %>% mutate(material = "CCRM")

ST_Diff <- rbind(ASLN_Diff_s,CCRM_Diff_s)

ST_DiffzTab <- ST_Diff%>% ddply(.,~CondCode*material*Group,summarise,N=length(z_trim),median=median(z_trim,na.rm=TRUE),sd=sd(z_trim,na.rm=TRUE), min=min(z_trim,na.rm=TRUE),max=max(z_trim,na.rm=TRUE),
                          abnormal=paste0(sprintf("%0.2f",(sum(Outlier,na.rm=TRUE)/length(listener)*100)),"\\%")) %>% 
  mutate_if(is.numeric, round, 2) %>%
  arrange(., group_by = Group,material) 


ST_DiffzTab <- ST_DiffzTab[,-grep("Group",colnames(ST_DiffzTab))]
ST_DiffzTab <- ST_DiffzTab[,-grep("material",colnames(ST_DiffzTab))]

# APD first then TD
ST_DiffzTab <- cbind(ST_DiffzTab[1:9,1:ncol(ST_DiffzTab)],ST_DiffzTab[10:nrow(ST_DiffzTab),2:ncol(ST_DiffzTab)])
# ST_DiffzTab$CondCode <- c("Switching (Quiet-Alt - Quiet-NoAlt)","AMSSN (AMSSN - Quiet-Alt)",
#                           "MDR\\_F (MDR\\_F - Quiet-Alt)","ENG\\_F (ENG\\_F - Quiet-Alt)",
#                           "Switching (Quiet-Alt - Quiet-NoAlt)","AMSSN (AMSSN - Quiet-Alt)",
#                           "MDR\\_F (MDR\\_F - Q-Alt)","ENG\\_F (ENG\\_F - Quiet-Alt)","CCRM\\_F (CCRM\\_F - Quiet-Alt)")
ST_DiffzTab$CondCode <- c("Switching","AMSSN","MDR\\_F","ENG\\_F","Switching","AMSSN","MDR\\_F","ENG\\_F","CCRM\\_F")

colnames(ST_DiffzTab)[1] = "Derived measure"
# colnames(ST_DiffzTab)[2] = "Material"

# Diff = ST_DiffzTab[3] - ST_DiffzTab[9]
```

```{r, label='ST-DerivedTab', echo=FALSE}
# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf

kbl(ST_DiffzTab,booktabs = T, escape = F, linesep = "",caption = "ST: Descriptives for standardised residuals (z-scores) calculated for data measured with the ASL and CCRM speech material.",
    align = c("lcccccl"),format = "latex") %>%
  kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c(" ", "APD" = 6, "TD" = 6)) %>%
  column_spec(c(8),border_left = T) %>%
  kable_styling() %>%
  pack_rows("ASL", 1, 4) %>%
  pack_rows("CCRM", 5, 9) %>%
  add_footnote(c("abnormal: defined as the percentage of abnormal  z-score > 1.96.","Switching = Quiet-Alt - Quiet-NoAlt; AMSSN = AMSSN - Quiet-Alt","MDR_F = MDR_F - Quiet-Alt; ENG_F = ENG_F - Quiet-Alt; CCRM_F = CCRM_F - Quiet-Alt"), notation = "none") 

# %>% column_spec(1 ,width = "15em")
  # add_footnote(c("abnormal: the percentage of abnormal score, defined as z-score > 1.96.","Percentage in brackets were accounted for observations that were trimmed during the z-score calculation procedure."), notation = "none")
```

```{r, label='ST-DerivedPlotsz', fig.cap="ST: Boxplots of the listeners age-independent standardised residuals for the derived score (in DC) calculated from data measured with the ASL (A) and the CCRM speech material (B). Residuals were calculated separately for each condition and are based on a model prediction for the TD group only.", fig.align='center', fig.width=11, fig.asp=.9, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
p1 <- ggplot(ASLN_Diff, aes(x=CondCode,y=z_trim,fill=Group))+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-CutOff, ymax=CutOff),fill="lightgray", alpha=0.05)+
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
  geom_quasirandom(dodge.width=0.9,shape=1,colour="black")+
  geom_hline(yintercept=0, linetype="dashed",color = "black", size=0.5) +
  labs(y = latex2exp::TeX("\\textbf{\\overset{standardised residual (z-score)}{$\\leftarrow$ better performance}}"),x = NULL)+
  # annotate("text", label = latex2exp::TeX("$\\leftarrow$ better performance"),x=0.48, y=1, angle=90, size=5)+
  annotate(geom ="text",  x=1, y = 5.7, label = "ST-ASL", size=9, face="bold",fontface=2) +
  # scale_fill_discrete(name = "Group", labels = c("APD", "TD",bquote(bold(TD[sib]))))+
  # geom_text(aes(label = ifelse(ASLN$z_trim>CutOff,ASLN$listener,"")),
  # position = position_dodge2(width = .8,padding = 0.1))+
  scale_y_continuous(limits = c(-4,6),breaks=seq(-4,6,2))+
  scale_x_discrete(labels=c("Diff_QNoAlt" ="Switching \n (Quiet-Alt - Quiet-NoAlt)","Diff_AMSSN"="AMSSN \n (AMSSN-Alt - Quiet-Alt)",
                            "Diff_MDR"="MDR_F \n (MDR_F-Alt - Quiet-Alt)","Diff_ENG"="ENG_F \n (ENG_F-Alt - Quiet-Alt)"))+
  # scale_fill_manual(values = my_colors_3, name = "Group", labels = c("APD",bquote(bold(TD[sib])),"TD"))+
  scale_fill_manual(values = my_colors_2, name = "Group", labels = c("APD","TD"))+
  theme_bw()+
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
        axis.title.x = element_text(size=11, face="bold"),
        axis.title.y = element_text(size=11, face="bold"),
...
p2 <- ggplot(CCRM_Diff, aes(x=CondCode,y=z_trim,fill=Group))+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-CutOff, ymax=CutOff),fill="lightgray", alpha=0.05)+
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
  geom_quasirandom(dodge.width=0.9,shape=1,colour="black")+
  geom_hline(yintercept=0, linetype="dashed",color = "black", size=0.5) +
  labs(y = latex2exp::TeX("\\textbf{\\overset{standardised residual (z-score)}{$\\leftarrow$ better performance}}"),x = NULL)+
  # annotate("text", label = latex2exp::TeX("$\\leftarrow$ better performance"),x=0.48, y=1, angle=90, size=5)+
  annotate(geom ="text",  x=1.3, y = 5.7, label = "ST-CCRM", size=9, face="bold",fontface=2) +
  # scale_fill_discrete(name = "Group", labels = c("APD", "TD",bquote(bold(TD[sib]))))+
  # geom_text(aes(label = ifelse(ASLN$z_trim>CutOff,ASLN$listener,"")),
  # position = position_dodge2(width = .8,padding = 0.1))+
  scale_y_continuous(limits = c(-4,6),breaks=seq(-4,6,2))+
...
Group differences were examined using two separate LMEM models for switching and IM effects, excluding data measured with the CCRM_F distractor. Parametric methods assumptions were met for IM data, whereas the assumption of normal distribution for the switching data was rejected. Nevertheless, since nonparametric methods gave similar results, it was decided to report here only the outcomes of the parametric method. Switching effect were examined using LMEM model with Material and Group (APD/TD) as fixed factors and random intercepts for subjects (reference levels: Material = ASL; Group = APD). The final model which gave the best fit did not include Material x Group interaction. There was no significant main effect of both Material ($\chi^{2}$ = 0.00016, $Df$ = 1, $p$ = 0.990) and Group ($\chi^{2}$ = 0.380, $Df$ = 1, $p$ = 0.538), thus indicating that the switching cost was the same in both groups. Similarly, group differences across the three distractors were tested using LMEM model with Distractor, Material and Group as fixed factors and random intercepts for subjects. The final model and main effects are given in Table\ \@ref(tab:ST-zIMEffTab). There was a significant main effect of Group ($p$ \< 0.05), whereas there was no significant main effect for Distractor or Material ($p$ \> 0.05).\

```{r,label='ST-GroupEffectDerivedzIM', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE}
##### Age effect? ################################################################# 
...
ASLN_Diff_s <- ASLN_Diff %>% as.data.frame(.) %>% mutate(material = "ASL") %>%
  dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs, z_trim) %>% 
  droplevels()

CCRM_Diff_s <- CCRM_Diff %>% as.data.frame(.) %>% mutate(material = "CCRM") %>%
  filter(CondCode !="Diff_CCRM") %>%
  dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs, z_trim) %>% 
  droplevels()

ST_Diff <- rbind(ASLN_Diff_s,CCRM_Diff_s)

ST_Diff_IM <- ST_Diff %>% filter(CondCode!="Diff_QNoAlt") %>%droplevels()
ST_Diff_IM$listener <- factor(ST_Diff_IM$listener)
ST_Diff_IM$material <- factor(ST_Diff_IM$material, levels = c("ASL","CCRM"))
...
model1 <- lmer(z_trim~CondCode*material*Group+(1|listener), ST_Diff_IM, REML=FALSE)
summary(model1)

(model2<-update(model1,  ~ . - CondCode:material:Group))
anova(model1,model2)
summary(model2)

(model3<-update(model2,  ~ . - material:Group))
anova(model2,model3)
summary(model3)

(model4<-update(model3,  ~ . -  CondCode:Group))
anova(model3,model4)
summary(model4)

(model5<-update(model4,  ~ . -  CondCode:material))
anova(model4,model5)
summary(model5)

(model6<-update(model5,  ~ . -  Group))
anova(model5,model6)
summary(model6)
# model6: z_trim ~ CondCode + material + (1 | listener)
# model5: z_trim ~ CondCode + material + Group + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# model6    6 749.90 771.21 -368.95   737.90                       
# model5    7 746.81 771.68 -366.40   732.81 5.0888  1    0.02408 *

# best model --> model 5: z_trim ~ CondCode + material + Group + (1 | listener)
...
# Response: z_trim
#           Chisq Df Pr(>Chisq)  
# CondCode 0.7032  2    0.70357  
# material 0.0004  1    0.98423  
# Group    5.4022  1    0.02011 *

# Group:
(BestModel.1 <- update(BestModel,  . ~ . -Group))
Group.LMEM <- anova(BestModel,BestModel.1)
summary(BestModel.1)
# BestModel.1: z_trim ~ CondCode + material + (1 | listener)
# BestModel: z_trim ~ CondCode + material + Group + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1    6 749.90 771.21 -368.95   737.90                       
# BestModel      7 746.81 771.68 -366.40   732.81 5.0888  1    0.02408 *

#  Material
(BestModel.2 <- update(BestModel,  . ~ . -material))
Mat.LMEM <- anova(BestModel,BestModel.2)
summary(BestModel.2)
# BestModel.2: z_trim ~ CondCode + Group + (1 | listener)
# BestModel: z_trim ~ CondCode + material + Group + (1 | listener)
#             npar    AIC    BIC logLik deviance Chisq Df Pr(>Chisq)
# BestModel.2    6 744.81 766.12 -366.4   732.81                    
# BestModel      7 746.81 771.68 -366.4   732.81 4e-04  1     0.9842

# CondCode
(BestModel.3 <- update(BestModel,  . ~ . -CondCode))
Cond.LMEM <- anova(BestModel,BestModel.3)
summary(BestModel.3)
# BestModel.3: z_trim ~ material + Group + (1 | listener)
# BestModel: z_trim ~ CondCode + material + Group + (1 | listener)
#             npar    AIC    BIC  logLik deviance Chisq Df Pr(>Chisq)
# BestModel.3    5 743.51 761.27 -366.75   733.51                    
# BestModel      7 746.81 771.68 -366.40   732.81 0.702  2      0.704
#------------------------------------------------------------------------------------------------------
# Post-hoc paired comparison
# -----------------------------------------------------------------------------------------------------
...
# Group:
emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ Group,adjust="bonferroni")
postLMEM.1 <- summary(emm_s.conf,infer = c(TRUE,TRUE))$contrasts[] %>%
               data.frame() %>%
               mutate_if(is.numeric, round, 2) %>%
               mutate(CI = sprintf("%s - %s",
...
#                                     postLMEM.1$p.value))
# 
# postLMEM.1_d <- ST_Diff_IM %>% 
#     rstatix::cohens_d(z_trim ~ Group, data=., ci = TRUE) %>% as.data.frame() %>%
#     select(effsize,magnitude) %>%
#     dplyr::rename("d"=effsize)
...
# get table
# -------------------------------------------------------------------------
ST_IMzTab <- data.frame(rbind(
  c("Distractor",Cond.LMEM$Df[2],Cond.LMEM$Chisq[2],sprintf("%0.3f",Cond.LMEM$`Pr(>Chisq)`[2])),
  c("Material",Mat.LMEM$Df[2],Mat.LMEM$Chisq[2],sprintf("%0.3f",Mat.LMEM$`Pr(>Chisq)`[2])),
  c("Group",Group.LMEM$Df[2],Group.LMEM$Chisq[2],sprintf("\\textbf{%0.3f}",Group.LMEM$`Pr(>Chisq)`[2]))
  ))

ST_IMzTab[,c(2:3)]= apply(ST_IMzTab[,c(2:3)], 2, function(x) as.numeric(as.character(x)))

colnames(ST_IMzTab)[1:4] <- c("Main effects","$Df$",sprintf("$\\chi^{2}$"),"$p$")
# -------------------------------------------------------------------------
...
# linear model
w1 <- lm(z_trim~CondCode*material*Group, data = ST_Diff_IM)
...
# Option 1:
car::leveneTest(z_trim ~ material* CondCode * Group, data=ST_Diff_IM,center=median)

# Option 2: 
DescTools::LeveneTest(lm(z_trim~ material* CondCode * Group, data = ST_Diff_IM))
```

```{r,label='ST-GroupEffectDerivedzAlt', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE}
##### Age effect? ################################################################# 

# Normality:   NOT met (p=0.001943)
# Homogeneity: met

ST_Diff_Switch <- rbind(ASLN_Diff_s,CCRM_Diff_s)

ST_Diff_Switch <- ST_Diff_Switch %>% filter(CondCode=="Diff_QNoAlt") %>%droplevels()
ST_Diff_Switch$listener <- factor(ST_Diff_Switch$listener)
ST_Diff_Switch$material <- factor(ST_Diff_Switch$material, levels = c("ASL","CCRM"))

######## NONPARAMETRIC method ##########################################
ST_Altz.nparLD <- nparLD(z_trim ~ material * Group, data = ST_Diff_Switch, subject = "listener", description = TRUE, plot.CI=TRUE)

ST_Altz.nparLD <- data.frame(round(ST_Altz.nparLD$ANOVA.test,3))

colnames(ST_Altz.nparLD)[3] <- "p-value"
row.names(ST_Altz.nparLD) <- c("Group","Material","Group:Material")
...
model1 <- lmer(z_trim~material*Group+(1|listener), ST_Diff_Switch, REML=FALSE)
summary(model1)

(model2<-update(model1,  ~ . - material:Group))
anova(model1,model2)
summary(model2)
...
# Response: z_trim
#           Chisq Df Pr(>Chisq)
# material 0.0002  1     0.9900
# Group    0.3818  1     0.5366
...
# get table
# -------------------------------------------------------------------------
ST_AltzTab <- data.frame(rbind(
  c("Material",Mat.LMEM$Df[2],Mat.LMEM$Chisq[2],sprintf("%0.3f",Mat.LMEM$`Pr(>Chisq)`[2])),
  c("Group",Group.LMEM$Df[2],Group.LMEM$Chisq[2],sprintf("%0.3f",Group.LMEM$`Pr(>Chisq)`[2]))
  ))

ST_AltzTab[,c(2:3)]= apply(ST_AltzTab[,c(2:3)], 2, function(x) as.numeric(as.character(x)))

colnames(ST_AltzTab)[1:4] <- c("Main effects","Df",sprintf("$\\chi^{2}$"),"p")
# -------------------------------------------------------------------------
...
# linear model
w1 <- lm(z_trim~material*Group, data = ST_Diff_Switch)
...
```{r,label='ST-zIMEffTab',echo=FALSE,warning=FALSE, message=FALSE}

kbl(ST_IMzTab, booktabs = T,escape = F, linesep = "",caption = "ST: Derived measures (z-score): LMEM model with Distractor (AMSSN, MDR\\_F, and ENG\\_F), Material (ASL/CCRM) and Group (APD/TD) as fixed factors and random intercepts for subjects. Reference levels: Distractor = AMSSN; Material = ASL; Group = APD).",
    align = c("lccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c("z ~ Distractor + Material + Group + (1 | Subjects)" = 4), align = "l",line = TRUE) %>%
  # column_spec(1, width = "8cm") %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
  column_spec(4, italic = T)
...
A separate LMEM model for the full CCRM data, including the CCRM_F distractor, gave similar results, with a significant main effect of Group ($\chi^{2}$ = 5.089, $Df$ = 1, $p$ = 0.024), whereas there as no signifiacnt main effect of Distractor ($\chi^{2}$ = 0.990, $Df$ = 3, $p$ = 0.804).\

<!-- CCRM Only -->
```{r,label='ST-GroupEffectDerivedzIMCCRM', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE}
##### Age effect? ################################################################# 
...
CCRM_Diff_s <- CCRM_Diff %>% as.data.frame(.) %>% mutate(material = "CCRM") %>%
  filter(CondCode !="Diff_QNoAlt") %>%
  dplyr::select(., listener, Age,Group,Group2, material, CondCode, uRevs, z_trim) %>% 
  droplevels()


CCRM_Diff_s$listener <- factor(CCRM_Diff_s$listener)
...
model1 <- lmer(z_trim~CondCode*Group+(1|listener), CCRM_Diff_s, REML=FALSE)
summary(model1)
...
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# model3    6 530.63 549.51 -259.31   518.63                       
# model2    7 528.44 550.48 -257.22   514.44 4.1835  1    0.04082 *


# best model --> model z_trim ~ CondCode + Group + (1 | listener)


tab_model(model2)
# visreg::visreg(model2)
# visreg::visreg(model2,"Group","CondCode")
...
# Response: z_trim
#           Chisq Df Pr(>Chisq)  
# CondCode 0.9940  3    0.80271  
# Group    4.3938  1    0.03607 *
...
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1    6 530.63 549.51 -259.31   518.63                       
# BestModel      7 528.44 550.48 -257.22   514.44 4.1835  1    0.04082 *
...
# BestModel: z_trim ~ CondCode + Group + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# BestModel.2    4 523.43 536.02 -257.72   515.43                     
# BestModel      7 528.44 550.48 -257.22   514.44 0.9902  3     0.8036
...
# -------------------------------------------------------------------------
# ST_IMzTab <- data.frame(rbind(
#   c("Condition",Cond.LMEM$Df[2],Cond.LMEM$Chisq[2],sprintf("%0.3f",Cond.LMEM$`Pr(>Chisq)`[2])),
#   c("Material",Mat.LMEM$Df[2],Mat.LMEM$Chisq[2],sprintf("%0.3f",Mat.LMEM$`Pr(>Chisq)`[2])),
#   c("Group",Group.LMEM$Df[2],Group.LMEM$Chisq[2],sprintf("\\textbf{%0.3f}",Group.LMEM$`Pr(>Chisq)`[2]))
#   ))
# 
...
# linear model
w1 <- lm(z_trim~CondCode*Group, data = CCRM_Diff_s)
...
# Option 1:
car::leveneTest(z_trim ~ CondCode * Group, data=CCRM_Diff_s,center=median)

# Option 2: 
DescTools::LeveneTest(lm(z_trim~ CondCode * Group, data = CCRM_Diff_s))
```
...
<!-- ---------------------------------------------------------------------------------------------------------------- -->
<!-- with sibs -->
```{r,label='ST-GroupEffectDerivedzIMold', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE}
##### Age effect? ################################################################# 
...
```{r,label='ST-GroupEffectDerivedzAltold', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE}
##### Age effect? ################################################################# 
...
<!-- CCRM Only -->
```{r,label='ST-GroupEffectDerivedzIMCCRMold', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE}
##### Age effect? ################################################################# 
...
<!-- get ggpairs plots for appendix: -->
...
for ( col in 1:ncol(ASLN_w2)){
  ...
  colnames(ASLN_w2)[col] <-  sub("_ASLN", "", colnames(ASLN_w2)[col])
}

ggpairs(ASLN_w2[c(3,10:14,2)],
        lower = list(continuous = "smooth",aes(fill = ASLN_w2$Group)),
        diag = list(continuous = wrap("densityDiag", alpha=0.7), aes(fill = ASLN_w2$Group)),
...
for ( col in 1:ncol(CCRM_w2)){
  ...
  colnames(CCRM_w2)[col] <-  sub("_CCRM", "", colnames(CCRM_w2)[col])
}

ggpairs(CCRM_w2[c(3,11:16,2)],
        lower = list(continuous = "smooth", aes(fill = CCRM_w2$Group)),
        diag = list(continuous = wrap("densityDiag", alpha=0.7), aes(fill = CCRM_w2$Group)),
...
# get TD trimmed data only for SSN, S0N0 & S0N90 (WITHOUT SRM)
TD_trimmed.LiSNS <- rbind(TD_trimmed_1,TD_trimmed_2,TD_trimmed_3)
TD_trimmed.LiSNS_SRM <- TD_trimmed_4
...
# model1: uRevs ~ CondCode * Age * APDsibling + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model2   12 268.56 295.02 -122.28   244.56                     
# model1   14 270.62 301.49 -121.31   242.62 1.9358  2     0.3799
...
# model2:     CondCode:APDsibling + Age:APDsibling
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model3   11 266.58 290.83 -122.29   244.58                     
# model2   12 268.56 295.02 -122.28   244.56 0.0177  1     0.8941
...
# model3:     CondCode:APDsibling
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model4    9 263.63 283.47 -122.81   245.63                     
# model3   11 266.58 290.83 -122.29   244.58 1.0507  2     0.5913
...
# model4: uRevs ~ CondCode + Age + APDsibling + (1 | listener) + CondCode:Age
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model5    7 261.09 276.52 -123.54   247.09                     
# model4    9 263.63 283.47 -122.81   245.63 1.4581  2     0.4824
...
# model5: uRevs ~ CondCode + Age + APDsibling + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model6    6 259.12 272.35 -123.56   247.12                     
# model5    7 261.09 276.52 -123.54   247.09 0.0344  1     0.8528
...
# model6: uRevs ~ CondCode + Age + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# model7    5 270.49 281.51 -130.24   260.49                         
# model6    6 259.12 272.35 -123.56   247.12 13.364  1  0.0002565 ***
...
LiSNS_AgeTab <- data.frame(rbind(
  c("Condition",Cond.LMEM$Df[2],Cond.LMEM$Chisq[2],ifelse(Cond.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(Cond.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",Cond.LMEM$`Pr(>Chisq)`[2]),
                                    Cond.LMEM$`Pr(>Chisq)`[2]))),
  c("Age",Age.LMEM$Df[2],Age.LMEM$Chisq[2],ifelse(Age.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(Age.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",Age.LMEM$`Pr(>Chisq)`[2]),
                                    Age.LMEM$`Pr(>Chisq)`[2])))))
...
kbl(LiSNS_AgeTab, booktabs = T,escape = F, linesep = "",caption = "LiSNS-UK: Age effect: LMEM model for SRT with Condition (SSN, S0N0, \\& S0N90) and Age as fixed factors and random intercepts for subjects (reference level: SSN). Note: only data measured with the control group (TD) following outliers trimming was included.",
    align = c("lccc"),format = "latex",digits = 3) %>% 
...
```{r,label='LiSNS-AgeStats-SRM', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide'}
# Test for age effect -----------------------------------------------------------------------------------

# Normality: met
# Homogeneity: NOT met? (p=0.02954*)

# SRM data only!

TD_trimmed.LiSNS_SRM <- TD_trimmed_4

TD_trimmed.LiSNS_SRM <- data.frame(TD_trimmed.LiSNS_SRM)
TD_trimmed.LiSNS_SRM$listener <- factor(TD_trimmed.LiSNS_SRM$listener)
TD_trimmed.LiSNS_SRM$APDsibling <- factor(TD_trimmed.LiSNS_SRM$APDsibling)


######## Nonparametric model ##########################################

# set.seed(10)
# WRS2::t1waybt(uRevs ~ APDsibling, data = TD_trimmed.LiSNS_SRM,tr = 0.1,nboot = 2000)

######## Parametric model ##########################################
## Find Best fit 

model1.lm <- lm(uRevs ~ Age*APDsibling, data = TD_trimmed.LiSNS_SRM)
summary(model1.lm)
anova(model1.lm)
# LiSNS_AgeTab2 <- apaTables::apa.aov.table(model1.lm,conf.level = 0.95)

# model1.aov <- aov(uRevs ~ Age*APDsibling, data = TD_trimmed.LiSNS_SRM)
# summary(model1.aov)
# anova(model1.aov)
# # 
# tab_model(model1.lm, show.df = TRUE)
...
# ---------------- uRevs ------------------------------------------------------------
# linear model
w1 <- lm(uRevs~ Age * Group2, data = TD_trimmed.LiSNS_SRM)
...
# Option 1:
# car::leveneTest(uRevs ~ Group2, data=TD_trimmed.LiSNS_SRM,center=median)

# leveneTest(residuals(BestModel) ~ Group2 , data = TD_trimmed.LiSNS_SRM)

# Option 2: 
# DescTools::LeveneTest(lm(uRevs~ Group2, data = TD_trimmed.LiSNS_SRM))

# Option 3:
# per condition
# VarTest <- TD_trimmed.LiSNS_SRM %>%
#   rstatix::group_by(Group) %>%
#   levene_test(uRevs ~ Age)

# Boxplot(uRevs~Age*Group2, data=TD_trimmed.LiSNS_SRM)
```

#### Age-independent z-scores {.unnumbered}
Boxplots of the listeners age-independent standardised residual z-scores (black circles) collapsed across the different test conditions are shown in Figure \@ref(fig:LiSNS-zfig), separately for the APD group (red) **and the two TD groups (shades of blue)**. The z-scores were calculated in the same way as for ST. Again, the dashed line indicates the theoretical TD group mean of zero, and the grey area indicates the lower and upper limit of the normal population (TD mean $\pm$ 1.96). Descriptive statistics collapsed by group and test conditions are given in Table \@ref(tab:LiSNS-ztab). Overall, when compared with the control group, the APD children exhibited poorer performance across all three test conditions (i.e., higher z-scores) as well as for the derived SRM measure (i.e., lower z-scores).\
...
<!-- Residuals were calculated separately for each condition and are based on a model prediction for the TD group only. The grey area represents scores in the 'normal' region, where about 95\\% of the normal population is expected to lay within. To each side of the grey area, deviance scores were defined as z-scores below or above 1.96, which represents the bottom 5\\% in the TD group. The dashed line represents the theoretical TD group mean (z = 0). Individual scores are indicated by circles. -->

```{r, label='LiSNS-zfig', fig.cap="LiSNS-UK: Boxplots of the listeners age-independent standardised residuals (open circles) for data measured with LiSNS-UK task (A) and the derived measure SRM (B). Residuals were calculated separately for each condition and are based on a model prediction for the TD group only as plotted for the ST data.", fig.align='center', fig.width=12, fig.asp=0.5, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}

LiSNS_NoSRM$Group2 <- factor(LiSNS_NoSRM$Group2, levels = c("APD","TD_1","TD_0"))
...
LiSNS_NoSRM$Group2 <- factor(LiSNS_NoSRM$Group2, levels = c("APD","TD_0","TD_1"))
```
...
```{r,label='LiSNS-zLMEM', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide'}
##### Test z_trim #####
...
model1 <- lmer(z_trim~CondCode * Group + (1|listener), LiSNS_NoSRM , REML=FALSE)
summary(model1)
# tab_model(model1)
...
# BestModel (model10):
BestModel <- lmer(z_trim ~ CondCode + Group + (1|listener), data=LiSNS_NoSRM, REML=FALSE)
tab_model(BestModel, 
          show.se =TRUE,
          show.stat = TRUE,
          show.icc = FALSE,
...
# linear model:
w1 <- lm(z_trim ~ CondCode + Group, data = LiSNS_NoSRM)
...
# Option 1: Is the lm model fully covered here?
leveneTest(residuals(w1) ~ CondCode * Group, data = LiSNS_NoSRM)
# -> homogeneity of the full model is met

leveneTest(residuals(BestModel) ~ CondCode * Group, data = LiSNS_NoSRM)
```

```{r,label='LiSNS-zLMEM-SRM', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}
##### Test z_trim SRM ONLY #####

# Normality: met
# homoscedasticity: NOT met (p=0.01352 *)
...
set.seed(10)
WRS2::t1waybt(z_trim ~ Group, data = LiSNS_SRM,tr = 0.1,nboot = 2000)

# -----------------------------------------------------------------------------------
# Parametric method
# -----------------------------------------------------------------------------------

model1 <- aov(z_trim ~ Group, data = LiSNS_SRM) %>% summary(.)

model1 <- lm(z_trim ~ Group, data=LiSNS_SRM)
anova(model1)
summary(model1)
tab_model(model1)
...
# linear model:
w1 <- lm(z_trim ~ Group, data = LiSNS_SRM)
...
# ggpairs(LiSNS_corr[c(4,5:7,2)],
        
ggpairs(LiSNS_corr[c(4,5:8,2)],
        lower = list(continuous = "smooth" , aes(fill = LiSNS_corr$Group)),
        diag = list(continuous = wrap("densityDiag", alpha=0.7), aes(fill = LiSNS_corr$Group)),
...
# ggpairs(LiSNS_corr[c(4,9:11,2)],
        
ggpairs(LiSNS_corr[c(4,9:12,2)],
        lower = list(continuous = "smooth", aes(fill = LiSNS_corr$Group)),
        diag = list(continuous = wrap("densityDiag", alpha=0.7), aes(fill = LiSNS_corr$Group)),
...
TD_trimmed.ENVASA_NoTotal <- TD_trimmed.ENVASA %>% 
  filter(CondCode!="Total") %>% 
  droplevels() %>% 
  data.frame()

TD_trimmed.ENVASA_NoTotal$listener <- factor(TD_trimmed.ENVASA_NoTotal$listener)
...
model1 <- lmer(PC~CondCode*Age*Group2+(1|listener), data=TD_trimmed.ENVASA_NoTotal, REML=FALSE)
summary(model1)

(model2<-update(model1,  ~ . - CondCode:Age:Group2))
anova(model1,model2)
summary(model2)
# model2: PC ~ CondCode + Age + Group2 + (1 | listener) + CondCode:Age + 
# model2:     CondCode:Group2 + Age:Group2
# model1: PC ~ CondCode * Age * Group2 + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model2    9 269.17 284.14 -125.58   251.17                     
# model1   10 270.44 287.08 -125.22   250.44 0.7268  1     0.3939

(model3<-update(model2,  ~ . - Age:Group2))
anova(model2,model3)
summary(model3)
# model3: PC ~ CondCode + Age + Group2 + (1 | listener) + CondCode:Age + 
# model3:     CondCode:Group2
# model2: PC ~ CondCode + Age + Group2 + (1 | listener) + CondCode:Age + 
# model2:     CondCode:Group2 + Age:Group2
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model3    8 269.75 283.06 -126.88   253.75                     
# model2    9 269.17 284.14 -125.58   251.17 2.5842  1     0.1079

(model4<-update(model3,  ~ . - CondCode:Group2))
anova(model3,model4)
summary(model4)
# model4: PC ~ CondCode + Age + Group2 + (1 | listener) + CondCode:Age
# model3: PC ~ CondCode + Age + Group2 + (1 | listener) + CondCode:Age + 
# model3:     CondCode:Group2
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model4    7 268.84 280.49 -127.42   254.84                     
# model3    8 269.75 283.06 -126.88   253.75 1.0915  1     0.2961

(model5<-update(model4,  ~ . - CondCode:Age))
anova(model4,model5)
summary(model5)
# model5: PC ~ CondCode + Age + Group2 + (1 | listener)
# model4: PC ~ CondCode + Age + Group2 + (1 | listener) + CondCode:Age
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model5    6 267.05 277.03 -127.53   255.05                     
# model4    7 268.84 280.49 -127.42   254.84 0.2092  1     0.6474
...
# model5: PC ~ CondCode + Age + Group2 + (1 | listener)
#        npar    AIC    BIC  logLik deviance Chisq Df Pr(>Chisq)  
# model6    5 269.30 277.62 -129.65   259.30                      
# model5    6 267.05 277.03 -127.53   255.05 4.251  1    0.03923 *


# best mode --> model 5:  PC ~ CondCode + Age + Group2 + (1 | listener)
...
# Response: PC
#            Chisq Df Pr(>Chisq)    
# CondCode 15.2389  1  9.473e-05 ***
# Age      28.6698  1  8.583e-08 ***
# Group2    4.7215  1    0.02979 *  
...
# BestModel: PC ~ CondCode + Age + Group2 + (1 | listener)
#             npar    AIC    BIC  logLik deviance Chisq Df Pr(>Chisq)  
# BestModel.1    5 269.30 277.62 -129.65   259.30                      
# BestModel      6 267.05 277.03 -127.53   255.05 4.251  1    0.03923 *

(BestModel.2 <- update(BestModel,  . ~ . -Age))
Age.LMEM <- anova(BestModel,BestModel.2)
summary(BestModel.2)
# BestModel.2: PC ~ CondCode + Group2 + (1 | listener)
# BestModel: PC ~ CondCode + Age + Group2 + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# BestModel.2    5 282.86 291.17 -136.43   272.86                         
# BestModel      6 267.05 277.03 -127.53   255.05 17.802  1  2.451e-05 ***

(BestModel.3 <- update(BestModel,  . ~ . -CondCode))
CondCode.LMEM <-anova(BestModel,BestModel.3)
summary(BestModel.3)
# BestModel.3: PC ~ Age + Group2 + (1 | listener)
# BestModel: PC ~ CondCode + Age + Group2 + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# BestModel.3    5 276.34 284.65 -133.17   266.34                         
# BestModel      6 267.05 277.03 -127.53   255.05 11.285  1  0.0007814 ***
...
comps <- emmeans::contrast(ref1,alpha=0.05,method="pairwise",adjust=NULL) #,adjust=NULL) adjust="bonferroni"
summary(comps,infer = c(TRUE,TRUE)) %>% 
               data.frame() %>% 
               mutate_if(is.numeric, round, 2) %>%
               mutate(CI = sprintf("%s - %s",
                      round(lower.CL,2),
...
effsize::cohen.d(TD_trimmed.ENVASA_NoTotal$PC[which(TD_trimmed.ENVASA_NoTotal$CondCode=="Total_s")], 
                 TD_trimmed.ENVASA_NoTotal$PC[which(TD_trimmed.ENVASA_NoTotal$CondCode=="Total_d")])
...
effsize::cohen.d(TD_trimmed.ENVASA_NoTotal$PC[which(TD_trimmed.ENVASA_NoTotal$Group2=="TD_0")], 
                 TD_trimmed.ENVASA_NoTotal$PC[which(TD_trimmed.ENVASA_NoTotal$Group2=="TD_1")])

# -------------------------------------------------------------------------
# get table
# -------------------------------------------------------------------------
ENVASA_AgeTab <- data.frame(rbind(
  c("Background",CondCode.LMEM$Df[2],CondCode.LMEM$Chisq[2],ifelse(CondCode.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(CondCode.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",CondCode.LMEM$`Pr(>Chisq)`[2]),
                                    CondCode.LMEM$`Pr(>Chisq)`[2]))),
  c("Age",Age.LMEM$Df[2],Age.LMEM$Chisq[2],ifelse(Age.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(Age.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",Age.LMEM$`Pr(>Chisq)`[2]),
                                    Age.LMEM$`Pr(>Chisq)`[2]))),
  c("Sibling",APDsib.LMEM$Df[2],APDsib.LMEM$Chisq[2],ifelse(APDsib.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(APDsib.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",APDsib.LMEM$`Pr(>Chisq)`[2]),
                                    APDsib.LMEM$`Pr(>Chisq)`[2])))))
...
w1 <- lm(PC~ CondCode*Age*Group2, data = TD_trimmed.ENVASA_NoTotal)

#### normality (shapiro-wilk) ###########
# linearity is met if p>.05
# Option 1:
shapiro.test(residuals(w1))
# p-value = 0.6048 --> normaly distr.

#### Homogeneity of variance (Levene's test) ###########
# homogeneity is met if p>.05

# Option 1: Is the lmer model fully covered here?
leveneTest(residuals(w1) ~ CondCode*Group2,data = TD_trimmed.ENVASA_NoTotal)
# p=0.5256 --> homogeneity is met!

####### plot ###########

# ggplot(TD_trimmed.ENVASA_NoTotal, aes(x=CondCode,y=PC,fill=Group2))+ 
#   geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
#   geom_quasirandom(dodge.width=0.9,shape=1,colour="blue")+
#   theme_bw()+
#   theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
...
# ddply(TD_trimmed.ENVASA_NoTotal,~CondCode*Group2,summarise,median=median(PC,na.rm=TRUE),mean=mean(PC,na.rm=TRUE),sd=sd(PC,na.rm=TRUE), min=min(PC,na.rm=TRUE),max=max(PC,na.rm=TRUE)) %>% 
#   mutate_if(is.numeric, round, 2) 
#   CondCode Group2 median  mean    sd   min   max
# 1  Total_s   TD_0  82.61 82.07  8.30 65.22 95.65
# 2  Total_s   TD_1  82.61 81.68  8.22 67.39 93.48
# 3  Total_d   TD_0  80.43 76.59 11.85 50.00 89.13
# 4  Total_d   TD_1  69.57 72.67  7.19 65.22 84.78
```

```{r, label='ENVASA-AgeLMEM-Total', eval=TRUE, include=FALSE, warning=FALSE}

# Normality: met
# Homogeneity: met

TD_trimmed.ENVASA_Total <- TD_trimmed.ENVASA %>% 
  filter(CondCode=="Total") %>% 
  droplevels() %>% 
  data.frame()

TD_trimmed.ENVASA_Total$listener <- factor(TD_trimmed.ENVASA_Total$listener)

######## Parametric model ##########################################

model1.lm <- lm(PC ~ Age*Group2, data = TD_trimmed.ENVASA_Total)
summary(model1.lm)
anova(model1.lm)
# apaTables::apa.aov.table(model1.lm,conf.level = 0.95)

# model1.aov <- aov(PC ~ Age*Group2, data = TD_trimmed.ENVASA_Total)
# aov(PC~Age*Group2, data = TD_trimmed.ENVASA_Total) %>% summary(.)
...
effsize::cohen.d(TD_trimmed.ENVASA_Total$PC[which(TD_trimmed.ENVASA_Total$Group2=="TD_0")], 
                 TD_trimmed.ENVASA_Total$PC[which(TD_trimmed.ENVASA_Total$Group2=="TD_1")])
# d estimate: 0.1965642 (negligible)
...
w1 <- lm(PC~ Age*Group2, data = TD_trimmed.ENVASA_Total)
...
# Option 1: Is the lmer model fully covered here?
leveneTest(residuals(w1) ~ Group2,data = TD_trimmed.ENVASA_Total)
# p=0.5256 --> homogeneity is met!


# ddply(TD_trimmed.ENVASA_Total,~Group2,summarise,median=median(PC,na.rm=TRUE),mean=mean(PC,na.rm=TRUE),sd=sd(PC,na.rm=TRUE), min=min(PC,na.rm=TRUE),max=max(PC,na.rm=TRUE)) %>%
#   mutate_if(is.numeric, round, 2)

#   Group2 median  mean   sd   min   max
# 1   TD_0  81.52 78.85 9.47 57.61 92.39
# 2   TD_1  76.09 77.17 6.15 67.39 85.87
```
...
ENVASA_Total$Group2 <- factor(ENVASA_Total$Group2, levels = c("APD","TD_1","TD_0"))

ggplot(ENVASA_Total, aes(x=CondCode,y=z_trim,fill=Group2))+ 
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-CutOff, ymax=CutOff),fill="lightgray", alpha=0.05)+
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+ 
  geom_quasirandom(dodge.width=0.8,shape=1,colour="black")+
  geom_hline(yintercept=0, linetype="dashed",color = "black", size=0.5) +
...
ENVASA_Total$Group2 <- factor(ENVASA_Total$Group2, levels = c("APD","TD_0","TD_1"))
```

```{r, label='ENVASA-zAnalysis', include=FALSE, warning=FALSE}

# Normality: NOT met (p=0.00134)
# Homogeneity: met

ENVASA_Total_NoTotal <- ENVASA_Total %>% 
  filter(CondCode!="Total") %>% 
  droplevels() %>% 
  data.frame()

# Test group difference (z-scores) -------------------------------------------

####################### Nonparametric test ###################################      

# OPTION 1: nparLD
ENVASA.nparLD_NoTotal <- nparLD(z_trim ~ CondCode * Group2, data = ENVASA_Total_NoTotal, subject = "listener", description = TRUE, plot.CI=TRUE)

ENVASA.nparLD_NoTotal <- data.frame(round(ENVASA.nparLD_NoTotal$ANOVA.test,3))
#                 Statistic    df p.value
# Group2              3.280 1.950   0.039
# CondCode            1.047 1.000   0.306
# Group2:CondCode     1.859 1.589   0.164

# Post hoc for Group2:
ENVASA_NoTotal_z_Wilcox <- data.frame(rstatix::wilcox_test(z_trim ~ Group2, data = ENVASA_Total_NoTotal,detailed=TRUE)) %>% mutate_if(is.numeric, round, 2) %>%
    mutate( "95\\%-CI" = sprintf("%s - %s",conf.low,conf.high),
          p = ifelse(p.adj<.001,"\\textbf{< 0.001}", ifelse(p.adj<.05, sprintf("\\textbf{%.02f}",p.adj),p.adj)),
          contrast = c("APD - TD", "APD - TD$_{sib}$", "TD - TD$_{sib}$")) %>% select(contrast,n1,n2,estimate,`95\\%-CI`,p)
  # mutate_if(is.numeric, round, 2) %>%
  # mutate( "95\\%-CI" = sprintf("%s - %s",conf.low,conf.high)) %>% 
  # select(estimate, n1,n2,`95\\%-CI`,p.adj) %>% 
  # rename("p"=p.adj)

ENVASA_NoTotal_z_effsize <- as.data.frame(rstatix::wilcox_effsize(z_trim ~ Group2, data = ENVASA_Total_NoTotal)) %>% 
  dplyr::rename("r"=effsize) %>% 
  select(r,magnitude)  %>% 
  mutate_if(is.numeric, round, 2)

ENVASA_NoTotal_z.postT <- cbind(ENVASA_NoTotal_z_Wilcox,ENVASA_NoTotal_z_effsize)

# ----------------------------------------------------------------------------

# OPTION 2: npIntFactRep
# ENVASA_Total_w_NoTotal <- ENVASA_Total_w[,c(1:2, 4:5)] %>% data.frame()
# colnames(ENVASA_Total_w_NoTotal)[1] = "subj"
# colnames(ENVASA_Total_w_NoTotal)[2] = "group"
# npIntFactRep.ENVASA_NoTotal <- npIntFactRep::npIntFactRep(ENVASA_Total_w_NoTotal)
# print(npIntFactRep.ENVASA_NoTotal)

# $RegularRanks
# $RegularRanks$ANOVA
#      Effect DFn DFd           F          p p<.05          ges
# 2     group   1  35         Inf 0.00000000     * 4.660889e-34
# 3       rep   1  35 0.004394357 0.94752421       1.255373e-04
# 4 group:rep   1  35 2.985126167 0.09284833       7.858671e-02
...
## Find Best fit 
# start with a saturated model including APDsibling, due to the significant difference in the raw scores

model1 <- lmer(z_trim~CondCode*Group2+(1|listener), data=ENVASA_Total_NoTotal, REML=FALSE)
summary(model1)

(model2<-update(model1,  ~ . - CondCode:Group2))
anova(model1,model2)
summary(model2)
# model2: z_trim ~ CondCode + Group2 + (1 | listener)
# model1: z_trim ~ CondCode * Group2 + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)
# model2    6 273.74 287.56 -130.87   261.74                     
# model1    8 273.15 291.58 -128.58   257.15 4.5871  2     0.1009

(model3<-update(model2,  ~ . - Group2))
anova(model2,model3)
summary(model3)
# model3: z_trim ~ CondCode + (1 | listener)
# model2: z_trim ~ CondCode + Group2 + (1 | listener)
#        npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# model3    4 276.38 285.60 -134.19   268.38                       
# model2    6 273.74 287.56 -130.87   261.74 6.6396  2    0.03616 *

# best mode --> model 2:  z_trim ~ CondCode + Group2 + (1 | listener)
...
# Response: z_trim
#           Chisq Df Pr(>Chisq)  
# CondCode 4.5534  1    0.03285 *
# Group2   7.2727  2    0.02635 *
...
summary(BestModel.1)
# BestModel.1: z_trim ~ CondCode + (1 | listener)
# BestModel: z_trim ~ CondCode + Group2 + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1    4 276.38 285.60 -134.19   268.38                       
# BestModel      6 273.74 287.56 -130.87   261.74 6.6396  2    0.03616 *

(BestModel.2 <- update(BestModel,  . ~ . -CondCode))
CondCode.LMEM <- anova(BestModel,BestModel.2)
summary(BestModel.2)
# BestModel.2: z_trim ~ Group2 + (1 | listener)
# BestModel: z_trim ~ CondCode + Group2 + (1 | listener)
#             npar    AIC    BIC  logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.2    5 276.03 287.55 -133.02   266.03                       
# BestModel      6 273.74 287.56 -130.87   261.74 4.2942  1    0.03824 *
...
#      contrast estimate   SE    df t.ratio p.value            CI
# 1  APD - TD_0    -1.35 0.52 40.26   -2.58    0.04 -2.66 - -0.04
# 2  APD - TD_1    -0.64 0.64 40.26   -1.00    0.98  -2.23 - 0.96
# 3 TD_0 - TD_1     0.72 0.67 40.26    1.08    0.86  -0.95 - 2.38
...
#            contrast estimate   SE    df t.ratio p.value            CI
# 1 Total_s - Total_d    -0.52 0.25 38.03    -2.1    0.04 -1.02 - -0.02

# -------------------------------------------------------------------------
# get table
# -------------------------------------------------------------------------
ENVASA_zTab <- data.frame(rbind(
  c("Background",CondCode.LMEM$Df[2],CondCode.LMEM$Chisq[2],ifelse(CondCode.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(CondCode.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",CondCode.LMEM$`Pr(>Chisq)`[2]),
                                    CondCode.LMEM$`Pr(>Chisq)`[2]))),
  c("Group",Group2.LMEM$Df[2],Group2.LMEM$Chisq[2],ifelse(Group2.LMEM$`Pr(>Chisq)`[2] <.001,"\\textbf{< 0.001}", 
                             ifelse(Group2.LMEM$`Pr(>Chisq)`[2] <.05, sprintf("\\textbf{%.02f}",Group2.LMEM$`Pr(>Chisq)`[2]),
                                    Group2.LMEM$`Pr(>Chisq)`[2])))))

ENVASA_zTab[,c(2:3)]= apply(ENVASA_zTab[,c(2:3)], 2, function(x) as.numeric(as.character(x)))

colnames(ENVASA_zTab)[1:4] <- c("Main effects","Df",sprintf("$\\chi^{2}$"),"p")
# -------------------------------------------------------------------------
...
w1 <- lm(z_trim~ CondCode+Group2, data = ENVASA_Total_NoTotal)

#### normality (shapiro-wilk) ###########
# linearity is met if p>.05
# Option 1:
shapiro.test(residuals(w1))
# p-value = 0.00134 --> NOT normaly distr.

#### Homogeneity of variance (Levene's test) ###########
# homogeneity is met if p>.05

# Option 1: Is the lmer model fully covered here?
leveneTest(residuals(w1) ~ CondCode*Group2,data = ENVASA_Total_NoTotal)
# p=0.135 --> homogeneity is met!

####### plot ###########

# ggplot(ENVASA_Total_NoTotal, aes(x=CondCode,y=z_trim,fill=Group2))+
#   geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA)+
#   geom_quasirandom(dodge.width=0.9,shape=1,colour="blue")+
#   theme_bw()+
#   theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
...
# ddply(ENVASA_Total_NoTotal,~CondCode*Group2,summarise,median=median(z_trim,na.rm=TRUE),mean=mean(z_trim,na.rm=TRUE),sd=sd(z_trim,na.rm=TRUE), min=min(z_trim,na.rm=TRUE),max=max(z_trim,na.rm=TRUE)) %>%
#   mutate_if(is.numeric, round, 2)

#   CondCode Group2 median  mean   sd   min  max
# 1  Total_s    APD  -0.97 -1.66 2.11 -5.56 1.93
# 2  Total_s   TD_0   0.09  0.00 1.01 -2.37 1.28
# 3  Total_s   TD_1  -0.97 -0.35 1.26 -1.44 1.52
# 4  Total_d    APD   0.29 -0.67 2.07 -5.34 1.54
# 5  Total_d   TD_0   0.57  0.38 0.74 -1.48 1.22
# 6  Total_d   TD_1  -0.65 -0.71 0.92 -1.90 0.60
```
...
kbl(ENVASA_tab, booktabs = T,escape = F, linesep = "",caption = "ENVASA: Descriptive and statistics of the listeners age-independent standard residuals (z-scores) split by groups and test measures.",
  ...
  kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c(" " = 1, "APD" = 6, "TD" = 6)) %>%
  column_spec(c(8),border_left = T) 
```

```{r,label='ENVASA-zTabPostHoc',echo=FALSE,warning=FALSE,message=FALSE}

# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf
kbl(ENVASA_NoTotal_z.postT, booktabs = T,escape = F, linesep = "",caption = "ENVASA: Post-hoc paired comparison tests with Bonferroni correction (Wilcoxon rank-sum test) for Group differences in z-scores.",
    align = c("lccccccc"),format = "latex",digits = 2) %>% 
  kable_styling(latex_options = c("scale_down")) %>%
  column_spec(6, italic = T) 
```
...
```{r, label='CELF', fig.cap="CELF-RS: Boxplots for CELF-5 UK Recall Sentences subtest scaled scores by groups. The grey area represents scores in the 'normal' region ($\\pm$\ 1\ SD) and the dashed line represents the average score within the normal population.", fig.align='center', fig.width=3, fig.asp=1, out.width='50%', ,echo=FALSE,warning=FALSE, message=FALSE}

d_RS$Group2 <- factor(d_RS$Group2, levels = c("APD","TD_1","TD_0"))
...
ggplot(d_RS, aes(x=Group2,y=ScaledScore,fill=Group2))+ 
  ...
  # scale_fill_discrete(name = "Group", labels = c("APD", "TD",bquote(bold(TD[sib]))))+
  geom_quasirandom(dodge.width=0.8,colour="black", shape=1,show.legend = FALSE)+
  scale_y_continuous(limits = c(1,19),breaks=seq(1,19,2))+
  scale_x_discrete(labels=c(expression(bold("APD")),bquote(bold(TD[sib])),expression(bold("TD"))))+
...
d_RS$Group2 <- factor(d_RS$Group2, levels = c("APD","TD_0","TD_1"))
```
...
d_CCC2_L$Group2 <- factor(d_CCC2_L$Group2, levels = c("APD","TD_1","TD_0"))
...
t2 <- ggplot(SumScores, aes(x=GCC,y=SPC,color=Group2))+
  geom_vline(xintercept=55, color="black",size=1)+
  geom_segment(aes(x=0,xend=55,y=0,yend=0), color="black",size=1)+
  geom_point(size=2,alpha=1,aes(color=Group2),show.legend = FALSE) +
...
d_CCC2_L$Group2 <- factor(d_CCC2_L$Group2, levels = c("APD","TD_0","TD_1"))


# with different shapes for TDs:
...
CCC2_tab$Measure <- str_replace_all(CCC2_tab$Measure,"\\."," ")
...
```{r, label='ECLIPS-plot', fig.cap="ECLiPS parental report scaled scores split by groups and sub-scales. The grey area represents scores in the 'normal' region ($\\pm$\ 1\ SD) and the dashed line represents the average score within the normal population.", fig.align='center', fig.width=10, fig.asp=.5, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE, results='hide'}

d_E_L$Group2 <- factor(d_E_L$Group2, levels = c("APD","TD_1","TD_0"))
...
d_E_L$Group2 <- factor(d_E_L$Group2, levels = c("APD","TD_0","TD_1"))
```

```{r,label='ECLIPS-Stats', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide'}
##### ECLIPS analysis #####
...
As seen in the figure, the proportion of abnormal scores across the APD group is substantially higher than in the TD group. The majority of the APD children (80\%, 16/20) performed abnormally in at least two test conditions either in the ST or LiSNS-UK task, whereas there were only three cases (13\%, 3/23) in the TD children. Another interesting observation is that apart from one TD child, who experienced difficulties in various measures including the CCC-2, none of the other TD children experienced language difficulties. This is in contrast to the APD group where 90\% (18/20) of the children experienced some kind of language deficit. The CELF-RS has been reported to be a good marker for children with DLD. Nevertheless, in the present study, the test norms were not found to be very sensitive in capturing the APD children language difficulties. Although their performance was noticeably poorer than in the TD group, only two APD children obtained abnormally poor CELF-RS score. Whereas nearly half of the APD children (45\%, 9/20) exhibited a CCC-2 score indicative of DLD, and about the remaining half (40\% 8/20) obtained a CCC-2 score indicative of pragmatic language and social communication deficit (PLI). The association between the above mentioned language measures for the listeners' performance ("normal"/abnormal) was further examined using multiple Spearman's rho correlation tests [*rcorr* function, Hmisc R package; @HmiscPackageR]. Interestingly, there was a highly significant correlation between the CELF-RS and the DLD performance $\rho$ = 40, $p$ \< 0.001), whereas correlation between CELF-RS and PLI was not significant ($\rho$ = -0.11, $p$ \> 0.05)). Thereby suggesting that both the CELF-RS and DLD taps into similar skills.\

<!-- Although the CELF-RS has been reported to be a good marker for children with DLD, nevertheless, the results of the present study suggests otherwise. While performance in the APD group was noticeably poorer than in the TD group, only two APD children obtained abnormally poor CELF-RS score, whereas nearly half of the APD children (45\%, 9/20) exhibited a CCC-2 score indicative of DLD, and about the remaining half (40\% 8/20) obtained a CCC-2 score indicative of pragmatic language and social communication deficit (PLI).\ -->
...
```{r, echo=FALSE, eval=FALSE}
# Test correlation between RS and DLD or PLI:

# Results: Although RS is less sensitive than DLD, they both correlates! (rho=0.40), where as correlation between RS and PLI is n.s. and negative (rho=-0.11).

corstars(FullData_w[,5:9], result="none", method="spearman",p.adj = "none")
# corstars(FullData_w[,5:9], result="none", method="spearman",p.adj = "fdr")
```
...
# change some names
SumAbnormal$CondCode <-   str_replace(SumAbnormal$CondCode,"ASLN","ASL")
SumAbnormal$CondCode <-   str_replace_all(SumAbnormal$CondCode,"\\.","-")
...
ASLN_s <- ASLN %>% 
  dplyr::select(., listener,APDsibling, Group,Group2, material, CondCode, "z"=z_trim) %>% droplevels()
...
effSize_ST <- ST2 %>% rstatix::group_by(CondCode,material) %>%
    rstatix::cohens_d(z ~ Group, data=., ci = TRUE) %>% as.data.frame() %>% select(CondCode,material,n1,n2,conf.low,conf.high,effsize,magnitude)

effSize_ST$CondCode <- paste0(str_replace_all(effSize_ST$CondCode, fixed("_"), "\\_"))
effSize_ST$CondCode <- paste0(str_replace_all(effSize_ST$CondCode, fixed("ASLN-"), ""))
effSize_ST$CondCode <- paste0(str_replace_all(effSize_ST$CondCode, fixed("CCRM-"), ""))

effSize_ST$material <- paste0(str_replace_all(effSize_ST$material, fixed("ASLN"), "ASL"))

# effSize_ST %>% ddply(.,~material,summarise,mean=round(mean(effsize),2),median=round(median(effsize),2))

effSize_ST2 <- data.frame(cbind("condition"=as.character(effSize_ST$CondCode), 
                                "material"=effSize_ST$material,
                                "N1 APD"=effSize_ST$n1,"N2"=effSize_ST$n2,
                                "CI"=sprintf("%s - %s",effSize_ST$conf.low,effSize_ST$conf.high), 
                                "d"=round(as.numeric(effSize_ST$effsize),2),"magnitude"=as.character(effSize_ST$magnitude)))

rownames(effSize_ST2) <- NULL

colnames(effSize_ST2)[3] <- "N$_{APD}$"
colnames(effSize_ST2)[4] <- "N$_{TD}$"
colnames(effSize_ST2)[5] <- "95\\%-CI"
colnames(effSize_ST2)[6] <- "Cohen's d"


kbl(effSize_ST2, booktabs = T,escape = F, linesep = "", caption = "Cohen's d by condition and material.",format = "latex",digits = 2) %>% 
  kable_styling(latex_options = c("scale_down"))

# by material
effSize_ST3 <- ST2 %>% rstatix::group_by(material) %>%
...
PCA_tab$Item <- as.factor(c("Q-ASL-NoAlt","Q-ASL-Alt","AMSSN-ASL.Alt","MDR\\_F-ASL-Alt",
                            "ENG\\_F-ASL-Alt", "Q-CCRM-NoAlt", "Q-CCRM-Alt","AMSSN-CCRM-Alt",
...
my_fn <- function(data, mapping, ...){
  ggplot(data = data, mapping = mapping, ...) +
    geom_point(size=0.5,na.rm=TRUE) + 
    geom_smooth(method="lm",fill="#FDE725FF", se=FALSE, na.rm=TRUE) +
...
graph_corr <- ggpairs(ST_w[c("ST","Material","Nz","PC1.ST","PC2.Material","PC3.Nz")],
                      # upper = list(continuous= my_fn),
                      lower = list(continuous= my_fn,aes(fill = ST_w$Group)),
                      diag = list(continuous = wrap("densityDiag", alpha=0.7),aes(fill = ST_w$Group)),
...
rownames(PCA_tab) <- NULL
PCA_tab$Item <- as.factor(c("CELF-RS","ECLIPS.SAP","ECLIPS.L/L/L","ECLIPS.M\\&A",
...
# scale_colour_manual(values = my_colors_3_expl)+
# scale_fill_manual(values=c("blue","lightblue","black"))+

# stat_compare_means(): method="wilcox.test" OR method="t.test"

# Mark all APD kids with some kind of diagnosed language deficits
# Eval_L$DLD <- ifelse(Eval_L$listener %in% c("APD01","APD05","APD12","APD13","APD14","APD16","APD17","APD19","APD21"),1,0)
# Eval_L$DLD <- as.factor(Eval_L$DLD)

t1 <- ggplot(Eval_L, aes(x=Diagnosis,y=score)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=21,aes(fill=Diagnosis),size=2.5)+
  # scale_fill_manual(values=c("blue","lightblue","black"))+
  scale_fill_manual(values = my_colors_3_expl)+
  facet_grid(. ~ CondCode, scales = "free", switch = "y")+
...
t4 <- ggplot(Eval_L, aes(x=EarProblems,y=score)) +
  ...
  labs(y="Weighted score" ,x = "") + 
  scale_y_continuous(limits = c(-7,7),breaks=seq(-7,7,2))+
  scale_x_discrete(breaks=c("1", "0"),labels=c("MEHx", "No \n MEHx"))+
...
#### Exploratory predictors -- TD vs. TD$_{sib}$
Studies have shown a strong association between APD and history of hearing problems (e.g., due to chronic OME) causing auditory deprivation, and Developmental Language Deficit (DLD). Moreover, several studies demonstrated that these factors are genetically influenced (Pennington \& Bishop, 2009; Bishop, Adams, & Norbury, 2006), while others suggested that some APD-linked aspects (tone sequencing) are more environmentally driven (cf. Moore 2007; Bishop, 2002). 

In the developmental language literature, there are many studies (REFs!) showing that children tends to have similar language skills to their families.

Studying twins or siblings is a useful way to examine both genetic and environmental factors. 

Although the present study was not designed to investigate such questions in a controlled and balanced way, as it happens, our control group sample can be subdivided into two groups of children with an APD sibling (TD$_{sib}$, 8/23) and without (TD, 15/23). Thereby, it is possible that the two groups would preform differently in the study measures. Whether the influencing factors were mostly heritable or acquired by the child's environment, we hypothesised that TD$_{sib}$ will perform poorer than their non-sibling peers. This supposition arises from the common finding that family members tend to be more alike and so TD$_{sib}$ should be more APD-like, especially insofar as there appear to be language deficits in the APD group. 

The association between potential predictors and performance in the two TD groups was graphically explored. Shown in Figure\ \@ref(fig:EvalTD-fig) are boxplots comparing between the listeners' performance split by groups for the ST and language PCA weighted scores, SRM z-scores, CELF-RS, ECLiPS Total and CCC-2 GCC scaled scores. 

Looking at the boxplots, two noticeable observations emerges. Firstly, the TD$_{sib}$ children CELF-RS scores are noticeably poorer than compared with the TD children. **To elaborate?** Give Mdn's? Both groups still perform significantly better than the APD group..

Moreover, parents of TD$_{sib}$ children tended to score their child higher in the ECLiPS, resulting in higher (i.e., better) Total scaled score. This may suggest some kind of positive rating bias towards the non-APD child. This bias was possibly exasperated by the recruitment procedure, where parents were well informed about the study objectives and whether their child will be assigned to the APD or the control group.

Nonetheless, differences between the two TD groups are relatively small and were found insignificant (see ECLiPS analysis in section ???). In contrast, differences in rating between the groups for the CCC-2 GCC score were much smaller. A possible explanations for the smaller trend in the CCC-2 is that....\

```{r, echo=FALSE, fig.align='center', fig.asp=0.6, fig.cap="Add text here. Significant p-values for independent t-test comparison are marked with asterisk (p < 0.05).", fig.width=14, message=FALSE, warning=FALSE, label='EvalTD-fig', out.width='100%'}
...
t1 <- ggplot(Eval_L_TD, aes(x=Group2,y=score, fill=Group2)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=21, size=2.5, colour="black", fill=NA)+
  facet_grid(. ~ CondCode, scales = "free", switch = "y")+
  stat_compare_means(comparisons = list( c("TD_0","TD_1")),label =  "p.signif")+
  scale_x_discrete(breaks=c("TD_0", "TD_1"),labels=c(bquote(bold(TD)), bquote(bold(TD[sib]))))+
  labs(y="Weighted score" ,x = "") + 
  scale_y_continuous(limits = c(-7,7),breaks=seq(-7,7,2))+
  scale_fill_manual(values = my_colors_2TD, name = "Group", labels = c("TD",bquote(bold(TD[sib]))))+
  theme_bw()+
  theme(axis.text = element_text(size = 11, face="bold",colour = "black"),
        axis.title.x = element_text(size=11, face="bold"),
        axis.title.y = element_text(size=11, face="bold"),
...
t2 <-  ggplot(Eval_TD, aes(x=Group2,y=zSRM, fill=Group2)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=21, size=2.5, colour="black", fill=NA)+
  labs(y = "SRM (z-score)" ,x = "") +
  scale_y_continuous(limits = c(-7,5),breaks=seq(-7,5,2))+
...
t3 <-  ggplot(Eval_TD, aes(x=Group2,y=CELF_RS.Scaled, fill=Group2)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=21, size=2.5, colour="black", fill=NA)+
  labs(y = "CELF-RS (scaled score)" ,x = "") +
...
t4 <-  ggplot(Eval_TD, aes(x=Group2,y=ECLIPS.Total, fill=Group2)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=21, size=2.5, colour="black", fill=NA)+
...
t5 <-  ggplot(Eval_TD, aes(x=Group2,y=CCC2.GCC, fill=Group2)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=21, size=2.5, colour="black", fill=NA)+
...
## Discussion
<!-- - Summery of findings -->
<!-- - Implications: interesting findings, did the results meet our expectations? conflicts/agreement with past studies?  -->
<!-- - future work -->
...
- Group difference in the left ear. The small difference (~2.5 dB) suggests it is due to sampling error rather than a real effect. Any similar effect in other studies? Hunter et al., 2020 did not find sig. difference between the ears. 

#### Extended Audiometry
- No group difference. However there seem to be again a trend of slightly poorer thresholds for the APD group in the left ear only.
 
Comparison with Hunter data:
 
Freq	TD_LH	LiD_LH	TD_SK	APD_SK
0.25	14	12	7	8
0.50	13	12	7	8
1.00	9	7	1	4
2.00	9	8	3	5
...
Many studies showed throughout the years that children perform poorer than adults in different auditory tasks.
Examples for different tasks/aspects of listening
Is the cause peripheral or more central maturation with age? 

**Developmental Psychoacoustics BOOK, Chapter 4: Individual Differences in Auditory Capability Among Preschool Children (Frederic Wightman and Prudence Allen):**
They found that children displayed an overall poorer performance in a wide range of measures compared to adults. However the quality of the data (the spread and number of trials/reversals) suggests that this impovrished performance is not due to lack of attention or motivation on behalf of the children..
...
- only a small sample: a more representative data is needed with larger sample-size and a well balanced number of listeners within each year group for a wide age span.
...
#### Speech in noise (where does this go?)
