...
Final_ST.m1 <- model7
tab_model(Final_ST.m1,Final_ST.m2)
# ---------------------------------------------------------------------------------
# test main effects:
...
```{r,label='ST-AgeEffectNEWASL', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE,eval=FALSE}
##### Age effect? ################################################################# 
...
```{r,label='ST-AgeEffectNEWCCRM', echo=FALSE,results='hide',message=FALSE,warning=FALSE, include=FALSE,eval=FALSE}
##### Age effect? ################################################################# 
...
```{r, eval=FALSE}
test <- ST_trimmed %>% 
  arrange(listener,CondCode,material) %>% 
    group_by(listener) %>%
    mutate(Diff = uRevs - lag(uRevs)) %>% 
  subset(., material=="CCRM")
...
# 1          1 0.5156667 0.5217875 0.1830361 0.2480000 0.8573333
# 2          0 0.4783333 0.5556022 0.1994344 0.2886667 0.9700000
...
# APDsibling    median      mean        sd    min       max
# 1          1 0.2981667 0.3363565 0.1640863 0.0875 0.6746667
# 2          0 0.2896667 0.3153858 0.1593562 0.0750 0.7750000  
...
```{r, fig.width=14, fig.asp=0.8, out.width='100%',eval=FALSE}

# uRevs ------------------------------------------------------------------
p1 <- ggplot(ASLN, aes(x=APDsibling, y=uRevs)) +
  geom_hline(yintercept=0.97, linetype="dashed",color = "black", size=0.5)+
  geom_hline(yintercept=0.05, linetype="dashed",color = "black", size=0.5)+
  geom_boxplot(data=filter(ASLN, Group=="TD"),position=position_dodge(width=0.7),outlier.shape=NA) +
  geom_quasirandom(data=filter(ASLN, Group=="TD"),dodge.width=0.7,shape=1,colour="blue")+
  labs(title = "ASL" ) + 
  scale_y_continuous(limits = c(0,1),breaks=seq(0,1,0.1)) +
  theme_bw() +
...
p2 <- ggplot(CCRM, aes(x=APDsibling, y=uRevs)) +
  geom_hline(yintercept=0.97, linetype="dashed",color = "black", size=0.5)+
  geom_hline(yintercept=0.05, linetype="dashed",color = "black", size=0.5)+
  geom_boxplot(data=filter(CCRM, Group=="TD"),position=position_dodge(width=0.7),outlier.shape=NA) +
  geom_quasirandom(data=filter(CCRM, Group=="TD"),dodge.width=0.7,shape=1,colour="blue")+
  labs(title = "CCRM" ) + 
  scale_y_continuous(limits = c(0,1),breaks=seq(0,1,0.1)) +
  theme_bw() +
  facet_grid(. ~ CondCode, scales = "free", switch = "y")

(p1 + p2) + plot_layout(ncol = 1, nrow = 2, heights = c(1, 1), widths = c(1.5,1.5)) + plot_annotation(tag_levels = 'A') + plot_layout(guides='collect') &
  theme(legend.position='bottom',        
        legend.direction = "horizontal",
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"),
...
# z-scores ------------------------------------------------------------------
p1 <- ggplot(ASLN, aes(x=APDsibling, y=z_trim)) +
  geom_boxplot(data=filter(ASLN, Group=="TD"),position=position_dodge(width=0.7),outlier.shape=NA) +
  geom_quasirandom(data=filter(ASLN, Group=="TD"),dodge.width=0.7,shape=1,colour="blue")+
  labs(title = "ASL" ) + 
...
p2 <- ggplot(CCRM, aes(x=APDsibling, y=z_trim)) +
  geom_boxplot(data=filter(CCRM, Group=="TD"),position=position_dodge(width=0.7),outlier.shape=NA) +
  geom_quasirandom(data=filter(CCRM, Group=="TD"),dodge.width=0.7,shape=1,colour="blue")+
  labs(title = "CCRM" ) + 
  scale_y_continuous(limits = c(-5,10),breaks=seq(-5,10,2)) +
...
```{r, fig.align='center', fig.asp=1, fig.width=14, eval=FALSE}
# SRM in dB
