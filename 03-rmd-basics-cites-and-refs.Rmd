...
<!-- (ref:tabCap3) ST: Age-effect: post-hoc paired-comparison t-tests for Condition x Material interaction. The test was performed on the fitted LMEM model and included adjusted least-squared-mean for the random intercepts (subjects) using lsmeans package [emmeans package; @emmeansPackageR]. P-values were adjusted for multiple-comparisons using Bonferroni correction. -->

<!-- Comparison of the groups overall performance by material revealed an opposite direction of performance (see also Figure\ \@ref(fig:ST-AgeTD)). While the performance of the TD$_{sib}$ children was on average 0.06 better than their TD peers for the ASL material, their performance was 0.04 poorer for the CCRM material. These results are in contradiction to our expectations if any differences arose. In such a case, we would have predicted a larger decrement in performance (i.e., poorer score) for the ASL sentences which are more linguistically challenging than for the CCRM sentences. This disagreement and the very small estimated mean difference for the interaction between Material and Group in the full model (0.052) suggests that the differences picked up by the model are due to sampling error. The results suggests that the differences between the two groups depends upon the material type. This is not surprising considering the large differences between the two material types, where the CCRM sentences are expected to be more intelligible.\ -->

<!-- Two factors contributed to the significant interaction: -->
<!-- - opposite direction of performance within group across material. -->
<!-- - material type differences -->

Lastly, the significant interaction between Material and Sibling was examined with a model-based post-hoc t-tests comparison (see Table\ \@ref(tab:ST-AgeLMEMpost1)). There was no significant difference in SRdTs between the two TD groups for both speech materials (both p's \>\ 0.05). The model prediction estimated marginal means predicted that performance of the TD$_{sib}$ children was on average 0.008 better than their TD peers for the ASL material and 0.04 poorer for the CCRM material. These results are in contradiction to our expectations if any differences arose. In such a case, we would have predicted a larger decrement in performance (i.e., poorer score) for the ASL sentences which are more linguistically challenging than for the CCRM sentences. Nonetheless, the very small estimated mean differences and the negligible effect-sizes suggests that the differences picked up by the model are due to sampling error. The results suggests that the differences between the two groups depends upon the material type. This is not surprising considering the large differences between the two material types, where the CCRM sentences are expected to be more intelligible.\
...
# -- get derived scores --------------------------------------
# uRevs
ASLN_w$Diff_QNoAlt <-  ASLN_w$uRevs_Q_ASLN_NoAlt - ASLN_w$uRevs_Q_ASLN_Alt
...
# -- get derived scores --------------------------------------
# uRevs
CCRM_w$Diff_QNoAlt <-  CCRM_w$uRevs_Q_CCRM_NoAlt - CCRM_w$uRevs_Q_CCRM_Alt
...
\
Derived scores were calculated by subtracting the individuals' score for the reference condition Quiet-Alt from the rest of the test conditions (see Figure\ \@ref(fig:ST-DerivedPlotsuRevs)). Whereby the cost of switching the stimuli between the two ears was represented by subtracting Q-Alt from the condition without a distractor or alternations (Quiet-NoAlt). Whereas, differences between test conditions with a distractor describes the IM effect after accounting for the costs of switching. It appears from the figure that performance in the TD and APD group are more similar for less challenging conditions, and becomes more different for more challenging conditions with distractors. In addition, there is no strong age effect.\
...
Two separate models were used to examine the effect of switching and IM with the derived scores (in DC) as dependent variable (parametric model assumptions were met). Note that data for CCRM_F was excluded from the analysis since it was only measured for the CCRM material. The first model examined the effect of switching using a linear regression model with Material, Sibling and Age as predictors. The model that gave the best fit and test results are given in Table\ \@ref(tab:ST-AgeAltEffTab). There was a (marginally) significant Sibling x Age interaction ($p$ = 0.043). None of the remaining terms were significant (all $p$'s \> 0.05). IM effects were tested using LMEM model for the three distractors (Distractor), Material, Sibling, and Age as fixed factors and random intercepts for subjects. The model that gave the best fit did not include the fixed factors Sibling or Age (see Table\ \@ref(tab:ST-AgeIMEffTab)). There was a highly significant main effect of Distractor ($p$\ \<\ 0.001) as well as a significant effect of Material ($p$\ \< 0.05). Post-hoc paired-comparison t-tests based on the fitted model for Distractor (with Bonferroni correction) found a significant difference in IM effect between AMSSN and MDR ($Estimate$ = -0.24, $95\%-CI$ = -0.29 - -0.19, $p$\ \< 0.001) and AMSSN and ENG_F ($Estimate$ = -0.28, $95\%-CI$ = -0.33 - -0.23, $p$\ \< 0.001), whereas there was no significant difference between the two speech distractors ($Estimate$ = -0.04 $95\%-CI$ = -0.09 - 0.01, $p$ = 0.16).\
...
(model3<-update(model2,  ~ . - material:Age:Group2))
anova(model2,model3)
summary(model3)

(model4<-update(model3,  ~ . -  CondCode:Age:Group2))
anova(model3,model4)
summary(model4)

(model5<-update(model4,  ~ . - CondCode:material:Group2))
anova(model4,model5)
summary(model5)
...
(model7<-update(model6,  ~ . - Age:Group2))
anova(model6,model7)
summary(model7)

(model8<-update(model7,  ~ . - material:Group2))
anova(model7,model8)
summary(model8)

(model9<-update(model8,  ~ . - CondCode:Group2))
anova(model8,model9)
summary(model9)
...
(model11<-update(model10,  ~ . - CondCode:Age))
anova(model10,model11)
summary(model11)

(model12<-update(model11,  ~ . - CondCode:material))
anova(model11,model12)
summary(model12)

(model13<-update(model12,  ~ . - Group2))
anova(model12,model13)
summary(model13)

(model14<-update(model13,  ~ . - Age))
anova(model13,model14)
summary(model14)

(model15<-update(model14,  ~ . - material))
anova(model14,model15)
summary(model15)
# model15: uRevs ~ CondCode + (1 | listener)
# model14: uRevs ~ CondCode + material + (1 | listener)
#         npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# model15    5 -194.20 -179.98 102.10  -204.20                       
# model14    6 -198.35 -181.28 105.17  -210.35 6.1495  1    0.01314 *

# best mode --> model 14: uRevs ~ CondCode + material + (1 | listener)

tab_model(model14)
# visreg::visreg(model14)
# visreg::visreg(model14,"CondCode","material")


BestModel <- model14
# report::report(model14)
...
# Response: uRevs
#             Chisq Df Pr(>Chisq)    
# CondCode 214.1186  2    < 2e-16 ***
# material   6.3204  1    0.01194 *


# material:
(BestModel.1 <- update(BestModel,  . ~ . -material))
Mat.LMEM <- anova(BestModel,BestModel.1)
summary(BestModel.1)
# BestModel.1: uRevs ~ CondCode + (1 | listener)
# BestModel: uRevs ~ CondCode + material + (1 | listener)
#             npar     AIC     BIC logLik deviance  Chisq Df Pr(>Chisq)  
# BestModel.1    5 -194.20 -179.98 102.10  -204.20                       
# BestModel      6 -198.35 -181.28 105.17  -210.35 6.1495  1    0.01314 *
...
summary(BestModel.2)
# BestModel.2: uRevs ~ material + (1 | listener)
# BestModel: uRevs ~ CondCode + material + (1 | listener)
#             npar      AIC      BIC  logLik deviance  Chisq Df Pr(>Chisq)    
# BestModel.2    4  -84.784  -73.407  46.392  -92.784                         
# BestModel      6 -198.346 -181.281 105.173 -210.346 117.56  2  < 2.2e-16 ***
...
ST_trimmed_Diff_switch <- ST_trimmed_Diff_switch %>% filter(CondCode=="Diff_QNoAlt") %>% as.data.frame(.) %>% droplevels()
...
ST_IM_DC_tab$p <- as.numeric(gsub(",","",ST_IM_DC_tab$p,fixed=TRUE))

for (i in 1:length(ST_IM_DC_tab$p)) {

  ST_IM_DC_tab$p[i] <-  if(!is.na(ST_IM_DC_tab$p[i])==TRUE){
    ifelse(ST_IM_DC_tab$p[i] <.001,"\\textbf{< 0.001}",
           ifelse(ST_IM_DC_tab$p[i] <.05, sprintf("\\textbf{%.02f}",ST_IM_DC_tab$p[i]),
                  ST_IM_DC_tab$p[i]))
  } else {
   " "
  }
...
kbl(ST_AgeTab_IM, booktabs = T,escape = F, linesep = "",caption = "ST: Derived measures (DC): LMEM model examining the effect of Distractor (AMSSN, MDR\\_F, and ENG\\_F) and Material (ASL/CCRM) as fixed factors and random intercepts for subjects (reference levels: Distractor = AMSSN; Material = ASL; Sibling = TD). Note: only TD data following outliers trimming was included.",
    align = c("lccc"),format = "latex",digits = 3) %>%
    # kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c("DC ~ Distractor + Material + (1 | Subjects)" = 4), align = "l",line = TRUE) %>%
  # column_spec(1, width = "8cm") %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
  column_spec(4, italic = T)
...
```{r,label='ST-AgeAltEffTab',echo=FALSE,warning=FALSE, message=FALSE}
...
\
As before, descriptive statistics for the listeners' z-scores for the derived measures are given in Table\ \@ref(tab:ST-DerivedTab) and plotted in Figure\ \@ref(fig:ST-DerivedPlotsz). Overall, differences between the groups were larger for IM effect than for the switching effect. Furthermore, the percentage of abnormal performance is generally low in both test materials (range: 0\% - 25\%), with slightly larger proportion in the CCRM material. The proportion of abnormal z-scores in the TD group was relatively low. None of the TD children obtained an abnormal score for the ASL material, whereas the proportion of abnormal score in the CCRM material ranged between 0\% to circa 9\% (2/23).\
...
ST_DiffzTab$CondCode <- c("Switching (Q-NoAlt-Q-Alt)","AMSSN (AMSSN - Q-Alt)","ENG\\_F (ENG\\_F - Q-Alt)","MDR\\_F (MDR\\_F - Q-Alt)",
                          "Switching (Q-NoAlt-Q-Alt)","AMSSN (AMSSN - Q-Alt)","ENG\\_F (ENG\\_F - Q-Alt)","MDR\\_F (MDR\\_F - Q-Alt)","CCRM\\_F (CCRM\\_F - Q-Alt)")
...
Group differences were examined using two separate models for switching and IM effects, excluding data meaured with the CCRM_F distractor. Parametric methods assumptions were met for IM data, whereas the assumption of normal distribution for the switching data was rejected. Nevertheless, since nonparametric methods gave similar results, it was decided to report here only the outcomes of the parametric method. Switching effect were examined using LMEM model with Material (ASL/CCRM) and Group (APD/TD) as fixed factors and and random intercepts for subjects (reference levels: Group = APD; Material = ASL). The final model which gave the best fit did not include Material x Group interaction. There was no significant main effect of both Material ($\chi^{2}$ = 0.00016, $Df$ = 1, $p$ = 0.990) and Group ($\chi^{2}$ = 0.380, $Df$ = 1, $p$ = 0.538). Similarly, group differences across the three distractors were tested using LMEM model with Distractor, Material and Group as fixed factors and random intercepts for subjects. The final model and main effects are given in Table\ \@ref(tab:ST-zIMEffTab). There was a significant main effect of Group ($p$\ \<\ 0.05), whereas there was no significant main effect for Distractor or Material ($p$\ \>\ 0.05).\
...
A separate LMEM model for the full CCRM data, including the CCRM_F distractor, gave similar results, with a significant main effect of Group ($\chi^{2}$ = 4.184, $Df$ = 1, $p$ = 0.041), whereas there as no signifiacnt main effect of Distractor ($\chi^{2}$ = 0.990, $Df$ = 3, $p$ = 0.804).\
...
They found that children displayed an overal poorer performance in a wide range of measures compared to adults. However the quality of the data (the spread and number of trials/reversals) suggests that this impovrished performance is not due to lack of attention or motivation on behalf of the children..
