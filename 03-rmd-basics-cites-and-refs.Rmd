...
kbl(BackgroundTab, booktabs = T,escape = F, linesep = "",caption = "Add caption here.",
...
kbl(ProcedureTab, booktabs = T,escape = F, linesep = "",caption = "Add caption here.",
...
The locally developed Listening in Spatialised Noise Sentences UK (LiSNS-UK) assesses the ability to use binaural cues in speech-on-speech listening conditions. The test development, speech material normalisation, and norms standardisation followed @Cameron2007 development steps and are described in detail in Chapter ???. The test uses virtualisation techniques to create spatial distribution of sound sources in space for headphones presentation where target sentences [ASL; @MacLeod1990] are presented in two simultaneous speech distractors (unrelated children's stories spoken by the target talker). The LiSNS-UK comprises of two main listening conditions, differing in their availability of spatial cues. The target sentences are configured to always appear in front of the listener's head, at 0$^{\circ}$ azimuth on the horizontal plane, with the two streams of speech distractors either co-located in space with the target (S0N0), resulting in relatively poor speech perception, or offset in space, with one distractor to either side of the target at $\pm$ 90$^{\circ}$. The spatial separation as in the later condition results in an improvement in speech perception of circa 13 dB [@Cameron2011], typically termed as spatial release from masking (SRM). This SRM advantage is calculated by taking the difference between performance in the co-located condition and the separated condition.\
...
All the statistical analyses were computed in R (version; REF). Residual analysis was performed separately for each measure to determine whether the data fulfils parametric methods assumptions of normal distribution using Shapiro-Wilk test [*shapiro.test()*, @RCore] and homogeneity of variance using Levene's test [*leveneTest()*; @carPackageR]. Consequently, statistical analyses for factorial design data that met these requirement was performed using a linear mixed-effects regression models (LMEMs). LMEM was fitted using the *lmer()* function [lme4 package; @lme4PackageR]. Backward model selection procedure was applied to find the model that gives the best fit using a likelihood ratio test ($\chi^2$). Main effects and interaction terms were tested by comparing predictions of the full model to a reduced model where each fixed term was separately removed, starting with the interaction terms. When applicable, post-hoc paired comparison t-test was performed on the fitted model and included adjusted least-squared-mean for the random intercepts (subjects) using the *lsmeans()* function from the emmeans R package [@emmeansPackageR]. In addition, group differences for single parametric measures such as CELF-RS and CCC-2 total score were examined using *boot.t.test()* function [MKinfer package; @MKinferPackageR] which performs independent-samples t-test with bootstrapping (n=9999).

Nonparamertic data was analysed using nparLD() function [nparLD package; @nparLDPackageR] which is a robust rank-based method for analysis of skewed data or with outliers or from a small sample size [see [@Feys2016] for a good introduction for robust nonparametric techniques]. The function enables different types of nonparametric tests for factorial design data with repeated measures with variable between-/within-subjects factors. The results reported in the present study are based outputs of the ANOVA-type statistic test (ATS). Inspection of the ENVASA task age-independent z-scores revealed that the assumption of sphericity (Mauchly's test) was violated. Therefore, analysis was performed using *npIntFactRep* package [@npIntFactRepPackageR], which is another robust aligned rank technique that enables sphericity correction (Greenhouse-Geisser). Post hoc comparison and group difference was examined using Wilcoxon rank-sum test with permutation (N=999999, independent two samples) which is a t-test equivalent for non-parametric data [*coin::wilcox_test()*; @CoinPackageR].
...
```{r, label='PTA', fig.cap="Standard audiometry: APD participants pure-tone detection thresholds plotted seperately for the left and the right ear (black lines). The shaded grey area represents the TD group thresholds range and the white line represents the TD group mean at each frequency. The dashed line represents the threshold criteria of hearing level $\\leq$ 25\ dB\ HL.", echo=FALSE, fig.align='center', figures-side, out.width='85%',fig.width=12, fig.height=6}
...
```{r, label='RegAud-fig', echo=FALSE,fig.cap="Standard audiometry: Pure-tone detection thresholds by frequency bands between 0.25 to 8 kHz (A), and averaged thresholds (B). Individual scores are indicated by circles. The boxes show the data interquartile range (25th-75th percentile) and the horizontal line indicate the median (i.e., 50th percentile). Values that fall within 1.5 times the interquartile range are indicated by the wiskers.",fig.align='center', fig.width=12, fig.asp=0.5, out.width='100%'}
### Frequencies ###
...
kbl(PTA_tab1, booktabs = T, escape = F, linesep = "", caption = "Standard audiometry: Descriptives for pure-tone detection thresholds by frequency bands and ear split by the two groups.",
...
```{r,label='RegAud-Assumptions', message=FALSE, warning=FALSE, include=FALSE, results='hide'}
########################### Test assumptions #########################################
...
# linear model
w1 <- lm(dBHL ~ Freq * Ear * Group, data = d_L)
...
leveneTest(residuals(w1) ~ Freq * Ear * Group, data = d_L)
...
(ref:tabCap1) Post-hoc paired comparison t-test for PTA x Group. The test was performed on the fitted LMEM model and included adjusted least-squared-mean for the random intercept (subjects) using lsmeans package [@emmeansPackageR].

```{r, label='RegAud-LMEMpost', echo=FALSE}
kbl(RegAud_tab2,booktabs = T, escape = F, linesep = "",caption = '(ref:tabCap1)',
    align = c("lcccccccccccccccc"),format = "latex",digits = 3) %>%
    kable_styling(latex_options = c("scale_down")) %>%
    add_header_above(c(" "=1, "APD" = 5, "TD" = 5,"post-hoc paired t-test" = 6)) %>%
  add_footnote(c("significant p-values (p < 0.05) are shown in bold."), notation = "symbol") %>%
...
Regression lines for PTAs are not sig. nparLD with Age was not executable. LMEM *dBHL ~ Freq + Ear + Group + Age + Freq:Ear + Freq:Group + (1 | listener)* (although parametric assumptions were not met) resulted in a p-value of 0.0594.
```
...
```{r, label='EHF', fig.cap="EHF audiometry: Pure-tone detection thresholds for extended high-frequency bands measured in the left and the right ear. The thin black lines represents the individual thresholds in the APD group and the group mean is marked by the bold black line. The shaded grey area represents the TD group threshold range and the white line represents the TD group mean at each frequency.", echo=FALSE, fig.align='center', figures-side, out.width='85%',fig.width=12, fig.height=6}
...
# min and max of y axis
max	=	max((d_HF[1:nrow(d_HF),34:36]),na.rm=T) + 17
min	=	min(d_HF[1:nrow(d_HF),34:36],na.rm=T) - 10
...
```{r, label='EHF-Boxplot', echo=FALSE,fig.cap="EHF audiometry: Boxplots for pure-tone detection thresholds measured at the extended high-frequency bands split by ear and groups (A). Boxplots of the groups averaged PTAs and better-ear BE thresholds are depicted in figure B. Individual scores are indicated by circles. The boxes show the data interquartile range (25th-75th percentile) and the horizontal line indicate the median (i.e., 50th percentile). Values that fall within 1.5 times the interquartile range are indicated by the wiskers.",fig.align='center', fig.width=10, fig.asp=0.5, out.width='100%'}
### Frequencies ###
...
kbl(EHF_tab, booktabs = T,escape = F, linesep = "", caption = "EHF audiometry: Descriptive collapsed by group.",
    align = c("lccccccccccc"),format = "latex") %>% 
  kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c(" " = 2, "APD" = 5, "TD" = 5)) %>%
  column_spec(c(8),border_left = T) %>% 
  kable_styling() %>%
  pack_rows("Octave frequency bands", 1, 6) %>%
...
d_HF_L <- data.frame(d_HF_L)
EHF.nparLD <- nparLD(dBHL ~ Freq * Ear * Group, data = d_HF_L, subject = "listener", description = TRUE, plot.CI=TRUE)
...
```{r,label='ST-FigLevsPC', fig.cap="ST raw data: frequency of possible outliers for observations with LevsPC\ $\\leq$\ 35\\%. LevsPC denotes the proportion of correct keywords within the final test trials.", fig.width=9, fig.asp=0.7, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
[*Belongs to discussion?*]{.correction}
...
Next, age effect was evaluated using LMEM model, with Condition (Quiet-NoAlt, Quiet-Alt, AMSSN, MDR_F, \& ENG_F), Material (ASL / CCRM) and Age as fixed factors, SRdT as dependent variable and a random intercept for subjects (reference levels: Condition = Quiet-NoAlt; Material = ASL). Note that data for CCRM_F was excluded from the model since it was only measured for the CCRM material[^rmd-basics-cites-and-refs-2]. Additional random intercept for APD siblings resulted in singular fit an was thus not included in the saturated model. Parametric assumption inspection of normal distribution was met, whereas the assumption of homogeneity of variance was marginal (p \= 0.04). A model without three-way interaction between Condition, Material and Age was found to give the best fit (see Table \@ref(tab:ST-AgeLMEM)). Model comparison revealed a highly significant two-way interaction between Condition x Age (p \< 0.001) as well as a marginal two-way interaction between Condition x Material and Material x Age (p = 0.052) which further support the trends seen in the scatterplots.

[^rmd-basics-cites-and-refs-2]: A separate model for CCRM data only, also gave a significant Condition x Age interaction (p \< 0.001).

`r kableExtra::text_spec("The significant Material x Age interaction indicates that the developmental trend is different between the two speech materials, with larger age effect (i.e., steeper slopes) for the ASL than for the CCRM sentences. [the LMEM summary gives an estimate for materialCCRM:Age of 0.014 (ASL is the reference level). Does that mean that the ASL sentences mean improvement per 1 year was 0.014 larger than for the CCRM material when averaged across conditions? I'm not too sure how to interpret this..]", color = "red")` Furthermore, the significant Condition x Material interaction implies that performance in the different test conditions differed between the two speech materials. A post-hoc t-test comparison based on the fitted model given in Table \@ref(tab:ST-AgeLMEMpost), revealed a highly significant difference in performance between the speech materials across all five test conditions (all p-values \< 0.001). The estimated mean difference between the contrast pairs ranged between \+0.18 to \+0.27, hence, the CCRM speech material was significantly more intelligible than the ASL material, across all test conditions.\
...
(ref:tabCap2) ST: Age-effect - post-hoc paired comparison t-test for Condition x Material two-way interaction. The test was performed on the fitted LMEM model and included adjusted least-squared-mean for the random intercept (subjects) using lsmeans package [emmeans package; @emmeansPackageR].

```{r, label='ST-AgeLMEMpost', echo=FALSE}
postLMEM$`p-value` = ifelse(postLMEM$`p-value`<.001,sprintf("\\textbf{$<$ 0.001}"),postLMEM$`p-value`)

kbl(postLMEM,booktabs = T, escape = F, linesep = "",caption = '(ref:tabCap2)',
...
d_LiSNS_w <- d_LiSNS %>%
  pivot_wider(
    id_cols = c("listener","Age","Group","APDsibling"),
    names_from = "CondCode",
    values_from = c("uRevs")) %>%
  ungroup()
...
ggpairs(LiSNS_w[c(3,9:11)],lower = list(continuous = "smooth"),mapping = aes(color = LiSNS_w$Group),title="z-scores")

ggpairs(LiSNS_w[c(3,9:11)],lower = list(continuous = "smooth"),upper = list(continuous = "smooth"), mapping = aes(color = LiSNS_w$Group)) + ggtitle("z-scores")

ggpairs(LiSNS_w[c(3,5:7)],lower = list(continuous = "smooth"),mapping = aes(color = LiSNS_w$Group),title="dB")

ggpairs(LiSNS_w[c(3,5:7)],lower = list(continuous = "smooth"),upper = list(continuous = "smooth"), mapping = aes(color = LiSNS_w$Group)) + ggtitle("dB")

LiSNS_w2 <- LiSNS_w

# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="TD23",NA,LiSNS_w2$z_trim_SRM)
# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="APD20",NA,LiSNS_w2$z_trim_SRM)
# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="APD02",NA,LiSNS_w2$z_trim_SRM)
# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="APD17",NA,LiSNS_w2$z_trim_SRM)
# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="APD16",NA,LiSNS_w2$z_trim_SRM)
# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="APD05",NA,LiSNS_w2$z_trim_SRM)
# LiSNS_w2$z_trim_SRM <- ifelse(LiSNS_w2$listener=="APD09",NA,LiSNS_w2$z_trim_SRM)

my_fn <- function(data, mapping, ...){
  ggplot(data = data, mapping = mapping, ...) + 
    geom_text(label=LiSNS_w$listener, colour="black",size=2) +
...
graph_corr <- ggpairs(LiSNS_w[c(3,9:11)],
                      upper = list(continuous= my_fn),
                      lower = list(continuous= my_fn),
                      aes(color = LiSNS_w$Group),legend=1) +
  theme(legend.position = "bottom",
        legend.direction = "horizontal")
...
# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf
kbl(LiSNS_z_tab, escape = F, booktabs = T, linesep = "", caption = "LiSNS-UK standard residuals (z-scores) descriptives by group. abnormal: the percentage of abnormal score, defined as: z-score > 1.96 (SSN, S0N0, \\& S0N90) and  z-score < 1.96 (SRM). Percentage in brackets were accounted for observations that were trimmed in the z-score calculation procedure.",
...
kbl(LiSNS_AgeTab, booktabs = T,escape = F, linesep = "",caption = "LiSNS-UK - age effect: LMEM model for SRT with condition (reference level: SSN) and age as fixed factors and a random intercept for subjects. Note: only data measured with the control group following outliers trimming was included.",
    align = c("lccc"),format = "latex",digits = 3) %>% 
...
```{r, label='LiSNS-zfig', fig.cap="Individual scores are indicated by circles. The boxes show the data interquartile range (25th-75th percentile) and the horizontal line indicate the median (i.e., 50th percentile). Values that fall within 1.5 times the interquartile range are indicated by the wiskers..", fig.align='center', fig.width=12, fig.asp=0.5, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
#A random intercept for 'APDsibling' was not used as it resulted in singular fit down the line after model selection..
...
Measurements of the ENVASA task in the current study followed the same factorial design as used by @Leech2009, with 2 background types (single-/dual) x 4 SNRs (low: -6, -3 dB; high: 0 +3 dB), resulting in a total of 92 responses (%-correct, PC) per listener or between 10 to 11 test items per background/SNR combination. Because of the small number test items per condition, responses were averaged into three measures: 1) *single background*, 2) *dual backgrounds*, and 3) *combined background* which reflects the overall performance across the two background types.
...
t1 <- t1 + facet_grid(. ~ reorder(CondCode,CondCode), scales = "free", switch = "y",
                      labeller = labeller(CondCode = c("Total_s" ="Single \n background",
                                                       "Total_d"="Dual \n backgrounds",
                                                       "Total"="Combined"))) +
  theme(panel.spacing.x = unit(0,"line"),
        strip.background = element_rect(fill="white", color="white"),
...
```{r,label='ENVASA-npIntFactRep', message=FALSE, warning=FALSE, results='hide'}
...
Descriptives of the ECLiPS parental report scaled scores split by groups is given in Table \@ref(tab:ECLIPS-Tab) and shown in Figure \@ref(fig:ECLIPS-plot). A score below the 10$^{th}$ percentile (corresponding to a scale score of circa 6) is generally considered as clinically significant listening and processing difficulties [@Barry2014]. Overall, the ECLiPS was able to well separate between the two groups across all the different sub-scales. All APD children exhibited abnormal Total score, whereas only two TD children (out of 22) obtained abnormal Total score.

A closer look at Figure \@ref(fig:ECLIPS-plot) reveals a clear difference in the distribution of the scaled scores across the two groups, with relatively larger spread for the TD group. Inspection of the groups Total score revealed that the APD group don't follow a normal distribution and that the assumption of homoscedasticity was violated (p \< 0.001). Thus, group difference for the Total score was examined using a non-parametric independent two samples Wilcoxon rank-sum test with permutation [N=999999, *coin::wilcox_test()*; @CoinPackageR]. There was a highly significant difference between the groups with a large effect-size (p \< 0.001, r =  0.86).
...
```{r,label='ECLIPS-stats', echo=FALSE,warning=FALSE,message=FALSE,results='hide'}
# --------------------------------------------------------------------------------------------------
# Assumptions:
...
An overview of the children performance split by group is given in Figure \@ref(fig:MatrixPlot) providing an overlook at individuals that performed outside the norm (filled black cells) in one or more tasks. Abnormally poor performance for the listeners age-independent scores was defined using standardised norms for the CELF-RS, ECLiPS and the CCC-2 data or was defined as a z-score outside a one-tailed cut-off $\pm$ 1.96 (where circa 97.5% of the normal population is expected to lay within) for the rest of the tasks.

As seen in the figure, the proportion of abnormal score of the APD group is substantially higher than of the TD group. The majority of the APD children (16/20) performed abnormally in at least two test conditions from the ST or the LiSNS-UK task, whereas this was the case only in three (out of twenty-one) TD children. The proportion of abnormal scores by measure or task split by group is shown in Figure ???. Both the ECLiPS total score and the CCC-2 GCC sum score resulted in the largest separation between the groups. Out of the auditory tasks the tests conditions that resulted in the highest proportion of abnormal scores in the APD group are AMSSN (ASL: 50%, CCRM: 40%), Quiet-ASL-NoAlt (45%) and MDR_F-ASL (40%), whereas only 26% of the APD children had abnormal SRM score
...
Abnormal_RS <-  d_RS %>% 
  mutate("RS" = ifelse(d_RS$ScaledScore<7,1,0)) %>% select(.,listener, Group, RS)
...
Abnormal_CCC2 <-  d_CCC2 %>% mutate("GCC" = ifelse(d_CCC2$GCC<55,1,0),
  ...
  dplyr::select(.,listener,Group,GCC,DLD,PLI)
...
Abnormal_ENVASA <-  ENVASA_Total %>% filter(CondCode=="Total_s") %>% 
  dplyr::select(.,listener, Group,"ENVASA"=Outlier) %>% droplevels()
...
FullData_w  <- df_list %>% reduce(full_join, by = c("listener","Group")) 
# re-arrange
...
FullData_w_TD <- as.matrix(FullData_w_TD)
...
t1 <- t1 + labs(x = "", y = "",title = "APD") +
    scale_y_discrete(labels=c("RS"="CELF-RS","GCC","DLD","PLI","ECLiPS","ENVASA","Q.ASLN.NoAlt"="Q-ASL-NoAlt",
                              "Q.ASLN.Alt"="Q-ASL-Alt","AMSSN.ASLN.Alt"="AMSSN-ASL","MDR_F.ASLN.Alt"="MDR_F-ASL",
                              "ENG_F.ASLN.Alt"="ENG_F-ASL","Q.CCRM.NoAlt"="Q-CCRM-NoAlt","Q.CCRM.Alt"="Q-CCRM-Alt",
...
t2 <- qheat(data.frame(FullData_w_TD), by.column=NULL, high="black", text.color =NA,
    grid="grey20") + guides(fill=FALSE) +
    scale_x_discrete(labels = gsub("[^0-9.-]", "", FullData_w$listener[which(FullData_w$Group=="TD")])) 
t2 <- t2 + labs(x = "", y = "",title = "TD") +
...
```{r,label='MatrixPlot', fig.align='center', fig.asp=0.5, fig.cap="Abnormal (black cells) and normal (empty cells) performance in the present study test battery of individuals from the APD group (n=20) and the TD group (n=23). Missing data is marked by the grey cells.", fig.width=12, out.width='100%', message=FALSE, warning=FALSE, echo=FALSE}
...
```{r,label='Abnormal-perTestPlot', fig.align='center', fig.asp=0.5, fig.cap="Porportion of abnormal score per measure or task split by groups.", fig.width=14, out.width='100%', echo=FALSE}
...
```{r, label='PCAST-fig', fig.cap="Switching task PCA: Input variables as a function of PCA components: PC1.ST vs. PC2.Material (A), PC1.ST vs. PC3.Nz (B). ASL conditions are marked in circles and CCRM data in rectangles. Filled shapes denotes conditions with speech distractors and non-filled shaps denotes nonspeech conditions.", fig.align='center', fig.width=8, fig.asp=0.5, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
```{r, label='PCAST-fig2', fig.cap="Switching task PCA: Comparison between PCA weighted scores and calculated measures: 1) ST = mean score across all ST data, 2) Material = $\\overline{ASL}$ - $\\overline{CCRM}$, 3) Nz = $\\overline{NoSpch}$ - $\\overline{Spch}$.", fig.align='center', fig.width=11, fig.asp=0.8, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
my_fn <- function(data, mapping, ...){
  ggplot(data = data, mapping = mapping, ...) +
    geom_point(size=0.5,na.rm=TRUE) + 
    geom_smooth(method=lm,fill="#69b3a2", se=FALSE, na.rm=TRUE) +
    stat_cor(method = "pearson", na.rm = TRUE, size=3,label.y.npc = "top") +
    guides(fill=guide_legend(title="Group")) +
    theme(legend.position = "bottom",
          legend.direction = "horizontal")
}
...
```{r, label='PCAST-fig3', fig.cap="Switching task PCA: Listeners weighted scores split by components.", fig.align='center', fig.width=5, fig.asp=0.6, out.width='85%',echo=FALSE, message=FALSE, warning=FALSE}
# PCA individual scores (res$ind$coord)
...
```{r, fig.align='center', fig.asp=0.5, fig.cap="Language measures PCA: Individual scores for loadings in PC1.Lang and PC2.Lang split by groups.", fig.width=7, message=FALSE, warning=FALSE, include=FALSE, label='PCALang-fig', out.width='100%'}
...
```{r eval=FALSE, fig.asp=0.5, fig.cap="Switching task input variables as a function of PCA components: A) PC1.ST vs. PC2.Material, fig.width=8, message=FALSE, warning=FALSE, B) PC1.ST vs. PC3.Nz. ASL conditions are marked in circles and CCRM data in rectangles. Filled shapes denotes conditions with speech distractors and non-filled shaps denotes nonspeech conditions.", fig.align='center',label='PCALang-fig2', out.width='100%',echo=FALSE, results='hide', message=FALSE}
...
t2 <- ggplot(Loadings, aes(PC1.Lang, PC3.Lang)) + 
  ...
  geom_text(label=Loadings$CondCode) +
  # scale_fill_manual(values=c("white","darkgrey","white","darkgrey")) +
...
```{r, label='PCALang-fig1', fig.cap="To keep????????", fig.align='center', fig.width=5, fig.asp=0.6, out.width='85%',echo=FALSE, message=FALSE, warning=FALSE}
# PCA individual scores (res$ind$coord)
...
```{r, label='CorrAll-Tab',echo=FALSE,warning=FALSE,message=FALSE}
...
```{r,label='EvalAPD-ttests',echo=FALSE, message=FALSE, warning=FALSE,results='hide'}
Eval_APD <- merge(d_Info, d_Corr[,c("listener","Group","SRM","PC1.ST","PC1.Lang")],by="listener") %>% filter(Group=="APD") %>% droplevels()
# Eval_APD <- Eval_APD[,-match(c("X"),names(Eval_APD))] 
Eval_APD$SPD <- factor(Eval_APD$SPD,levels=c("1","0"))

# -- Change data layout from Wide2Long -----------------------
Eval_APD_L <- Eval_APD %>% 
  select(listener:PC1.Lang) %>%
  pivot_longer(
...
Eval_APD_L$SPD <- factor(Eval_APD_L$SPD,levels=c("1","0"))
Eval_APD_L$EarProblems <- factor(Eval_APD_L$EarProblems,levels=c("1","0"))
Eval_APD_L$FMuse <- factor(Eval_APD_L$FMuse,levels=c("1","0"))

ST_Diag.t_test   <- t.test(PC1.ST~ Diagnosis, data=Eval_APD, conf.level = 0.95, paired = FALSE)
ST_SPD.t_test   <- t.test(PC1.ST~ SPD, data=Eval_APD, conf.level = 0.95, paired = FALSE)
ST_FM.t_test    <- t.test(PC1.ST~ FMuse, data=Eval_APD, conf.level = 0.95, paired = FALSE)
ST_OME.t_test   <- t.test(PC1.ST~ EarProblems, data=Eval_APD, conf.level = 0.95, paired = FALSE)

Lang_Diag.t_test   <- t.test(PC1.Lang~ Diagnosis, data=Eval_APD, conf.level = 0.95, paired = FALSE)
Lang_SPD.t_test    <- t.test(PC1.Lang~ SPD, data=Eval_APD, conf.level = 0.95, paired = FALSE)
Lang_FM.t_test     <- t.test(PC1.Lang~ FMuse, data=Eval_APD, conf.level = 0.95, paired = FALSE)
Lang_OME.t_test    <- t.test(PC1.Lang~ EarProblems, data=Eval_APD, conf.level = 0.95, paired = FALSE)

SRM_SPD.t_test    <- Eval_APD %>% filter(Group=="APD") %>% t.test(SRM~ SPD, data=., conf.level = 0.95, paired = FALSE)

```

```{r echo=FALSE, fig.align='center', fig.asp=1, fig.cap="Add caption here.", fig.width=7, message=FALSE, warning=FALSE, label='EvalAPD-fig', out.width='85%'}
t1 <- ggplot(Eval_APD_L, aes(x=Diagnosis,y=score)) +
  geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=1,colour="blue")+
...
t2 <- ggplot(Eval_APD_L, aes(x=FMuse,y=score)) +
    geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=1,colour="blue")+
...
t3 <- ggplot(Eval_APD_L, aes(x=EarProblems,y=score)) +
    geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=1,colour="blue")+
...
t3 <- t3 + facet_grid(. ~ CondCode, scales = "free", switch = "y")+
  theme(panel.spacing.x = unit(0,"line"),
        strip.background = element_rect(fill="white", color="white"),
        strip.text = element_text(face = "bold", size = 11))

t4 <- ggplot(Eval_APD[which(Eval_APD$Diagnosis=="APD"),], aes(x=SPD,y=SRM)) +
    geom_boxplot(position=position_dodge(width=0.8),outlier.shape=NA) + 
  geom_quasirandom(dodge.width=0.8,shape=1,colour="blue")+
  scale_x_discrete(labels=c("1" = "SPD", "0" = "APD"))+
...
(t1 + t2 + t3 + t4) + plot_layout(ncol = 2, nrow = 2, heights = c(1,1), widths = c(1,1)) + plot_annotation(tag_levels = 'A') + plot_layout(guides='collect') &
  theme(legend.position='bottom',        
        legend.direction = "horizontal",
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"),
