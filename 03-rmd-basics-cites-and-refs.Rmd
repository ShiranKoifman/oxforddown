...
kbl(PTA_tab, booktabs = T, escape = F, linesep = "", caption = "Standard audiometry: Descriptives for pure-tone detection thresholds (dB\ HL) by frequency (kHz) and ear split by the two groups.",
  ...
  add_footnote(c("PTA: average detection threshold at 0.5, 1, 2, & 4 kHz.","BE: PTA at the better ear."), notation = "none")
```
...
```{r,label='RegAud-PTALMEM-NoNestedData', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide'}

d_PTA_L2 <- d_PTA_L[ ! d_PTA_L$PTA %in% c("PTA_RL","PTA_BE"), ] %>% droplevels()
...
```{r,label='RegAud-PTALMEM-PTA_RL', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}

# -------------------------------------------------------------------------
# PTA_RL
# -------------------------------------------------------------------------

PTA_RL <- d_PTA_L[ d_PTA_L$PTA %in% c("PTA_RL"), ] %>% droplevels()
...
# Parametric method: 
# -------------------------------------------------------------------------
PTA_RL.lm <- lm(dBHL~ Group, data=PTA_RL)
PTA_RL.1wayANOVA <- anova(PTA_RL.lm)

# m <- lm(dBHL ~ Group, data=PTA_RL)
# anova(m) 
...
# Group      1  32.37  32.373  3.8662 0.05606 .
# Residuals 41 343.31   8.373                     
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

t.test(dBHL~ Group, data=PTA_RL, conf.level = 0.95, paired = FALSE)
MKinfer::boot.t.test(dBHL~ Group, data=PTA_RL, paired = FALSE,conf.level = 0.95, R = 9999)
# Results: t = 1.9637, df = 39.96, p-value = 0.05721
...
# linearity is met if p>.05
shapiro.test(residuals(w1))
# W = 0.99106, p-value = 0.9601 --> normally distr.

#### Homogeneity of variance (Levene's test) ###########
# homogeneity is met if p>.05

# Option 1: Is the lm model fully covered here?
leveneTest(residuals(w1) ~ PTA * Group, data = PTA_RL)
# -> homogeneity is met

# Levene's Test for Homogeneity of Variance (center = median)
#       Df F value Pr(>F)
# group  1  0.2673 0.6079
#       41 

# -------------------------------------------------------------------------
# PTA_BE:
# -------------------------------------------------------------------------
PTA_BE <- d_PTA_L[ d_PTA_L$PTA %in% c("PTA_BE"), ] %>% droplevels()
...
# Parametric method: 
# -------------------------------------------------------------------------
PTA_BE.lm <- lm(dBHL~ Group, data=PTA_BE)
PTA_BE.1wayANOVA <- anova(PTA_BE.lm)
...
# Response: dBHL
#           Df Sum Sq Mean Sq F value Pr(>F)
# Group      1  16.50 16.4978  1.9383 0.1714
# Residuals 41 348.98  8.5117                  
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

t.test(dBHL~ Group, data=PTA_BE, conf.level = 0.95, paired = FALSE)
MKinfer::boot.t.test(dBHL~ Group, data=PTA_BE, paired = FALSE,conf.level = 0.95, R = 9999)
# Results: t = 1.9637, df = 39.96, p-value = 0.05721
...
<!-- Group differences for PTAs (PTA$_{Left}$, PTA$_{Right}$, and PTA) and BE were examined using multiple comparisons t-tests with Bonferroni correction (rstatix package, REF). Note that the TD children were considered as a single group, since there was no significant difference in thresholds across the TD children with or without an APD sibling. Groups descriptive and test statistics are given in Table\ \@ref(tab:RegAud-PTAttest)]. There was a significant difference between the groups for PTA measured in the left ear (p \< 0.05). However, a group difference of 2.5\ dB is rather small and clinically negligible, and is likely to occur due to sampling error. No significant difference was found in the remaining measures (all p's \> 0.05).\ -->
...
As in the standard audiometry, group differences for PTAs (PTA$_{Left}$, PTA$_{Right}$, and PTA) and BE were examined using multiple comparisons tests. As before, the TD group was treated as a single group since no significant difference was found between TD children with or without an APD sibling. Parametric model assumption of normal distribution was rejected (Shapiro-Wilk test; p\ \<\ 0.05), while the assumption of homoscedasticity was met. Therefore, Wilcoxon rank-sum test was used with Bonferroni correction (see Table \@ref(tab:EHFAud-PTAttest)). There was no significant difference between the groups for any of the PTAs (all p's \>\ 0.05).\

<!-- Similarly, an additional nonparametric 4 x 2 factorial design model was used to examine the difference between the two groups (APD/TD) for the four combined threshold measures (PTA$_{Left}$, PTA$_{Right}$, PTA, and BE). As before, the TD group was treated as a single group since no significant difference was found between TD children with or without an APD sibling. Parametric model assumption of normal distribution was rejected (Shapiro-Wilk test; p\ \<\ 0.05), while the assumption of homoscedasticity was met. The nparLD ATS test found no significant two-way interaction between Group and PTAs nor a main effect of groups, while there was a significant main effect of PTAs (see Table \@ref(tab:EHF-PTATabnparLD)).\ -->


```{r,label='EHFAud-PTAstats', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide'}
...
ttest.EHFPTA_Wilcox <- d_HF_PTA_L %>% rstatix::group_by(PTA) %>%
  ...
  dplyr::rename("p-value"=p.adj, "Estimate"=estimate)

ttest.EHFPTA_Wilcox$`p-value` <- ifelse(ttest.EHFPTA_Wilcox$`p-value` <.001,"\\textbf{< 0.001}", 
                             ifelse(ttest.EHFPTA_Wilcox$`p-value` <.05, sprintf("\\textbf{%.02f}",ttest.EHFPTA_Wilcox$`p-value`),
                                    ttest.EHFPTA_Wilcox$`p-value`))

colnames(ttest.EHFPTA_Wilcox)[4] = sprintf("95\\%%-CI")

  
# ttest.EHFPTA_effsize <- d_HF_PTA_L %>% rstatix::group_by(PTA) %>%
#   rstatix::wilcox_effsize(data=., dBHL ~ Group) %>% 
#   dplyr::rename("r"=effsize) %>% 
#   select(r,magnitude)  %>% 
#   mutate_if(is.numeric, round, 2)


EHFPTA_d <- d_HF_PTA_L %>% rstatix::group_by(PTA) %>%
    rstatix::cohens_d(dBHL ~ Group, data=., ci = TRUE) %>% as.data.frame() %>%
  mutate_if(is.numeric, round, 2) %>%
  select(effsize,magnitude) %>%
  dplyr::rename("d"=effsize)

# ttest.EHFPTA <- cbind(PTA=c("PTA$_{Left}$","PTA$_{Right}$","PTA","BE"),ttest.EHFPTA,EHFPTA_d)
ttest.EHFPTA <- cbind(PTA=c("PTA$_{Left}$","PTA$_{Right}$","PTA","BE"),ttest.EHFPTA_Wilcox,EHFPTA_d)

colnames(ttest.EHFPTA)[1] = ""

# remove attributes as it causes issues with kbl:
ttest.EHFPTA$Estimate <- labelled::remove_attributes(ttest.EHFPTA$Estimate, "difference in location")
ttest.EHFPTA$statistic <- labelled::remove_attributes(ttest.EHFPTA$statistic, "W")
ttest.EHFPTA$d <- labelled::remove_attributes(ttest.EHFPTA$d, "Cohen's d")
```
...
```{r, label='EHF-PTAnparLD', message=FALSE,warning=FALSE, echo=FALSE, results='hide', eval=FALSE}
# PTA
...
```{r, label='EHF-PTATabnparLD', echo=FALSE,fig.pos = "!H", eval=FALSE}
...
CCRM.ST_z_Wilcox <- data.frame(rstatix::wilcox_test(z_trim ~ Group2, data = ST_CCRM,detailed=TRUE, p.adjust.method ="bonferroni")) %>% 
  mutate_if(is.numeric, round, 2) %>%
...
ASL.ST_z_Wilcox <- data.frame(rstatix::wilcox_test(z_trim ~ Group2, data = ST_ASL,detailed=TRUE, p.adjust.method ="bonferroni")) %>% 
  mutate_if(is.numeric, round, 2) %>%
  mutate( "95\\%-CI" = sprintf("%s - %s",conf.low,conf.high),
          p = ifelse(p.adj<.001,"\\textbf{< 0.001}", ifelse(p.adj<.05, sprintf("\\textbf{%.02f}",p.adj),p.adj))) %>% 
  select(n1,n2,estimate,`95\\%-CI`,p)
...
ASL_ST_Z.postT <- cbind("contrast" = c("APD - TD","APD - TD$_{sib}$","TD - TD$_{sib}$"),
