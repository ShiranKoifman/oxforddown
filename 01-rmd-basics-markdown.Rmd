...
On one occasion, a threshold of 30 dB\ HL at 2\ kHz was accepted. Nonetheless, all the participants had a PTA$_{4}$ below 25 dB\ HL, averaged across the frequencies 0.25, 1, 2, and 4\ kHz [@WHO1998], in the left (3.6 $\pm$ 3.6 dB\ HL) and the right ear (4.5 $\pm$ 5.8 dB\ HL). The listeners' thresholds for the left and the right ear are plotted in Fig.\ \ref{fig:PTA-Exp1}. The shaded grey area represents the range of audiometric thresholds at each frequency, while the white line represents the mean of the participants at each frequency. The dashed line represents the threshold criteria. None of the participants reported a history of ear or hearing problems or language or other cognitive impairment.  The Study was approved by the UCL Research Ethics Committee (Project ID Number 0544/006) and testing commenced once an informed consent was given. Participants were recruited from the UCL psychology subject pool and were paid for their participation.
...
```{r, label='PTA-cpt1', fig.cap="Individual pure-tone detection thresholds plotted seperately for the left and the right ear (black lines). The shaded grey area represents the range of the audiometric threholds and the white line represents the mean at each frequency across the listeners. The dashed red line represents the threshold criteria of hearing level $\\leq$ 25\ dB\ HL.", echo=FALSE, fig.align='center', figures-side, out.width='85%',fig.width=12, fig.height=6}
...
```
#### Test-retest
```{r,label='TestRetest-analysis1', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}
...
DSDG_ASL_TarM.MskM_Wilk <- cbind("95\\%-CI"=CI1,
                 "p"=p1,ifelse(p1 < .05, "sig.","n.s"),
                 "r"=r1,"magnitude"=magnitude1)

# Cohen's d:
# effsize::cohen.d(DSDG_ASL_TarM.MskM$uRevs[which(DSDG_ASL_TarM.MskM$Session=="1")],
#                  DSDG_ASL_TarM.MskM$uRevs[which(DSDG_ASL_TarM.MskM$Session=="2")])
...
# save test results by names
DSDG_CCRM_TarM.MskM_Wilk <- cbind("95\\%-CI"=CI2,
                 "p"=p2,ifelse(p2 < .05, "sig.","n.s"),
                 "r"=r2,"magnitude"=magnitude2)

# Cohen's d:
# effsize::cohen.d(DSDG_CCRM_TarM.MskM$uRevs[which(DSDG_CCRM_TarM.MskM$Session=="1")],
#                  DSDG_CCRM_TarM.MskM$uRevs[which(DSDG_CCRM_TarM.MskM$Session=="2")])
...
# save test results by names
SSDG_CCRM_TarM.MskM_Wilk <- cbind("95\\%-CI"=CI3,
                 "p"=p3,ifelse(p3 < .05, "sig.","n.s"),
                 "r"=r3,"magnitude"=magnitude3)

# Cohen's d:
# effsize::cohen.d(SSDG_CCRM_TarM.MskM$uRevs[which(SSDG_CCRM_TarM.MskM$Session=="1")],
#                  SSDG_CCRM_TarM.MskM$uRevs[which(SSDG_CCRM_TarM.MskM$Session=="2")])


##### get table #####

tr_Wilk <- rbind(DSDG_ASL_TarM.MskM_Wilk,DSDG_CCRM_TarM.MskM_Wilk,SSDG_CCRM_TarM.MskM_Wilk)
tr_Wilk <- tr_Wilk[,- 3]
...
```{r,label='TestRetest-analysis2', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}
# Idea: use two separate models to examine the effect of session 
# 

# get separate data frames:

# CCRM 
CCRM_tr <- d_r %>% filter(Material!="ASL", Background!="Quiet", Configuration=="TarM+MskM") %>% droplevels() 
ezPrecis(CCRM_tr) 

CCRM_tr.nparLD <- nparLD(uRevs ~ Background*Session, data = CCRM_tr, subject = "Listener", description = TRUE, plot.CI=TRUE)
CCRM_tr.nparLD <- data.frame(round(CCRM_tr.nparLD$ANOVA.test,3))
#                    Statistic df p.value
# Background             6.729  1   0.009
# Session                1.041  1   0.308
# Background:Session     0.518  1   0.472

CCRM_tr_tab <- CCRM_tr %>% ddply(.,~Background*Session,summarise,N=length(uRevs),mean=mean(uRevs,na.rm=TRUE),sd=sd(uRevs,na.rm=TRUE), min=min(uRevs,na.rm=TRUE),max=max(uRevs,na.rm=TRUE))  %>% 
  mutate_if(is.numeric, round, 2) %>%
  arrange(., group_by = Background,Session)

CCRM_tr_tab <- cbind("Material" = c("CCRM","CCRM"),CCRM_tr_tab[c(1,3),c(1,3:7)],CCRM_tr_tab[c(2,4),3:7])

# ASL
ASL_tr <- d_r %>% filter(Material!="CCRM", Background!="Quiet", Configuration=="TarM+MskM") %>% droplevels() 
ezPrecis(ASL_tr) 

ASL_tr.nparLD <- nparLD(uRevs ~ Session, data = ASL_tr, subject = "Listener", description = TRUE, plot.CI=TRUE)
ASL_tr.nparLD <- data.frame(round(ASL_tr.nparLD$ANOVA.test,3))
#         Statistic df p.value
# Session     0.046  1    0.83

ASL_tr_tab <- ASL_tr %>% ddply(.,~Session,summarise,N=length(uRevs),mean=mean(uRevs,na.rm=TRUE),sd=sd(uRevs,na.rm=TRUE), min=min(uRevs,na.rm=TRUE),max=max(uRevs,na.rm=TRUE))  %>% 
  mutate_if(is.numeric, round, 2) %>%
  arrange(., group_by = Session)

ASL_tr_tab <- cbind("Material" = c("ASL"),"Background" = c("Different material"), ASL_tr_tab[1,2:6],ASL_tr_tab[2,2:6])

tr_tab <- rbind(ASL_tr_tab,CCRM_tr_tab)
tr_tab <- cbind (tr_tab, tr_Wilk)
rownames(tr_tab) <- c()
```
\
...
```{r, label='TestRetest-plot', fig.cap="Test-retest: SRdTs obtained for 'TarM+DstrM' listening configuration in session 1 (x-axis) and session 2 (y-axis). Indivdual SRdTs are represented by the different shapes and colours corresponding to different material (ENG\\_F, circles) and same material (CCRM\\_F, triangle) distractors presented with the ASL (red) or CCRM speech material (cyan). The diagonal line represents the same score in both sessions. The dashed lines represents the task's lower and upper DC limit of 0.05 and 0.97.", fig.align='center', fig.width=7, fig.asp=1, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE}
...
Inspection of the data for parametric assumptions revealed that while homoscedasticity of variance was met, the assumption of normal distribution was rejected for the ASL and the CCRM data measured with the ENG_F (different material) distractor. Therefore, differences between sessions were tested using a noneparametric method using Wilcoxon rank-sum test with permutation (N=999999) (*wilcox_test()* function, coin package, REF). Descriptives and test results are shown in Table\ \@ref(tab:Test-RetestTab). There was no significant difference between SRdTs in the first and the second session across test material and distractor type (all p's \>\ 0.05). Therefore, since only MM condition was tested twice while the rest of the test conditions were only measured once, the individuals' average score across the two sessions was used for further analysis.

```{r, label='Test-RetestTab', echo=FALSE,eval=FALSE}
...
kbl(tr_tab,booktabs = T, escape = F, linesep = "",caption = "Add caption here.",
    align = c("llcccccccccccccl"),format = "latex") %>%
  kable_styling(latex_options = c("scale_down")) %>%
  add_header_above(c(" " =2, "Session 1" = 5, "Session 2" = 5, "Wilcoxon rank-sum test" = 4)) %>%
  column_spec(c(3,8,13),border_left = T) %>%
  column_spec(14, italic = T)
...
Boxplots of the listeners SRdTs split by speech material, background type (Quiet, ENG_F \& CCRM_F) and listening configuration are shown in Figure\ \@ref(fig:AverageDC-plot).
...
```{r, label='AverageDC-plot', fig.cap="Boxplots of the listeners SRdTs split by speech material (ASL/CCRM), background type (Quiet, ENG\\_F and CCRM\\_F) and listening configuration (binaural: TarB and TarB+DstrB; monaural: TarM and TarM+DstrM; loosely monaural: TarM+DstrB). The boxes are colour-coded, with red, green, and blue, marking the binaural, monaural and loosely monaural condition, respectively. The dashed lines represents the task's lower and upper DC limit of 0.05 and 0.97.", fig.align='center', fig.width=10, fig.asp=.9, out.width='100%',echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r,label='get-separateDFs', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}
#---------------------------------------------------
## get separate data frames 
#---------------------------------------------------
...
```{r,label='DC-LMEM-Q1', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}
# ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Post hoc: emmeans package
# See: https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html
# For interactions, see: : https://cran.r-project.org/web/packages/emmeans/vignettes/interactions.html

# plot
emmeans::emmip(BestModel, Material ~ Configuration)

# separate pairwise comparison per configuration
emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ Configuration | Material)
# separate pairwise comparison per material
emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ Material | Configuration)

# Material x Configuration:
# Plot: 
test.emm.s <- emmeans::emmeans(BestModel,  "Material","Configuration")
plot(test.emm.s, comparisons = TRUE)
...
# blue bars: CIs
# red arrows: for comparisons of CI's. If an arrow from one mean overlaps an arrow from another group, the difference is not "significant.
...
# plot
emmeans::emmip(BestModel, ~ Configuration)

# separate pairwise comparison per configuration
emm_s.conf <- emmeans::emmeans(BestModel, pairwise ~ Configuration)

# Plot: 
emm_s.conf <- emmeans::emmeans(BestModel,  "Configuration")
plot(emm_s.conf, comparisons = TRUE)
...
# get table
# -------------------------------------------------------------------------
# Quiet_Tab <- data.frame(rbind(
#   c("Material",CondCode.LMEM$Df[2],CondCode.LMEM$Chisq[2],sprintf("\\textbf{<0.001}")),
#   c("Configuration",Age.LMEM$Df[2],Age.LMEM$Chisq[2], sprintf("\\textbf{<0.001}")),
#   c("Material:Configuration",APDsib.LMEM$Df[2],APDsib.LMEM$Chisq[2],sprintf("%.03f",APDsib.LMEM$`Pr(>Chisq)`[2]))))
# 
# ENVASA_AgeTab[,c(2:3)]= apply(ENVASA_AgeTab[,c(2:3)], 2, function(x) as.numeric(as.character(x)))
# 
# colnames(ENVASA_AgeTab)[1:4] <- c("Main effects","Df",sprintf("$\\chi^{2}$"),"p")
...
```{r,label='DC-LMEM-Q2', message=FALSE, warning=FALSE, include=FALSE, echo=FALSE,results='hide',eval=FALSE}
...
**Quiet Model:** uRevsM ~ Material $\cdot$ Configuration + (1 | Listener) 

- There was a significant Background x Connfiguration interaction 

- Post-hoc tests: all Material x Configuration pairs were significant. --> add effect size Cohen's d!

**ASL model (ENG_F):** uRevsM ~ Configuration + (1 | Listener) 
...
- Post hoc-tests using lsmeans: sig. difference btw BB vs. MM and BB vs. MB. No sig difference btw MM vs. MB -> ceiling effect!

**CCRM model (ENG_F,CCRM_F):** uRevsM ~ Background + Configuration + (1 | Listener)
...
**Differences between materials for ENG_F only:** uRevsM ~ Material + Configuration + (1 | Listener)
...
```{r, label='Quiet_PostHocTab', echo=FALSE,eval=FALSE}
# prepare table using kbl()
# for more options see: https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_pdf.pdf

kbl(data.frame(Quiet_PostHoc),booktabs = T, escape = F, linesep = "",caption = "Quiet LMEM: Material x Configuration post-hoc.",
    align = c("lccccc"),format = "latex",digits = 2) %>%
  kable_styling(latex_options = c("scale_down")) %>%
...
Why? -> Due to the close nature of the responses in the CCRM material..
